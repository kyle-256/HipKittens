...
**Net: dead-flag close-out. 3 gated branches landed (commit `0f57bd8e`)
with `BF16_RCR_PADDED_B128 = 0` default; flag=0 .so SHA256-identical to
pre-P23 baseline (`47f76ea6`). Flag=1 builds clean and PASSES the Step 5
correctness gate on rcr/rrr/crr but REGRESSES perf 5-7pp on every RCR
weak shape due to a fundamental bank-conflict miscalculation in the P22
design.**

### Why dead — empirical PMC contradicts Reviewer's analysis

| Counter                  | flag=0 (baseline) | flag=1+fix | Δ        |
|--------------------------|-------------------|------------|----------|
| GRBM_GUI_ACTIVE          | 9,853,571         | 11,350,503 | +15.19%  |
| SQ_INSTS_VALU_MFMA_BF16  | 58,720,256        | 58,720,256 | 0%       |
| SQ_INSTS_LDS             | 22,020,096        | 22,020,096 | 0%       |
| **SQ_LDS_BANK_CONFLICT** | **0**             | **88,080,384** | **+88M (∞%)** |
| SQ_WAIT_INST_LDS         | 47,767,295        | 64,643,795 | +35.33%  |

Per-lane formula `64*M + 16*K_oct` walked against gfx950's 32-bank/4-byte
model:
- M-stride = 64 B = 16 dwords → mod 32 = 16 → only 2 distinct banks across M.
- K-octet stride = 16 B = 4 dwords → covers 8 banks.
- Combined: 8 of 32 banks engaged per cycle, 8-way conflict every cycle.
- 32B inter-subtile padding is bank-page-aligned (8 dwords) — shifts the
  pattern but does not reduce conflict count.

The Reviewer's "all 32 banks engaged per cycle" derivation in REVIEW.md
§1c/§2 was off by exactly the M-stride × bank-page coincidence. The
P22 Dev C probe analytically validated the *pattern* but missed that the
write/read pattern composes to alias all 8 K-octet groups onto only 8
banks under MFMA col_l/row_l lane mapping.

### Why baseline framing was wrong

Project memory `project_bf16_rcr_padded_b128_design.md` claimed
"TK BANK_CONFLICT/MFMA on RCR worst shape = 67.6M / 90.2M". Dev D's
PMC measurement: **baseline RCR = 0 SQ_LDS_BANK_CONFLICT**. The 67.6M
was P21 Dev C's measurement on **RRR**, not RCR (project memory
`project_bf16_rcr_rrr_weak_shape.md` notes "RRR=100% intra-subtile bank
conflict"). The Route 1 design was attacking a non-existent bottleneck.

### Real bug worth keeping (Dev E)

`load(row_l)` Branch A in `shared_to_register.cuh` integer-truncates
`subtiles_per_col = ST::rows / RT::tile_size_row = 32/64 = 0` when the
st_subtile is half-height of the underlying subtile. Outer load loop
runs zero times → register tile uninitialized → NaN. New gated branch
(Dev E, ~150 lines, `BF16_RCR_PADDED_B128`) handles half-height
st_subtiles and honors `src.row_offset`/`col_offset`. Real bug; may
matter for future layouts; landed dead-flag.

### What we ship from P23

- Commit `0f57bd8e`: 3 gated branches + kernel wiring; flag default 0;
  flag=0 .so byte-identical to `47f76ea6...`.
- The row_l half-height fix is genuinely useful infrastructure.
- The col_l Path A and Path B branches are dead but preserved in case a
  future redesign (different swizzle / different K-octet stride) revives
  the lever.

### What to do next (out of scope for P23 — pivot)

1. **Re-evaluate op-reuse deficit lever.** TK 8-wave 2M×4N achieves
   2.67 MFMAs/b128 vs BL MIWT8_8 4-wave 2M×2N at 4.0. This is RCR's
   actual bottleneck (per `project_bf16_rcr_rrr_weak_shape.md`), not
   bank conflicts. Wave-grid change requires VGPR reduction = multi
   session work.
2. **Reopening padded-b128 lever** would require XOR-based swizzle to
   break 16-dword M-stride bank alignment, OR 32B K-octet stride which
   forces `rt_16x32_s` redesign. Both are major scope changes.
3. **CRR + RRR Route 1** (separate BL `Ailk_Bjlk` reverse-engineering)
   was always deferred — still deferred; same caveat now applies (BL
   transfer assumption needs PMC validation before design).

## [P23 Session 1 archive] Earlier Status (2026-04-18, post-P23 Session 1 close — Route 1 swizzle reconciled to identity row-major; additive shape `st_64x32_padded_b128` landed; Step 5 correctness gate added; Path A approved for Step 4)
...
**Net: design APPROVED-WITH-AMENDMENTS by Reviewer; one critical
framing correction overrides P21 Dev A's memo; one autotuner-fix
lever (P21 Dev I deferred) DEFINITIVELY CLOSED.**

- **Dev A (TK current LDS write-pattern characterization, opus,
  worktree `p22-dev-a-tk-write-pattern`, commit `0c6f2b5a`)** —
  built a TK LDS-write probe (`probe_lds_writes/probe_write.cpp`) and
  derived the row-pair-interleaved swizzle Q5 candidate. **Hit the
  gfx950 DTL `vmcnt`/`__syncthreads()` race** on first attempt
  (silently dropped lanes ≥32); fixed with `s_waitcnt vmcnt(0)` ahead
  of the dump barrier. Output captured in
  `feedback_dtl_probe_kernel.md`. Q5 swizzle proposal applies to
  CRR (b64_tr_b16 read path), NOT RCR — see Dev B's correction.
...
- **Dev B (BL RCR write-pattern reverse-engineering, opus, worktree
  ...
  **Critical framing correction**: BL RCR uses **0×
  `ds_read_b64_tr_b16`, 0× `v_perm_b32`, only 248× `ds_read_b128`
  (64 in main loop)**. The s83/s84 v_perm pattern P21 Dev A
  attributed to RCR was actually from BL's CRR kernel. Closed-form
  BL write swizzle: `lds_byte_offset(M_row, K_col_octet) =
  K_col_octet * 4224 + M_row * 16 = K_col_octet * 4 * (1024 + 32) +
  M_row * 16`. Column-major over 8 K-octets per A-tile, +32B
  padding per 1024B subtile breaks the LDS-bank alias. Reviewer
  independently re-verified via `grep` on
  `/tmp/p22_dev_b/bl_main_loop.s` + all-pairs bank-conflict counter
  in `/tmp/p22_reviewer/check_bank_conflict.py`.

- **Dev C (design synthesis, opus, worktree `p22-dev-c-design-synth`,
  commit `5f01551b`)** — produced `DESIGN.md` and probe-kernel
  validation (`probe_st_padded_b128/probe.cpp`). Original proposal:
  `st_64x8_padded_b128`. Probe analytically validates 0 bank
  conflicts across all 8 K-octets and 8 cycle groups. **Reviewer
  amendment**: `st_64x8` fails BOTH template branches in
...
**P23 dispatch (next session) — 4-4.5 agent-sessions for full
RCR `st_64x32_padded_b128` implementation:**

See `project_bf16_rcr_padded_b128_design.md` for the 8-step plan:
1. (0.25 sess) Re-run probe with `subtile_rows=64, subtile_cols=32,
   subtile_padding=32` (4096-B subtile re-proof for Reviewer's
   amended shape).
2. (0.25 sess) Add `st_64x32_padded_b128` to
   `include/types/shared/st_shape.cuh` (additive); update `concept
   all` whitelist; verify per-block LDS ≤32 KB at occupancy=2.
3. (0.5 sess) Verify `prefill_swizzled_offsets` threads
   `subtile_padding` correctly (LDS-peek unit test).
4. (1.5-2 sess) Add `ds_read_b128` dispatch branch to
   `shared_to_register::load(col_l)` (`shared_to_register.cuh:195`)
   for the new shape; lane formula `row_offset = laneid,
   col_offset = 0`; pre-commit hard-coded fallback. **Trickiest step.**
...
## [P22 archive] Earlier Status (2026-04-18, post-P21 close — ALL short-term BF16 RCR/RRR levers EXHAUSTED; only Route 1 LDS write-side swizzle remains, multi-session)

P21 dispatched 9 Devs in parallel against the BF16 RCR/RRR weak-shape
bottleneck (and Dev A on the cross-cutting probe kernel). **Net:
zero commit-worthy perf wins; one architectural unblocker (Dev A);
six definitive lever closures; the BF16 short-term optimization arc
is now exhausted.**

- **Dev A (probe kernel for ds_read_b64_tr_b16 lane semantics, GPU 5,
  opus, worktree `p20-dev-a-lds-b128`, commit `514cc3da`)** — built a
  standalone 32-lane probe (gfx950) that initialises LDS with `sm[i]=i`
  and dumps per-lane register state for both `ds_read_b64_tr_b16` and
  `ds_read_b128`. Verdict: **Route 2 (intra-lane v_perm fixup) is
  infeasible** — 0/64 lanes can satisfy their b64_tr_b16 expected
  payload from their own b128 payload. b64_tr_b16: lane L holds 4
  bf16 = (col=L, rows base+0..3) [hardware 4-row transpose-on-read];
  b128: lane L holds 8 bf16 = (row=L%16, cols 8·(L/16)..+7) [linear].
  BL avoids this by **pre-shuffling on the LDS WRITE side** — its
  main loop uses ONLY intra-lane `v_perm_b32` with selectors
  `s83=0x05040100, s84=0x07060302`. Route 1 (LDS write swizzle to
  match BL) is the ONLY remaining path forward; multi-session.

- **Dev B (CRR_BARRIER_MODE research, opus, commit `fe46e941`)** —
  gated dead-code `CRR_BARRIER_MODE` for relaxed barrier cadence.
  V1 +0.10pp sub-noise (SNR=−8.89dB). Closed.

- **Dev C (BF16 RCR + RRR weak-shape PMC characterization, GPU 4,
  opus, worktree `p21-dev-c-rcr-rrr-weak`, commit `e2b2c9c9`)** —
  rocprofv3 + per-iter disasm. RCR worst (4096,28672,4096) ratio
  0.9634, bottleneck = 1.49× LDS reads/MFMA from TK's 8-wave 2M×4N
  warp grid (2.67 MFMAs/b128) vs BL MIWT8_8's 4-wave 2M×2N (4.0).
  RRR worst (8192,3584,18944) ratio 0.9398, bottleneck = same op-
  reuse deficit PLUS 100% intra-subtile bank conflict on B-tile
  (33.9M conf / 33.9M reads vs BL 1%; WAIT_INST_LDS 14.7× BL). L1
  (launch overhead <0.4%) + L2 (KI=296 SGPR pseudo-spill not
  present) closed. See `project_bf16_rcr_rrr_weak_shape.md`.

- **Dev D (BF16 RCR/RRR m0-broadcast hoist port, GPU 4, opus,
  worktree `p21-dev-d-bf16-m0-hoist`)** — ported FP8 P19 inline-asm
  m0-hoist to BF16 RCR + RRR via `bf16_dev_d::load_hoist` helper
  gated `BF16_HOIST_M0=0`. **Reviewer (paired-bench GPU 4, 4 runs
  interleaved) verdict: NEUTRAL — sub-SNR on all layouts (RCR
  +0.071pp / RRR +0.015pp / CRR -0.081pp).** Disasm proves the hoist
  did nothing structurally: `s_mov m0` count 96→96 unchanged.
  Mechanism: **BF16 baseline already calls `to_sgpr_u32 ==
  __builtin_amdgcn_readfirstlane` inside the kittens warp `load(...)`
  impl** (`warp/memory/tile/global_to_shared.cuh:286-306`), so LLVM
  already pre-scalarizes the LDS-byte address. **The FP8 P19 CSE-
  failure precondition does NOT apply to BF16.** Diff discarded in
  worktree. NOT committed. Closed. See `project_bf16_rcr_rrr_weak_shape.md`.

- **Dev E (BF16 RRR padded-LDS-swizzle, opus, commit `eef7a37d`)** —
  ...
  subtile, which breaks `ds_read_b64_tr_b16` lane-transpose semantics
  (same blocker as Dev A). Closed.

- **Dev F (warp-tile geometry analysis for BF16 RCR, opus, commit
  `0b6458d3`)** — characterized: TK 8-wave 2M×4N (0.375 LDS/MFMA) vs
  BL 4-wave 2M×2N (0.250 LDS/MFMA). Conservative projection: +1.1pp
  geomean from operand-reuse. Top blocker: VGPR pressure
  240 → ~380 logical for restructure.

- **Dev G (4-wave 8×8 BF16 RCR VGPR feasibility, opus, commit
  `1125d1f4`)** — Gate A: G1 variant fits clean at
...
- **Dev H (4-wave 8×8 BF16 RCR perf gate B, opus, commit `04bc643b`)** —
  Gate B: G1 **regresses 25-35pp catastrophically** (SNR 43 dB on
  worst shape). Mechanism: the transplanted 8-wave `s_waitcnt`
  watermarks (`vmcnt(4)`, `lgkmcnt(8)`) stall heavily on 4-wave
  hardware where each MMA phase is 4× longer. The -33% LDS issue
  rate materializes but the wave-count drop dominates by ~2 orders
  of magnitude. **L5 verdict: CLOSED.** A from-scratch 4-wave
  pipeline rewrite (mirroring BL's actual MIWT8_8 schedule, not the
  transplanted 8-wave TK schedule) might recover the loss but is
  multi-week work with no confidence. ROI poor vs other levers.

- **Dev I (HBM fetch decomposition for BF16 RCR/RRR, opus, commit
  `218fead9`)** — TK and BL issue identical L1→L2 read requests; the
  +19/+36% HBM gap is L2 cache-line eviction driven by (group_m, xcd)
  choice. Single fixable case identified for alt_rcr (8192,22016,4096)
  via autotuner NREPEAT bump (3 → 8-10). Estimated +0.9pp single-
  shape recovery. **Deferred to P22** (bench runtime grows ~3.3×).

**P21 close-out summary table:**
...
| L_M0_HOIST BF16 RCR/RRR            | CLOSED | structurally inert (baseline pre-scalarizes) — Dev D + Reviewer |
| L_HBM_FETCH (alt_rcr autotuner)    | DEFER  | +0.9pp single-shape, P22                          |
| Probe kernel Route 2 (v_perm)      | CLOSED | 0/64 lanes feasible — Dev A                        |
| Probe kernel Route 1 (LDS write swizzle) | OPEN | only path forward, multi-session — Dev A unblocked |

**P22 dispatch (next session):**

The BF16 short-term optimization arc is exhausted. The only remaining
path is **multi-session Route 1 work**: pre-shuffle the LDS write
pattern so subsequent `ds_read_b64_tr_b16` reads (a) avoid intra-
subtile bank conflicts on B (RRR L_BANK_RRR), AND/OR (b) reduce LDS
read pressure from 4×b64_tr_b16 to 1×b128 + intra-lane v_perm
(CRR H1).

Recommended P22 dispatch (3 sub-sessions, see
`project_bf16_probe_kernel_dev_a.md` for details):
1. **Session 1 — characterize TK's current LDS write pattern** for
   `st_32x16_s` and `st_16x32_s`; map to BL's write pattern; identify
   the swizzle delta. Output: design doc + analytical swizzle
   selectors that recover BL's `s83=0x05040100, s84=0x07060302`
   intra-lane post-load fixup.
2. **Session 2 — prototype `st_32x16_swizzled` (new shared-tile
   type, additive — does not change existing path)**; verify
   `ds_read_b64_tr_b16` on this new type produces the same logical
   operand. Probe-kernel validation only; no production wiring.
3. **Session 3+ — thread through CRR/RRR layouts**; PMC validation;
   cross-GPU paired bench. Estimated upside: 3-5pp CRR / 5pp RRR
   if Route 1 succeeds.

P22 may also opportunistically apply Dev I's NREPEAT 3→8 bump if
bench-runtime tradeoff is acceptable for that session.

## [P21 archive] Earlier Status (2026-04-18, post-P20 close — H1 (b128+v_perm) DEFER pending probe kernel; H4 (setprio) CLOSED; H6 (barrier reduction) is new candidate; P21 = probe kernel + H6 research)
