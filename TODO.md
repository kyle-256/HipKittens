...
## R55 cycle 完结 (2026-04-20, 6 devs + 1 reviewer carry-over) ★ 0 SHIP + 5 REFUTATIONS + 1 DIAGNOSTIC + 1 BASELINE — 70B Down RCR DIAGNOSTIC-SCALE-FETCH-WAIT class attacked on FOUR axes (P1 interleave, P2 SALU hoist, P3 VALU dep-break, P4 L2 residency); all four REFUTED-EMPIRICAL or REFUTED-PASS-CRITERION. Convergent evidence: V2 RCR HEADROOM is not addressable by source-level scale-load reordering — LLVM scheduler + LICM + strength reduction already optimize the obvious levers; CRR LDS-V2 COMPRESSED retry also REFUTED (LDS round-trip dominates 6-cycle v_lshrrev saving)

R55 派 6 devs (A P1 scale-fetch interleave, B P2 SALU hoist, C P3 VALU dep break, D P4 scale L2 residency, E CRR LDS V2 retry on compressed layout, F PMC replication on 8B GU RRR -1.0pp HEADROOM). Bottleneck targets driven by R54 Dev B's DIAGNOSTIC-SCALE-FETCH-WAIT class identification on 70B Down RCR. All 6 devs delivered REFUTED, REFUTED-PASS-CRITERION, or DIAGNOSTIC. Reviewer was carry-over from R54 (9-cell baseline check at HEAD `9ab79eff`).

### R55 commits on `feat/mxfp8-only`
- `4c099d62` R55 Dev F — 8B Gate/Up RRR PMC diagnostic — DIAGNOSTIC-SCALE-FETCH-WAIT-SALU-DOMINANT — new V2 RRR bottleneck class identified (SALU 3.4× R54B's RCR signature); 70B_Down RRR INVERTED 110.3% MX/FP8 (over-shoots FP8 baseline)
- `9acb5fdf` R54 Reviewer — 9-cell baseline at `9ab79eff` — 2/9 PASS, 7/9 HEADROOM (regression vs prior 5/9 PASS); 7 HEADROOM cells in 94.15-94.89% noise-band drift; dominant gap remains 70B_GateUp/CRR at 88.95%; SNR 49.59-49.61 dB and det all PASS
- `21d2656b` R55 Dev C — P3 VALU dependency break — REFUTED-PASS-CRITERION-EMPIRICAL — `sched_barrier(0)` before 11 setprio(1) sites yielded clean monotonic +0.65% (median 2930.69 → 2949.67 TFLOPS, every treatment run ≥ best baseline) with VGPRs 254 → 250, but +0.65% < +1% pass threshold; Phase 0 proved no broadcast→MFMA chain exists
- `eb7483c2` R55 Dev B — P2 SALU hoist — REFUTED-EMPIRICAL-COMPILER-ALREADY-HOISTED — V2-RCR K-loop body byte-identical SALU/MFMA between baseline and macro1 (15 s_add + 13 s_addc + 3 s_addk_i32 + 128 v_mfma); compiler already strength-reduces `k_pair<<N` to single `s_addk_i32` stride accumulator and LICM-hoists scale SRDs to prologue
- `47e63d70` R55 Dev E — 70B Gate/Up CRR LDS-resident scale layout COMPRESSED retry (R54A successor) — REFUTED-EMPIRICAL — LDS round-trip dominates 6-cycle v_lshrrev saving (median -7.98%, monotonic regression, SNR 49.60 dB / det 3/3 PASS preserved)
- `10632347` R55 Dev D — P4 scale L2 residency — REFUTED-EMPIRICAL — B-scale-only cachepolicy bias closes the unswept half of cachepolicy axis (R47C unified, R48F A-only, R55D B-only); all four configs (default_off/GLC/SLC/GLC|SLC) within ~10 TFLOPS noise (2960-2967 TFLOPS); cachepolicy axis TRIPLY CLOSED for V2 RCR scale loads
- `eeec1255` R55 Dev A — P1 scale-fetch interleave — REFUTED-EMPIRICAL-LLVM-RESCHEDULES — 4 INTERLEAVE source-permutation variants V0-V3 across 3 cells all collapse within ±0.5% noise band (70B_Down 2988-3001 TFLOPS, 8B_Down 2966-2989, 70B_QO 2920-2927 partial); ISA inspection confirms LLVM scheduler hoists b64 scale loads back next to b128 mantissa loads regardless of source order — defeats source-level scale/data interleaving

### R55 Dev results (P1-P4 attack on DIAGNOSTIC-SCALE-FETCH-WAIT)
- **Dev A** (P1 scale-fetch interleave, GPU 0) REFUTED-EMPIRICAL-LLVM-RESCHEDULES: 4 source-permutation variants `MXFP8_RCR_SCALE_INTERLEAVE=0..3` reorder b64 scale loads relative to b128 mantissa loads in K-loop. ISA `kernel_mxfp8_layouts.s` shows AMDGPU scheduler clusters all scale loads back together regardless of source order. Phase 2 bench across 70B Down + 8B Down + 70B QO RCR: all 4 variants within ±0.5% noise. Macro `MXFP8_RCR_SCALE_INTERLEAVE` (values 0-3) kept default-OFF.
- **Dev B** (P2 SALU hoist, GPU 1) REFUTED-EMPIRICAL-COMPILER-ALREADY-HOISTED: hoist scale base-address arithmetic out of K-loop body. Disassembly comparison: K-loop body byte-identical (15 s_add + 13 s_addc + 3 s_addk_i32 + 128 v_mfma) between baseline and macro1. Compiler already strength-reduces `(k_pair<<N) + base` → single `s_addk_i32` stride accumulator AND LICM-hoists scale SRD descriptor setup to prologue. Macro `MXFP8_RCR_SCALE_SALU_HOIST` no-op vs compiler. Default-OFF.
- **Dev C** (P3 VALU dep-break, GPU 2) REFUTED-PASS-CRITERION-EMPIRICAL: `sched_barrier(0)` before 11 `s_setprio(1)` sites compresses VALU issue window. Strict-SCLK 5-run/arm: median 2930.69 → 2949.67 TFLOPS (+0.65%, every treatment run ≥ best baseline run). VGPR 254 → 250 (-4 VGPR). Phase 0 ISA proved NO broadcast→MFMA chain exists (would have been the rationale). Clean signal but +0.65% < +1% pass threshold. Macro `MXFP8_RCR_VALU_DEP_BREAK` default-OFF.
- **Dev D** (P4 L2 residency, GPU 3) REFUTED-EMPIRICAL: B-scale cachepolicy bias (the unswept half of the cachepolicy axis: R47C swept unified, R48F swept A-only, R55D sweeps B-only). All four configs default_off/GLC/SLC/GLC|SLC sit within ~10 TFLOPS noise (2960-2967 TFLOPS). Macro `MXFP8_RCR_BSCALE_CACHEPOLICY` (in `r55d_workspace`, never wired into production) default-OFF. **Cachepolicy axis TRIPLY CLOSED for V2 RCR scale loads** (R47C/R48F/R55D).
- **Dev E** (CRR LDS V2 retry, GPU 4) REFUTED-EMPIRICAL: re-attempt LDS-resident scale layout on COMPRESSED scale layout (after R54A overflow at 188416 B vs 163840 B CU limit). New COMPRESSED layout fits in LDS but LDS round-trip latency dominates the 6-cycle v_lshrrev saving. Median -7.98% monotonic regression. SNR 49.60 dB / det 3/3 PASS preserved (numerics fine; perf REFUTED). Macro `MXFP8_CRR_SCALE_LDS_V2` default-OFF. **LDS-resident scale layout family DOUBLY CLOSED** (R54A capacity + R55E latency).
- **Dev F** (8B GU RRR PMC diagnostic, GPU 5) DIAGNOSTIC-SCALE-FETCH-WAIT-SALU-DOMINANT: PMC profiling on -1.0pp HEADROOM cell using R54B's method. Identifies new V2 RRR bottleneck class with **SALU pressure 3.4× R54B's RCR signature** — distinct from RCR's SCALE-FETCH-WAIT. Bonus finding: 70B_Down RRR is INVERTED 110.3% MX/FP8 (over-shoots FP8 baseline). Suggests RRR/RCR have orthogonal bottlenecks; SALU-DOMINANT class needs its own R56+ attack vector (NOT scale-load reordering).

### R55 cycle outcome
**0 new SHIP + 5 REFUTATIONS + 1 DIAGNOSTIC + 1 BASELINE** (Reviewer 2/9 PASS). Cumulative deliverable:
1. **70B Down RCR DIAGNOSTIC-SCALE-FETCH-WAIT class attacked on FOUR axes** (P1/P2/P3/P4 from R54 Dev B prediction). All four mechanisms REFUTED-EMPIRICAL: LLVM scheduler defeats source-level interleave (P1), compiler already hoists SALU (P2), VALU dep-break clean but sub-threshold (P3), cachepolicy axis closed (P4).
2. **Cachepolicy axis TRIPLY CLOSED** for V2 RCR scale loads (R47C unified, R48F A-only, R55D B-only).
3. **LDS-resident scale layout DOUBLY CLOSED** (R54A capacity overflow, R55E latency dominance on COMPRESSED retry).
4. **New SALU-DOMINANT bottleneck class identified** on 8B Gate/Up RRR (Dev F) — distinct from RCR class, requires new attack family.
5. **9-cell baseline drift to 2/9 PASS** (Reviewer): 7 HEADROOM cells in 94.15-94.89% noise-band marginally below 95% gate (vs prior 5/9 PASS); dominant structural gap remains 70B_GateUp/CRR at 88.95%. Likely thermal/SCLK noise drift; not regression from R54/R55 source changes (all macros default-OFF).
6. **Production tree byte-identical post-R55** — all 6 added macros default-OFF; baseline ISA preserved.

### R55 closure ledger (what is now exhausted)
- ~~Source-level scale-fetch reordering (P1)~~ — LLVM reschedules
- ~~SALU base-address hoist (P2)~~ — compiler already does it (LICM + strength reduce)
- ~~Cachepolicy axis V2 RCR~~ — TRIPLY CLOSED (R47C/R48F/R55D)
- ~~LDS-resident scale layout~~ — DOUBLY CLOSED (R54A capacity + R55E latency)
- ~~CRR opsel rewrite~~, ~~K-superblock~~, ~~70B Down CRR~~, ~~N-tile shrink~~ — closed pre-R55
- ~~V2 RRR K-loop body restructure~~ — QUADRUPLY closed (R49A/R53A/R53B/R54C)
- ~~v_pk_lshrrev_b32 ISA opcode~~ — closed by R54E (opcode doesn't exist on gfx950)
- ~~V2 RRR wider memory loads (dwordx4)~~ — closed by R54H (already at gfx950 max width)
- ~~CRR XCD swizzle granularity tuning~~ — closed by R54F (default on flat plateau)

### R56+ candidate levers (post-R55)
1. **8B Gate/Up RRR SALU-DOMINANT class** (R55F new) — orthogonal to RCR; needs SALU-pressure-reducing transforms (NOT scale-load reordering). PMC-driven new attack family.
2. **VALU dep-break sub-threshold +0.65% (R55C)** — keep macro available; if combined with another sub-threshold lever could cross +1%. Co-pilot candidate for stacking.
3. **70B Gate/Up CRR -5.7pp** still open but increasingly looks structural (CRR scale-shift floor 6× closed across R49A/R50A/R53A/R54A/R54E/R54F/R54I/R55E).
4. **PMC profile of 70B Gate/Up CRR** (analog to R54B's RCR PMC + R55F's RRR PMC) — only un-PMC'd HEADROOM cell. Could surface third bottleneck class.
5. **Cell-pair stacking**: combine R55C VALU dep-break (+0.65%) with another sub-threshold lever; production-default-OFF stays default-OFF until joint signal exceeds gate.
6. ~~Compiler-flag exploration~~ — CLOSED by R54C (41 flags swept on V2 RRR, all collapse to identical 254 VGPR).
...
- `4c099d62` Dev F — 8B GU RRR PMC DIAGNOSTIC-SALU-DOMINANT
- `9acb5fdf` Reviewer (R54 carry) — 9-cell baseline 2/9 PASS
- `21d2656b` Dev C — VALU dep-break REFUTED-PASS-CRITERION (+0.65%)
- `eb7483c2` Dev B — SALU hoist REFUTED-COMPILER-ALREADY-HOISTED
- `47e63d70` Dev E — CRR LDS V2 COMPRESSED REFUTED-LATENCY (-7.98%)
- `10632347` Dev D — cachepolicy B-only REFUTED (axis TRIPLY CLOSED)
- `eeec1255` Dev A — P1 scale-fetch interleave REFUTED-LLVM-RESCHEDULES

## R54 cycle 完结 (2026-04-19, 9 devs + 1 reviewer; Dev G MIA) ★ 0 SHIP + 7 REFUTATIONS + 2 DIAGNOSTICS — V2 RRR ceiling QUADRUPLY closed (R54C compiler-flag axis); CRR scale-shift family 6× closed (R54A/R54E/R54F/R54I add to R49A/R50A/R53A); **R54B identifies new bottleneck class DIAGNOSTIC-SCALE-FETCH-WAIT on 70B Down RCR — drives R55 attack plan**

R54 派 9 devs (A CRR LDS-resident pre-shifted scale, B 70B Down RCR PMC, C V2 RRR -mllvm flag sweep, D 8B Q/O wave-tail diagnostic, E v_pk_lshrrev_b32 ISA substitution, F CRR XCD swizzle granularity, G 8B GU RRR cache prefetch builtins, H V2 RRR dwordx4 wider loads, I CRR branchless unconditional shift) + 1 Reviewer. **Dev G MIA**: launched on GPU 6 but produced no artifacts after 3+ hours, declared MIA in cycle wrap. R54 outcome: 0 SHIP, 7 REFUTATIONS + 2 DIAGNOSTICS; key deliverable is R54B's new bottleneck class identification that became the R55 attack target.

### R54 commits on `feat/mxfp8-only`
- `975ce723` R54 Dev A — 70B Gate/Up CRR LDS-resident pre-shifted scale layout — REFUTED-EMPIRICAL — LDS overflow (188416 B vs 163840 B CU limit)
- `9ab79eff` R54 Dev B — 70B Down RCR PMC diagnostic — DIAGNOSTIC-SCALE-FETCH-WAIT — new bottleneck class identified, structurally distinct from CRR scale-shift floor (DROVE R55 P1-P4 ATTACK)
- `a5e9f1ae` R54 Dev C — V2 RRR -mllvm compiler-flag exploration — REFUTED-EMPIRICAL — V2 RRR 254/256 VGPR ceiling QUADRUPLY closed (41 flags swept, 33 built clean, all collapse to 254 VGPR / 0 spill / 0 scratch; best flag +0.122% inside noise band)
- `3ac0fd0e` R54 Dev D — 8B Q/O small-shape wave-tail diagnostic — DIAGNOSTIC-COMPLETE-NOT-A-GAP — all 8B Q/O cells already PASS 95% gate, no wave-tail gap exists
- `34809fa2` R54 Dev E — 70B Gate/Up CRR `v_pk_lshrrev_b32` packed-shift instruction substitution — REFUTED-EMPIRICAL-ISA — opcode does not exist on gfx950 ISA
- `46e73f8f` R54 Dev F — 70B Gate/Up CRR XCD swizzle granularity tuning (NUM_XCDS, GROUP_M sweep) — REFUTED-EMPIRICAL — default (NUM_XCDS=8, GROUP_M=4) sits on flat granularity plateau, no neighbor improves
- `ced8b68c` R54 Dev H — V2 RRR wider memory loads (`buffer_load_dwordx4` coalescing) — REFUTED-EMPIRICAL-ISA — V2 RRR already at gfx950 maximum load width
- `2499ce90` R54 Dev I — 70B Gate/Up CRR branchless unconditional shift block — REFUTED-DIAGNOSTIC — control-flow change SUCCEEDS (no `s_bitcmp0_b32`/`s_cbranch` in body) but compiler still refuses to unroll; CRR ~92% structural floor confirmed independent of conditional control flow

### R54 Dev results
- **Dev A** REFUTED: pre-shifted scale layout in LDS for 70B Gate/Up CRR (avoid the 6× v_lshrrev_b32 K-loop shifts). LDS footprint 188416 B exceeds 163840 B CU limit (gfx950 160 KB LDS/CU). Cannot land at this CTA shape. **Drove R55 Dev E retry on COMPRESSED layout** (also REFUTED, latency this time).
- **Dev B** DIAGNOSTIC-SCALE-FETCH-WAIT: PMC profiling on 70B Down RCR (the -3.4pp HEADROOM cell). Identifies new bottleneck class structurally distinct from CRR's scale-shift floor: **scale-fetch waitcnt dominates K-loop tail**. Predicts P1-P4 attack plan: P1 scale-fetch interleave with mantissa, P2 SALU hoist of scale base addr, P3 VALU dep-chain compression, P4 scale L2 residency hint. **All four P1-P4 mechanisms became R55 dev assignments**.
- **Dev C** REFUTED-EMPIRICAL: 41 -mllvm compiler flag combinations swept on 8B Gate/Up RRR. 33 of 41 built clean → all collapsed to identical 254 VGPR / 0 spill / 0 scratch. Only allocator-mover (`sched_iterative_minreg`) moved wrong direction (more spill). Phase 2 bench: best flag (`max_occ_plus_trackers`) +0.122% — 8× below SHIP gate, inside noise band. **LLVM is schedule-locked at this VGPR pressure point — V2 RRR ceiling QUADRUPLY closed** (R49A/R53A/R53B/R54C).
- **Dev D** DIAGNOSTIC-COMPLETE-NOT-A-GAP: 8B Q/O cells (256 CTA, 0.42 wave) re-validated. All cells already PASS 95% gate; no wave-tail HEADROOM gap exists to close. R49C "wave-tail" framing was misidentified. Removes a phantom target from the open-headroom list.
- **Dev E** REFUTED-EMPIRICAL-ISA: substitute 6× v_lshrrev_b32 (CRR scale shifts) with packed `v_pk_lshrrev_b32`. Opcode does not exist on gfx950 ISA — confirmed via LLVM target table + manual disassembly. ISA-axis closed.
- **Dev F** REFUTED-EMPIRICAL: XCD swizzle granularity sweep on 70B Gate/Up CRR. Default `NUM_XCDS=8, GROUP_M=4` sits on flat plateau — no neighbor (NUM_XCDS=4/8/16, GROUP_M=2/4/8) improves. R47B's CRR XCD swizzle ship continues to hold at default settings.
- **Dev G** MIA: 8B Gate/Up RRR cache prefetch builtins on GPU 6. Launched but produced no artifacts on disk after 3+ hours. Declared MIA — no findings to harvest. Cache prefetch axis remains unswept (R56+ candidate if revisited).
- **Dev H** REFUTED-EMPIRICAL-ISA: V2 RRR `buffer_load_dwordx4` wider memory loads. V2 RRR already uses dwordx4 (16-byte) loads — gfx950 maximum load width. No wider load instruction exists. ISA-axis closed.
- **Dev I** REFUTED-DIAGNOSTIC: branchless unconditional shift block on 70B Gate/Up CRR. Control-flow change SUCCEEDS (verified no `s_bitcmp0_b32` / `s_cbranch` in K-loop body) but compiler STILL refuses to unroll body. **Confirms CRR ~92% structural floor is NOT control-flow gated**; the unroll refusal is independent of `s_bitcmp0_b32`. CRR scale-shift floor closure now 6× across R49A/R50A/R53A/R54A/R54E/R54F/R54I.

### R54 Reviewer audit
9-cell baseline check committed at `9acb5fdf` against HEAD `9ab79eff` (after Dev B PMC commit, before any R55 dev landed). 2/9 PASS, 7/9 HEADROOM. 7 HEADROOM cells in 94.15-94.89% band — marginally below 95% gate, mostly noise-band drift from prior 5/9 PASS R53 baseline. Dominant structural gap remains 70B_GateUp/CRR at 88.95%. SNR 49.59-49.61 dB / det all PASS. **Not regression from R54 source changes** — all R54 macros default-OFF.

### R54 cycle outcome
**0 new SHIP + 7 REFUTATIONS + 2 DIAGNOSTICS + 1 MIA**. Cumulative deliverable:
1. **V2 RRR ceiling QUADRUPLY CLOSED** (R49A/R53A/R53B/R54C). Compiler-flag axis added by R54C. R55+ must NOT propose K-loop body restructure adding inter-iteration state OR new `-mllvm` scheduler/allocator flag exploration.
2. **CRR scale-shift floor 6× CLOSED** (R49A/R50A/R53A/R54A/R54E/R54F/R54I). Includes ISA-opcode-axis (R54E), XCD swizzle granularity (R54F), control-flow / branchless variant (R54I), LDS layout (R54A capacity).
3. **NEW BOTTLENECK CLASS IDENTIFIED** by R54B: DIAGNOSTIC-SCALE-FETCH-WAIT on 70B Down RCR — first concrete optimization path for an open RCR HEADROOM cell. **Drove the entire R55 P1-P4 attack plan**.
4. **Phantom target removed** (R54D): 8B Q/O wave-tail not a gap — all cells PASS.
5. **Production tree byte-identical post-R54** — all R54 macros default-OFF; baseline ISA preserved.

### R54 Cherry-pick status (8/9 on `feat/mxfp8-only`; Dev G MIA)
- `975ce723` Dev A CRR LDS-resident REFUTED (overflow)
- `9ab79eff` Dev B 70B Down RCR PMC DIAGNOSTIC-SCALE-FETCH-WAIT
- `a5e9f1ae` Dev C compiler-flag REFUTED (V2 RRR QUADRUPLY closed)
- `3ac0fd0e` Dev D wave-tail DIAGNOSTIC-NOT-A-GAP
- `34809fa2` Dev E v_pk_lshrrev_b32 REFUTED-ISA (opcode missing)
- `46e73f8f` Dev F CRR XCD granularity REFUTED (flat plateau)
- (Dev G MIA — no commit)
- `ced8b68c` Dev H dwordx4 REFUTED-ISA (already max width)
- `2499ce90` Dev I CRR branchless REFUTED-DIAGNOSTIC (unroll refusal not control-flow gated)
- (Reviewer baseline `9acb5fdf` listed under R55 commits since it was committed during R55 cycle)
...
- `9a246498` R52 Dev G — CRR XCD per-shape un-ship REFUTED (un-shipping regresses -5.40%)
- `34f273db` R52 Dev P — rocprof PMC stall breakdown DIAGNOSTIC-COMPLETE
- `b28c8054` R52 Dev Q — V2 RRR data-load cachepolicy REFUTED
- `12ed1083` R52 Reviewer — R49–R52 cycle audit + 21-cell baseline refresh + 3 verdict flips
- `c9a5e014` R52 Dev R — per-shape N-tile shrink REFUTED-PLUMBING (pre-bench)
- `44265e97` R52 Dev O — SCALE_VERSION=2 RRR dispatch CONFIRM-SHIPPED (V2 already production default since R22-A)

### R52 Dev results
- **Dev G** (`9a246498`) REFUTED: per-shape gate to disable `MXFP8_CRR_BLOCK_SWIZZLE` for B_70B_GateUp_CRR (motivated by R49 Reviewer's reported -3.87% regression). Strict-SCLK 8-run/arm confirm: PRE swizzle-ON med=2488.4 vs POST per-shape-gate-OFF med=2354.1 = **-5.40% regression** (sign-flip from R49). Diagnosis: R49's ON-arm spread of 10.27% landed in unstable-warmup regime; R52G's <1% spread is genuine steady-state. **B_70B_GateUp_CRR re-classified REGRESSION → CONFIRMED.** R47 Dev B's CRR XCD swizzle re-confirmed across all 7 CRR cells.
- **Dev N** (`72a3c217`) NEUTRAL-DIAGNOSTIC: V1 production RRR baseline spill investigation. Confirmed V1's 16 VGPR spill / 68 byte scratch is in **prologue (11 stores) + epilogue (11 reloads)** — K-loop body has ZERO scratch ops. V2 (`Lb1ELi2EE`) has 0 scratch / 0 spill / 254 VGPR. **Framing error**: claimed V2 RRR variant "NOT wired to dispatch by default" — corrected by R52P/R52O.
- **Dev O** (`44265e97`) **CONFIRM-SHIPPED**: V2-RRR is already production default since R22-A (`dabeffa0`). `MXFP8_DISPATCH_TRACE=1` on default env emits `rrr_v2: ... -> RRR-V2-EXACT-8WAVE`. Strict-SCLK A/B re-bench on all 7 RRR cells: V2-WIN +2.10% to +6.80% (mean +4.36%, all SNR ≥ 49.59 dB, det 3/3 PASS). 70B GateUp uses confirm-pass +3.38% (primary sweep had thermal outliers). No source patch authored — R22-A ship is intact.
- **Dev P** (`34f273db`) DIAGNOSTIC-COMPLETE: rocprof PMC profiling on 5 cells (V1/V2 × 8B Gate/Up, 70B Q/O, 70B Gate/Up). V2 8B Gate/Up MfmaUtil 55.35% vs V2 70B Q/O 67.78% (+12.43pp gap). **TA_DATA_STALLED_BY_TC/GRBM: 1.4968 vs 0.9501 (+57%)**. K=4096 confirmed as discriminator (V2 70B Gate/Up at N=28672 has LOWEST stall 0.42). **L2/TC cache-return backpressure dominates the +3.1pp HEADROOM** (hit rate unchanged → bandwidth-shape, not miss-driven). Recommendations: §5.1 cachepolicy (PRIMARY → REFUTED by R52Q), §5.2 K-superblock persistent CTA, §5.3 N-tile shrink (→ REFUTED-PLUMBING by R52R). Re-confirmed 4 prior refutations.
- **Dev Q** (`b28c8054`) REFUTED: V2 RRR data-load cachepolicy sweep at 8B Gate/Up (R52P §5.1 PRIMARY). 5 variants × strict SCLK. CP0 baseline 2549.3 TFLOPS wins; CP1 (R52P primary) -8.71% REGRESSION. All pass SNR=49.61 dB, 3/3 det, ISA-verified flag propagation. Plumbing in 3 files (load_coh template + macro routing) defaults preserve baseline ISA bit-identically. **Why failed**: A reloaded each K-iter via `buffer_load_lds`, R52P's "consumed once per stripe" model wrong. **Cachepolicy lever family closed for V2 RRR at 8B Gate/Up.**
- **Dev R** (`c9a5e014`) REFUTED-PLUMBING (pre-bench): per-shape N-tile shrink at K=4096 RRR (R52P §5.3). `rrr_mxfp8_exact_8wave_fastpath.inc:216-219` hard-codes `BLK==256, WARPS_N==4` via static_assert; V2 scale slab math hard-codes B-side `pack_count=2` (`slab_bytes_b=64*padded_k_blocks`). Multi-day scope across 4 components (kernel template, V2 scale slab, dispatcher, host preshuffle). No GPU work used.

### R52 Reviewer audit (`12ed1083`) — 3 R49 verdict flips
**Phase 1: 5/21 PASS on HEAD `9a246498`** (vs R48 reported 7/21). Re-classified 8192³ RRR/CRR as FAIL because R48 FP8 baseline was contaminated.
- **PASS cells (5/21)**: 8B Down RCR (95.5%) / RRR (107.6%) / CRR (95.2%), 70B Gate/Up RCR (96.5%), 70B Down RRR (108.5%)
- **Phase 2 verdict flips**:
  - **B_8B_Down_CRR**: NOISE → CONFIRMED (+0.97% real)
  - **C_70B_GateUp_CRR**: CONFIRMED → INFLATED (+0.89% real vs +9.74% R49 — 11× SCLK noise inflation)
  - **B_70B_GateUp_CRR**: REGRESSION → CONFIRMED (R52G's confirm proved R49 ON-arm spread was thermal noise)
- **Phase 3**: R52H + R52J ISA diffs re-validate refutations (compiler discards source-position metadata in CRR's manually-interleaved priority block)
- **Open headroom from baseline refresh**:
  - 70B Gate/Up CRR 89.0% (+6.0pp gap)
  - 70B Down CRR 88.0% (+7.0pp gap)
  - 70B Down RCR 90.8% (+4.2pp gap)
- **Methodology note**: 20 min contaminated by sibling reviewer on same GPU 7 — recommends per-GPU exclusivity locking for R53+ cycles.
- Recommendation: **CRR scale-cache layout restructure** is the highest-leverage open R53+ direction (CRR has 6 `v_lshrrev_b32` scale-pack shifts/K-pair vs RCR/RRR opsel = 0 shifts → ~92% structural floor).

### R52 cycle outcome
**0 new SHIP + 1 CONFIRM-SHIPPED + 4 REFUTATIONS + 1 DIAGNOSTIC**. Cumulative deliverable:
1. **V2-RRR ship validated** — was unclear in R52N framing; R52O confirms V2 has been production default since R22-A and beats V1 +2.10%–6.80% across all 7 cells.
2. **TC return backpressure identified** as the dominant cause of the +3.1pp 8B Gate/Up RRR HEADROOM gap (R52P PMC profiling).
3. **Cachepolicy lever family closed** for V2 RRR at 8B Gate/Up (R52Q).
4. **N-tile shrink scope refused** — multi-day plumbing rewrite needed across 4 components, not a one-flag swap (R52R).
5. **3 R49 verdict flips** + 21-cell strict-SCLK baseline refresh (R52 Reviewer): 5/21 PASS under refreshed protocol, validating R47 SHIP gates hold.
6. **CRR XCD swizzle re-confirmed** across all 7 CRR cells (R52G — un-shipping regresses, opposite of R49 Reviewer's noisy reading).

### R52+ remaining candidate levers (R53 directions)
1. **CRR scale-cache layout restructure** (R52 Reviewer recommendation) — eliminate 6 `v_lshrrev_b32` per K-pair via opsel-friendly pack at the host preshuffle layer. R49 Dev A's per-pair re-pack approach failed (forced `v_perm_b32` lane permutations + occ collapse); R53 should attempt a different pack invariant.
2. **K-superblock persistent CTA** (R52P §5.2) — fold multiple K-stripes per CTA so TC-return backpressure amortizes; not pursued in R52 because R52P §5.1 cachepolicy ranked higher (now refuted).
3. **A-side LDS double-buffer at occ=1** for 4096³ shapes — R52K REFUTED at the prototype prep stage; revisit when CRR scale layout work clears VGPR/LDS budget.
4. **8B Q/O RRR/CRR small-shape wave-tail** (256 CTA, 0.42 wave) — R49 Dev C persistent-CTA pre-prototype REFUTED; R53 may need a different framing (e.g., 2-CTA-per-tile cooperative).

### R52 Cherry-pick status (6/6 on `feat/mxfp8-only`)
- `9a246498` Dev G — CRR XCD un-ship REFUTED
- `34f273db` Dev P — TC backpressure DIAGNOSTIC
- `b28c8054` Dev Q — cachepolicy REFUTED
- `12ed1083` Reviewer — baseline refresh + audit
- `c9a5e014` Dev R — N-tile shrink REFUTED-PLUMBING
- `44265e97` Dev O — V2-RRR CONFIRM-SHIPPED

### R52 baseline (5/21 PASS — Reviewer Phase 1; supersedes R48 7/21)
| Shape | RCR% | RRR% | CRR% |
|---|---:|---:|---:|
| 8192³ | (FAIL) | (FAIL) | (FAIL) |
| 8B Q/O | — | — | — |
...
| 8B Down RCR | **95.5** | — | — |
| 70B Q/O | — | — | — |
| 70B Gate/Up | **96.5** | — | (89.0 HR) |
| 70B Down | (90.8 HR) | **108.5** | (88.0 HR) |

PASS cells in **bold**. HR = HEADROOM (open R53+ direction).
...
### R47 Dev results
- **Dev A** (`6edb05f9`) ★ SHIP: 移植 R46 Dev D's XCD swizzle 到 RCR fastpath。Sweep 全部 7 shapes (3-run cooldown medians): **70B Gate/Up RCR +8.46%★**, 8B Q/O +2.85%, 70B Down RCR +2.90%, 8B Down +1.21%, 70B Q/O -0.56%, 8B Gate/Up -0.58%, 8192³ -0.22% (worst -0.58%, within ±2% envelope). `MXFP8_RCR_BLOCK_SWIZZLE` 默认 ON. SNR 49.60 dB + det 3/3 on 8192³/70B Gate/Up. **METHODOLOGY: Initial back-to-back sweep was SCLK-contaminated (-48% on 70B Gate/Up reported) — 3-run cooldown re-bench gave clean numbers. R44/R45 SCLK-contamination pattern recurring.**
- **Dev B** (`0b2f19ae`) ★ SHIP: 移植 XCD swizzle 到 CRR fastpath. Sweep (3-run avg, GPU1): **70B Down CRR +3.35%★**, 8B Q/O +2.70%★, **70B Gate/Up CRR +2.21%★**, 8B Down +1.13%, 8B Gate/Up +0.01%, 8192³ -0.03%, 70B Q/O -1.02% (worst, within envelope). SNR 49.60 dB + det 3/3 PASS. `MXFP8_CRR_BLOCK_SWIZZLE` 默认 ON.
- **Dev C** (`9315a0bf`) PARADIGM CORRECTION + REFUTATION: 84-cell cachepolicy sweep (3 layouts × 4 policies × 7 shapes, 200/400 warmup/iters, median-of-3). Per-layout verdict:
  - RCR: keep p=0. p1 wash (mean Δ -0.02%), p2/p3 lose 2-4% on 5-7/7 shapes. **NO SHIP.**
  - RRR: keep p=0. p1 wash, p2/p3 lose 2-3% on 7/7 shapes. **NO SHIP.**
  - CRR: ★ **REMOVED R28-era conditional auto-default `SLC=2 if (N>=28672 && K>=8192)`**. On 4096×28672×8192 CRR cp0=2502 vs cp2=2464 → SLC LOSES 1.52%. R28's +2.6% win FLIPPED after R44/R45/R46 baseline rose. Re-verified post Dev A/B/D: cp0=2501 vs cp2=2479. Closes R46 Dev A open opportunity #2 (SLC tuning on large-N) — L2 scale pressure modeled real but not addressable by SLC bits.
  - Methodology: v1 sweep 50/100 no preheat had 2 transient outliers (8B_Down p0 1759, 8B_GateUp p3 414). v2 with longer warmup/iters + median-of-3 produced clean ±0.1-1.3%.
- **Dev D** (`c1319949`) ★ HARDWARE-CEILING PROOF: ISA-level MFMA gap verification. **Smoking gun (hex byte encoding)**:
  - FP8 unscaled: `D3AD008E 063A05BC` = **8 bytes** (1 dword pair)
  - MXFP8 scaled: `D3AC0000 00038112 D3AD0892 064A05C6` = **16 bytes** (2 dword pairs)
  - 2× front-end issue slot per MFMA. Predicted: 32 MFMA × ~5% = 5-6% per K-iter. Measured: 8192³ RCR 95.2% (4.8% gap) **matches within noise**.
  - Per-K-pair K-loop inventory MFMA/ds_read/buffer_load_lds/scale_load/s_waitcnt/s_barrier: FP8 64/48/16/0/10/16=234 vs MXFP8 64/48/16/2/11/16=250. **Bodies structurally identical** apart from scaled-MFMA encoding + 2 well-hidden scale loads. **R48+ guidance: STOP RCR MFMA-level optimization on K=8192 shapes (at hardware ceiling).** Headroom only on K/N=28672 shapes.
  - 5 ISA dumps committed under `r47d_isa_dumps/`.

### R47 全量 compute-bound baseline (GPU4, all 4 R47 commits, warmup=100, iters=200)
| Shape | FP8 RCR | MX RCR | RCR% | FP8 RRR | MX RRR | RRR% | FP8 CRR | MX CRR | CRR% |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 8192³ | 3155 | 2978 | 94.4% | 3157 | 2877 | 91.1% | 2950 | 2776 | 94.1% |
| 8B Q/O (4096³) | 2449 | 2335 | **95.3%** | 2522 | 2300 | 91.2% | 2420 | 2170 | 89.7% |
| 8B Gate/Up | 2725 | 2557 | 93.9% | 2796 | 2529 | 90.4% | 2588 | 2343 | 90.5% |
| 8B Down | 3067 | 2933 | **95.6%** | 2770 | 2911 | **105.1%** | 2878 | 2724 | 94.6% |
| 70B Q/O | 3045 | 2891 | 94.9% | 3075 | 2872 | 93.4% | 2867 | 2682 | 93.6% |
| 70B Gate/Up | 2965 | 2834 | **95.6%** | 2946 | 2761 | 93.7% | 2793 | 2472 | 88.5% |
| 70B Down | 3223 | 2931 | 90.9% | 2720 | 2944 | **108.2%** | 2985 | 2632 | 88.2% |

**PASS 5/21 stable** (peak 7/21 in mid-cycle run): 8B Q/O RCR (95.3%), 8B Down RCR (95.6%), 8B Down RRR (105.1%), 70B Gate/Up RCR (95.6%★ +5.4pp via Dev A), 70B Down RRR (108.2%). Borderline (±0.5pp): 70B Q/O RCR (94.9%), 8B Down CRR (94.6%). All 21 cells SNR=49.6 dB + det 3/3 PASS.

**NEW PASSES vs R46 baseline**: 8B Q/O RCR (was 91.8%), 8B Down RCR (was 94.4%), 70B Gate/Up RCR (was 90.5%, **+5.4pp lift**).

### R47 paradigm corrections (3 closed + 1 refuted + 1 stale-conditional removed → cumulative 49 closed + 5 refuted; R32-R46=46+4 + R47:3 closed/+1 refuted)

### R47 Cherry-pick status (4/4 on `feat/mxfp8-only`)
- `6edb05f9` Dev A — RCR XCD swizzle SHIP DEFAULT ON
- `c1319949` Dev D — ISA HARDWARE-CEILING verification
- `0b2f19ae` Dev B — CRR XCD swizzle SHIP DEFAULT ON
- `9315a0bf` Dev C — Cachepolicy sweep + R28 stale-conditional removed

### R48+ priority list (constrained by hardware ceiling)
1. **8192³ RRR slip 91.1%** — was 97.0% in R46. RRR XCD swizzle from R46 Dev D may be slightly suboptimal for cube shapes. Investigate per-shape gating of MXFP8_RRR_BLOCK_SWIZZLE.
2. **70B Down RCR/CRR at 90.9%/88.2%** — K=28672 dominated by VMEM contention per Dev A/D analysis. Try cooperative B-scale loading across CTAs (Dev D R48 lever recommendation).
3. **8B Q/O RRR + 8B Q/O CRR at 91.2%/89.7%** — Dev D ISA finding suggests 4096³ at hardware ceiling may be ~95%. Worth verifying via assembly compare on 4096³ specifically.
4. **8B/70B Gate/Up CRR at 90.5%/88.5%** — N=14336/28672 plus CRR transpose cost. CRR fastpath structural floor — needs different angle (e.g. LDS swizzle for A^T fetch).
5. **STOP RCR optimization on K=8192 shapes** — Dev D proved hardware ceiling. Don't waste cycles.
...
R44 派 4 dev (A M=2..16 MFMA fastpath, B persistent-CU N=1024, C 8B-KV B1 drift bisect, D 3-gate retry audit + margin survey) + Reviewer (14th-cycle baseline + 2 R43 SHIP RECONFIRM + 4 gold-standard re-bench + 4 methodology). **★ 1 SHIP-LITE-PARTIAL (Dev A 4/6 M=2..16 cells, full SHIP gated on R45+ triangulation) + 2 paradigm refutations (Dev B 44th lever + Dev C R43D drift hypothesis OVERTURNED via bisect) + 1 methodology hardening pass (Dev D 3 retroactive harness patches + 2 floor-limit verdicts) + 0 NEW R44 methodology rules pending (potential rule from Dev C: within-GPU variance must be measured before treating cycle-drift as real).**

### R44 Dev A — M=2..16 MFMA fastpath (`7b08fa19`, single-GPU smoke only) ★ SHIP-LITE-PARTIAL
- New kernel `gemv_m2_16_decode_kernel<L, M_FIXED, PRESHUFFLED_QUANT>` with `M_FIXED ∈ {2,4,8,16}`, BLK_N=64 single-wavefront, K-tiled LDS streaming, adaptive inner unroll, restricted predicate.
- Bench (single-GPU GPU2 smoke, RCR, V1 PQ scales, 30 warmup × 100 iters, M=4×4096² 101% / M=4×8192² 96% / M=8×8192² 79% / M=16×8192² 96% / M=8×4096² 68% EXCLUDED / M=16×4096² 53% EXCLUDED).
- Predicate covers 4/6 cells; excluded cells fall through to V1 unchanged (production-identity preserved).
- nm-gate: default 8192³ build 0 R44A symbols (byte-identity); enabled build 12 symbols (3 layouts × 4 M_FIXED templates), zero VGPR spills.
- Macro-gated `MXFP8_DECODE_M2_16_ENABLE` (default OFF). Dispatcher waterfall extended via `MXFP8_DISPATCH_TRACE_ONCE` from R39C.
- Blockers for full SHIP: GPU2/3/6/7 triangulation pending with R36 3-gate retry harness; RRR/CRR layouts coded but unbenched; M=8 8kx8k below 95% gate but kept (1.03× over V1, practical 70B speculative-decode case); excluded N=4096 cells suffer N/64=64 WGs ≪ 304 CUs (split-K or wider BLK_N R45+ scope).

### R44 Dev B — persistent-CU dispatch for KV-decode N=1024 (`1665846c`, 0 GPU) ★ REFUTED-BY-INSPECTION (44th lever)
- Brief premise WRONG: "M=32 N=1024 → 16 tiles vs 304 CUs = 5.3% occupancy" used BLK_M=256/BLK_N=64 V2 fastpath geometry. V2 unreachable for M<BLK=256 per R41 Dev C (`rcr_can_use_exact_8wave_scaled` requires `g.m == M_DIM`).
- Actual production path: R43C unified small-M waterfall → R42B `gemm_tail_kernel_smallm_b32` at TAIL_BLOCK_M=16, TAIL_BLOCK_N=16 gives 128 tiles (M=32, N=1024) and 512 tiles (M=128, N=1024) — NOT 16. M=128 already saturated; M=32 has 176 idle CUs but **R31 Dev D paradigm correction** forecloses all persistent-CU variants (early-exit-prologue identical to baseline; inner-loop persistence is NOT a wave-fill optimization since SPI dispatcher already does this; persistent-with-shared-LDS-reuse would require restructuring smallm_b32 from scalar 0-LDS into wavefront-LDS-prefetched kernel — overlaps with R44+ priority #2 M=2..16 MFMA, 3-5 day rewrite).
- 0 code change. Replacement recommendation: B-side scalar-load vectorization in `gemm_tail_kernel_smallm_b32` (priority #4, the highest-leverage lever for KV-decode N=1024 since the scale-load redundancy gap is closed by R42B SHIP).

### R44 Dev C — 8B-KV HB shrink B1 drift bisect (`22675246`, ~30 GPU-min) ★ REFUTED-AS-MEASUREMENT-NOISE
- R43D drift hypothesis (~0.95pp/cycle falling) **OVERTURNED**. Root cause = silicon-bin / GPU-rotation / run-to-run noise.
- Bisect (GPU4, single-session, on-tree SHAs R38→R43): R38 wrap-fix +24.737% / R39 trace +25.386% / R42B smallm32 +25.025% / R42A decode_m1 +25.210% / R42 HEAD **+26.077% (BEST)** / R43 HEAD +25.490%. Envelope 1.34pp on GPU4. NO monotonic decrease. R42 HEAD is BEST — directly refutes "cumulative dispatcher overhead from R42 A/B macro guards" hypothesis.
- Triangulation (GPU5, identical .so md5): R38 wrap-fix 3 reps: +27.187% / +25.783% / +23.802% (3.39pp single-GPU range); R42 HEAD 2 reps: +23.265% / +23.662%.
- **Single-GPU run-to-run noise (3.39pp) > cited cycle-drift (2.86pp)** — drift is noise. Cross-GPU spread on R42 HEAD .so (GPU4 +26.08% vs GPU5 +23.46%) = 2.62pp directly mirrors R42 reviewer GPU7 +24.29% vs R43 reviewer GPU3 +26.96% on identical .so files.
- Recommendation: DO NOT modify dispatcher (per R43D explicit instruction; bisect data confirms macro-guards cause no measurable regression). Promote R44 spot-bench gate but enforce R43 NEW Rules 2+3 before treating any single-cycle drop as drift.

### R44 Dev D — 3-gate retry harness audit + margin tightening survey (`93f0248f`, 0 GPU)
**Part 1 — R36 3-gate retry harness migration audit** (R43 NEW rule 1 enforcement): 6 `r37_paired_bench_2so.py` invocation sites, all 6 non-compliant in source tree.
- PATCH-NOW (3): r40_reviewer_phase2_pair.sh, r42_reviewer_phase23.sh (retroactively patches the R42 P3.3 false-positive source), r43_reviewer_phase23.sh (base for R44+ reviewer phases).
- DEPRECATED-marked (3): r37_reviewer_ship_verify.sh, r38_reviewer_ship_verify.sh, r39_reviewer_ship_verify.sh.
- 5/5 active gold-standard orchestrate scripts (r38c, r37d, r39_8bdown, r39b, r40b, r40d) verified compliant — no patches needed.
- Patches add `run_bench_attempt` shell helper that parses `[sclk-post-preheat]`, `[sclk-post-bench]`, median/stdev for both bench arms; outer `while attempt <= MAX_RETRIES` loop with 5s sleep on fail; copies clean attempt output to both `_clean.txt` and legacy `_bench.txt`; exits 2 on retry exhaustion. All 6 patched files PASS `bash -n`.
- **NOTABLE FINDING**: r39_reviewer_findings.md:177 claims "R36 NEW 3-gate" but source has no retry loop and no gate parsing — silent doc-drift documented in patch comment.
- R44+ reviewer-template lock recommendation: any new reviewer phase script should start as a copy of patched `r43_reviewer_phase23.sh` to inherit `run_bench_attempt` rule-1 compliance.

**Part 2 — Margin tightening survey** (R44+ priority #5):
- 70B-KV HB shrink B1 (R43 +28.49%, target ≥+28%, margin +0.49pp): **FLOOR-LIMIT NO-CHANGE**. Target = R36A SHIP floor; envelope 28.02-30.39% across 10 measurements; R43D slope ≈ −0.05pp/cycle (flat); kernel at PIPE=1 VGPR/HBM ceiling per R36 Dev A pivot analysis (already 107.4% of FP8 per-tensor); PIPE=2/3 degenerate per `crr_mxfp8_exact_8wave_hbshrink_fastpath.inc:39-47`.
- 8B Gate/Up V2-RRR (R43 +5.28%, target ≥+5.0%, margin +0.28pp): **FLOOR-LIMIT NO-CHANGE**. Target literally encoded in dispatcher comment `kernel_mxfp8_layouts.cpp:5978` as `"R34B +5.025% min"`. R40 BOUNDARY-LOCK rule: silicon-bin perf cap. Default V2-RRR kernel at WARPS_N=4 RBN=32 already wide-N optimal; HB-N shrink REFUTED V2-CRR (R38A: −43%) and V2-RRR (R39A inspection); WARPS_N=8 REFUTED (R35C).
- Optional R45+ kernel-design scope: V2-RRR Gate/Up BLK_N=256 exploration (RBN=64 OR analogous) bounded ~0.5-1.5pp expected lift; HB-shrink B1 PIPE=4 ("B3v2", placeholder reserved at `.inc:75`) sub-tile interleave only if a falling trend surfaces.

### R44 Reviewer (`8528738b` → `01fd6a79`, ~40 GPU-min)
- **Phase 1 — 14th-cycle 70B-KV V2-CRR baseline**: PASS. Median-of-4 = **787.76 TF** (GPU2/3/6/7), drift +2.86% from R43 (within ±3.5% gate). 4-GPU envelope 3.62% (R43 was 3.85%, slight tightening +0.23pp). 3/4 GPUs needed retry for G1 sclk-post-preheat on attempt 1 — retry caught all (validates NEW rule 1).
- **Phase 2a — R43 Dev B M=1 RRR/CRR fastpath SHIP STRICT RECONFIRM**: 8/8 cells PASS ≥95% MXFP8/FP8 (min 899.8%, max 1209.6%); median FAST/V1-PQ-FALLBACK speedup 11-17× (matches R43); cross-GPU spread on FAST TF max 5.2%.
- **Phase 2b — R43 Dev C unified waterfall STRICT RECONFIRM**: HEAD default 8192³ build byte-identical (md5 d1f50d70c088bb2554ea48cf1af036f2); nm-gate 9/9 default-off features count=0 + 3/3 V2 dispatchers count=1 PASS; 4/4 trace strings verified across separately-built artifacts (SMALLM-DECODE-M1-RCR R42A, -RRR R43B, -CRR R43B, SMALLM-B32-TAIL R42B).
- **Phase 3 — 4 gold-standard re-bench (all use R36 3-gate retry, abs TFLOPS reported per NEW rule 2)**:
  - 70B-KV HB B1: Δ%=+29.25%, t=+47.6, abs **1022.20 TF** (R43 +28.16%, **WIDENED +1.09pp**)
  - 8B-KV HB B1: Δ%=+25.20%, t=+18.4, abs **883.42 TF** (R43 +26.96%, **NARROWED -1.76pp** ← AT-RISK consistent with R43D survey BUT within R44C bisect noise envelope of 3.39pp single-GPU variance)
  - 8B-Down V2-RRR: Δ%=+8.80%, t=+11.0, abs **2961.42 TF** (R43 +8.36%, +0.44pp)
  - 8B Gate/Up V2-RRR: Δ%=+5.37%, t=+9.4, abs **2523.41 TF** (R43 +5.00%, +0.37pp ← still TIGHT margin, gate ≥+5%)
- **Phase 4 — 4 methodology checks (all 4 R43 NEW rules VALIDATED)**:
  - Rule 1 (R36 3-gate retry MANDATORY) → VALIDATED via own use (3/4 Phase 1 + 1/4 Phase 3 caught by retry)
  - Rule 2 (absolute TFLOPS alongside Δ%) → VALIDATED (all Phase 3 cells include abs TF)
  - Rule 3 (Δ%-reproducibility cross-GPU MANDATORY) → VALIDATED on Phase 2 8 cells; Phase 1 gives 3.62% baseline cross-GPU spread
  - Rule 4 (nm-gate must extend per macro) → VALIDATED (9/9 default-off features tracked, +R43 Dev B/C symbols)
- **Phase 5 — PY_MODULE_NAME defensive assert**: PASS (synthetic .so collision → ImportError fired)
- **20/20 PASS overall**. 2 escalations: (1) 8B Gate/Up V2-RRR margin TIGHT +0.37pp (consistent with R44D FLOOR-LIMIT verdict — no kernel work warranted); (2) 8B-KV B1 trajectory drift -1.76pp (R44C bisect REFUTES this as kernel/dispatcher regression; within noise envelope but warrants NEW rule 1 application in R45).

### R44 NEW methodology rules (3, all VALIDATED via Reviewer Phase 4 + Dev work)
1. **Within-GPU run-to-run variance must be measured before treating cycle-drift as real** (Dev C explicit recommendation): when investigating a "trend", first measure within-single-GPU variance with N≥3 reps of SAME .so. If single-GPU variance ≥ cycle-cited drift, the drift is noise. (R44 8B-KV B1 example: 3.39pp single-GPU variance > 2.86pp cycle-drift = noise verdict.)
2. **Reviewer-template lock** (Dev D recommendation): any new reviewer phase script should start as a copy of latest patched `phase23.sh` to inherit `run_bench_attempt` 3-gate retry pattern.
3. **Dispatcher-overhead refutation methodology** (Dev C): when accusing a dispatcher branch of macro-guard regression, must show monotonic decrease across R(N-2)→R(N) on at least 2 independent GPUs, not just 1 cycle drop.

### R44 paradigm corrections (1 → cumulative 44 closed levers; R32:21 + R33:5 + R34:4 + R35:1 + R36:2 + R37:2 + R38:2 + R39:1 + R40:1 + R41:3 + R42:1 + R43:0 + R44:1)
...
- `1665846c` R44 Dev B — persistent-CU dispatch REFUTED-BY-INSPECTION (doc only)
- `22675246` R44 Dev C — 8B-KV B1 drift bisect REFUTED-AS-MEASUREMENT-NOISE (docs + orchestrator + run logs)
- `93f0248f` R44 Dev D — 3-gate retry harness audit + margin survey (audit doc + survey doc + 3 patched harness scripts + 3 deprecated)
- `01fd6a79` R44 Reviewer — 14th-cycle baseline 787.76 TF + 2 R43 SHIP RECONFIRM + 4 gold-standard re-bench (8B-KV B1 NARROWED -1.76pp WITHIN noise; 8B Gate/Up TIGHT +0.37pp) + 4 R43 NEW rules VALIDATED + PY_MODULE_NAME assert
...
### ★★★ Headline result — Dev A M=1 fastpath SHIP-LITE (first decode SHIP in cycle history)

- 8B 1-tok (1×4096×4096): **0.500 TF** (median 4-GPU) vs V1 baseline 0.0645 TF vs FP8 0.0939 TF → **7.76× speedup, MXFP8/FP8 = 532%**
- 70B 1-tok (1×8192×8192): **0.936 TF** vs V1 0.107 TF vs FP8 0.186 TF → **8.74× speedup, MXFP8/FP8 = 504%**
- Geometry: BLK_M=1 BLK_N=64 single-wavefront RCR K-streaming GEMV with cooperative LDS prefetch of A row + A_scale
- Output **bit-identical** to V1 baseline (max|C_dec - C_base| = 0.0); det 3/3 PASS; SNR 47.89/47.75 dB matches FP8 reference floor (47.73/47.94 dB) — structural M=1 numerical floor not noise
- Default 8192³ MXFP8 build: **0 gemv_m1 symbols** (kernel + dispatcher branch dead-code-eliminated when MXFP8_DECODE_M1_ENABLE undefined). Dispatch trace: `RCR-DECODE-M1-FASTPATH (R42A)` fires correctly
- Limitations: RCR layout only (RRR/CRR deferred); M=1 only (M=2..16 not generalizable from this geometry); bandwidth efficiency ~9% of HBM peak — vectorization headroom for R43+
- New `mxfp8_decode_m1_fastpath.inc` (181 lines), bench scaffold `r42a_*`, build script

### ★★★ Headline result — Dev B M=32/128 STRICT SHIP (commit `0c96d60a`, prior-session)

Mechanism: existing `gemm_tail_kernel` calls `load_scale_scalar_preshuffled(g.{a,b}_scale, ..., kk/32)` every K-iter. At K=4096 that's 8192 redundant scalar global loads per (row, col) thread when scale only changes every 32 K-iters. New `gemm_tail_kernel_smallm_b32` restructures K-loop as nested (k_block × within_block_32), naturally hoisting scale loads.

Results (4-GPU triangulation GPU2/3/6/7 lock, N_PAIRS=10, PREHEAT=60s):
- 32×4096×4096 8B b=32: **98.76-99.49% MXFP8/FP8** (was 77.3-79.2%)
- 128×4096×4096 8B b=128: **98.98% MXFP8/FP8** (was 77.3%)
- 32×8192×8192 70B b=32: **98.21% MXFP8/FP8** (was 74.5%)
- 128×8192×8192 70B b=128: **99.00-99.14% MXFP8/FP8** (was 77.2%)

ALL 4 production decode shapes clear 95% rule. Min Δ% over V1-LEGACY-FALLBACK = +24.73% (1 of 8 just under +25); 7 of 8 cells ≥ +25%. Min Welch t = +9.79 (typically +400 to +800). SNR = ∞ dB (bit-identical output — pure K-loop hoist). Default 8192³ build byte-identical (MXFP8_SMALLM_B32_FASTPATH=0 → 0 smallm symbols; V2-CRR perf sanity 835.10 TF within R31-R41 envelope).

Why not full MFMA fastpath (R41 Dev C's BLK_M=32+BLK_N=128+K-pipelined proposal): 3-5 day effort, ~1500-3000 LoC. R42 Dev B 3-4 hour time-box pursued the **minimum-invasive lever that hits the ROOT CAUSE** of the 23% gap — future MFMA fastpath can incorporate same amortization principle. New `mxfp8_smallm_b32_fastpath.inc` + bench scaffold `r42b_*`.

### R42 Dev C closure (43rd cumulative lever, no GPU burned, ~3-5 GPU-hr saved)

`buffer_load_dword_lds` flagged by R29D rec #4 + R41D §2.4 as "the only B-side lever not yet closed" is **the production state**. All 4 overloads of `kittens::load` in `include/ops/warp/memory/tile/global_to_shared.cuh:17-318` (which V2-CRR/RRR/RCR reach via `G::load` at `kernel_mxfp8_layouts.cpp:432`) emit `llvm.amdgcn.raw.buffer.load.lds` directly. No `buffer_load_b128 → ds_write_b128` chain anywhere in V2 dispatch.

Empirical confirmation (two pre-existing SASS audits in tree): `r30c_sass_inventory.log` ds_write*=0, buffer_load_dwordx4...lds=24; `r31c_sass_audit_summary.log` ds_write/ds_store=0, buffer_load_*x4...lds=96. Toolchain: ROCm 7.1.0 / clang 20 / gfx950, intrinsic supports 1/3/4 dwords/lane on gfx950 vs gfx942's 1-dword limit.

**Genealogy**: R29D recommended → R30 implemented → R30C SASS confirmed → R31C SASS re-confirmed → R41D forwarded stale recommendation without re-checking audits → R42C closes.

**NEW R42+ methodology rule (mandatory)**: any "lever from a prior recommendation list" must be re-audited against the most-recent SASS inventory before scoping/prototyping.

### R42 Dev D infrastructure (`28293a4f` → `0157bc62`, 0 code change, 0 GPU)

- **Phase 1 small-M dispatcher waterfall design** (`r42d_smallm_dispatch_design.md`): top-of-`dispatch_pq_v2<L>` waterfall — when `g.m < BLK`, try `MXFP8_SMALLM_BLK_M_1` (Dev A) → `_32` / `_16` (Dev B) → fall to V1-LEGACY-FALLBACK. All branches macro-guarded (mirrors R35B / R37A pattern). Trace strings emit `SMALLM-V1-FASTPATH BLK_M=N (R42A/B)` via R39 `MXFP8_DISPATCH_TRACE_ONCE`. Patch sketch: insertion points RCR ~5683, RRR ~5689, CRR ~5701 + predicate/dispatch-helper contracts. Critical property: `g.m ≥ BLK` traffic byte-identical to today
- **Phase 2 shape coverage survey** (`r42d_shape_coverage_survey.md`): 8 LLaMA prefill cells from `llama_baseline_r25.json` cross-referenced — **8/8 routed via V2 wire-ins or R36 advisories**; 0 NEW prefill gaps. Smallest LLaMA N=1024 (KV-attn) optimized via R37 HB shrink B1. No N<256 wide-N tail in production. No K<128 K-tail
- **Phase 3 LDS-direct audit** (`r42d_lds_direct_load_audit.md`): `llvm_amdgcn_raw_buffer_load_lds` broadly used in tree (7 sites across 5 files). MXFP8 V2 A/B tile fills already use this intrinsic via `G::load`. **Independent confirmation** of Dev C's REFUTED-IN-TREE finding via different code-path

### R42 Reviewer Phase 1 — 12th-cycle baseline (`a8347a8c` → `be4cf007`)

12-cycle 70B-KV V2-CRR median (4-GPU lock GPU2/3/6/7): **771.87 TF** (GPU2 763.84 / GPU3 769.81 / GPU6 773.92 / GPU7 792.97). Drift vs R41 (778.57): **0.86%**, well under 3.5% gate.

12-cycle history: 766.72/772.51/766.17/767.59/768.07/775.15/786.64/777.62/768.68/789.48/778.57/**771.87**. Envelope widens to **3.81%** (GPU2 extends low end -0.30%, GPU7 extends high end +0.44%). NOT breached but flagged. GPU2 + GPU7 each took 1 G1' fallback retry.

### R42 Reviewer Phase 2 — 2/2 STRICT RECONFIRM (R40 V2-RCR QO predicates)

- **8B QO V2-RCR**: Δ%+7.49 (N=10) and Δ%+7.22 (N=20 retry on GPU6 after GPU2 memory fault); Welch t 8.07 / 6.10. **Δ% PASS** (target ≥+5%); **Welch t MARGINAL** (<10) due to RCR-side tail variance. Δ%-reproducibility within ±0.3pp is stronger evidence — recorded as PASS with note. **NEW R42+ rule (recommended)**: Δ%-reproducibility may be primary STRICT gate when Welch t shows structural noise
- **70B QO V2-RCR**: Δ%+9.52, Welch t 18.15 — STRICT RECONFIRM PASS

### R42 Reviewer Phase 3 — 3/4 gold-standard re-bench PASS + 1 ★ CRITICAL FAIL

- ✅ **R37A 70B-KV HB shrink B1**: Δ%+28.17, t=219.8 — PASS by 0.17pp (margin TIGHT)
- ✅ **R38 wrap fix 8B-KV HB shrink B1**: Δ%+24.29, t=74.7 — PASS by 0.29pp (margin TIGHT)
- ★ **R38C 8B-Down V2-RRR**: Δ%+2.32, t=5.46 — **CRITICAL FAIL** vs R36B SHIP target ≥+6.5%. Predicate `ADVISE-V2-RRR-8B-DOWN` fires, kernel runs, but delivers ~1/3 of original SHIP claim. Cause unclear. **★★★ ESCALATE TO R43**
- ✅ **R39B 8B Gate/Up V2-RRR**: Δ%+5.17, t=15.77 — PASS by 0.15pp (margin TIGHT)

### R42 Reviewer Phase 4 — 3/3 methodology PASS

- PY_MODULE_NAME defensive assert in `r37_paired_bench_2so.py:50-55` — PASS
- MXFP8_DISPATCH_TRACE emits expected predicate names in 6/6 cells — PASS
- `r38_nm_gate.sh` on default 8192³: 7 default-off features count=0, 3 V2 dispatchers count=1, OVERALL PASS

### R42 NEW methodology rules (4)

1. **Stale-recommendation re-audit (mandatory)**: any "lever from a prior recommendation list" MUST be re-audited against the most-recent SASS inventory before scoping/prototyping (R42 Dev C: R29D rec #4 stale-but-forwarded across 12 cycles)
2. **Δ%-reproducibility as primary STRICT gate (recommended)**: when Welch t shows structural RCR-side tail noise (e.g., 8B QO V2-RCR), Δ%-reproducibility within ±0.3pp across 4 GPUs is stronger evidence than Welch t (R42 Reviewer Phase 2.1)
3. **Decode geometry context distinct (validated)**: closure rules for M=4096 prefill geometry (HB-*, tile-area-conservation, B-operand alt-layout) DO NOT directly transfer to small-M decode geometry (R42 Devs A/B both shipped despite closures)
4. **SNR gate revision (recommended)**: when kernel is mathematically equivalent to baseline (parity verified by max|delta|=0), accept SNR ≥ FP8-reference SNR floor instead of conventional 48 dB threshold (R42 Dev A finding — M=1 numerical floor)
...
- **`buffer_load_dword_lds` REFUTED-IN-TREE (R42 NEW)**: Already production via `kittens::load`. R29D→R30→R31C→R41D stale-recommendation chain closes. **Genealogy lesson encoded as new methodology rule (#1 above)**

### R42 cumulative tally → 43 closed levers

### ★★★ R42 STRATEGIC ACHIEVEMENT — R41 Dev C MAJOR FINDING addressed end-to-end

R41 Dev C survey: 0/6 decode shapes PASS 95% rule. R42 outcome:
- **M=1 (2 shapes)**: SHIP-LITE Dev A → MXFP8/FP8 532%/504% (BEATS FP8 by 5×)
- **M=32/128 (4 shapes)**: STRICT SHIP Dev B → MXFP8/FP8 98-99%

**6/6 decode shapes now ≥ 95% MXFP8/FP8 ratio.** Project goal "MXFP8 V2 ≥ FP8 per-tensor × 95%" achieved on production decode workload. Combined with R37/R38/R39/R40 prefill SHIPs, project goal now achieved across BOTH prefill AND decode geometries.

### R42 ★ R43 ESCALATION — 8B-Down V2-RRR regression

R36B shipped 8B-Down V2-RRR predicate at +6.5-9.51% Δ%. R42 Reviewer reconfirms predicate fires (`ADVISE-V2-RRR-8B-DOWN` traces correctly), kernel runs, but delivers only **+2.32% Δ%** — regression of ~4-7pp vs SHIP claim. Cross-cycle history before R42 was monotonic increasing +6.66 → +8.47. Sudden drop to +2.32 is anomalous.

**Hypotheses to test (R43)**:
- Driver/firmware regression (check ROCm + GPU firmware versions vs R38C's environment)
- Missing compile flag (verify production .so build matches R38C exactly via nm-gate + size diff)
- RRR pathway specialization gap (check MXFP8_DISPATCH_TRACE rigorously)
- Cherry-pick contamination (some R39-R42 commit may have inadvertently weakened V2-RRR codegen — git bisect)

### R43+ priority list (rebuilt from R42 results)

1. **【★★★ critical】Investigate 8B-Down V2-RRR regression** (R42 Reviewer Phase 3.3 ★ CRITICAL FAIL): production +2.32% vs SHIP +6.5-9.51%. Hypotheses above. May need git bisect across R36-R42
2. **【★★ high】Generalize Dev A M=1 fastpath to M=2..16 + RRR/CRR layouts**: current Dev A is RCR-only + M=1 only. Vectorization headroom (uint4 packed loads, B coalescing via warp shuffle). Add MFMA-based fastpath for M=2..16 leveraging Dev B's K-loop hoist principle
3. **【★ medium】Integrate Dev D dispatcher waterfall** with Dev A + Dev B kernels for unified small-M routing — currently A and B are independent macro-gates
4. **【medium】Survey R37 / R38 wrap fix gold-standard margin tightening**: Phase 3.1 (+28.17 vs target +28) and Phase 3.2 (+24.29 vs target +24) have shrinking margins. Investigate if cumulative source drift is eroding these
5. **【methodology — R43+ rules, MUST follow】**:
   - All R29-R41 + R42 NEW rules carry forward
   - R42 NEW (mandatory): stale-recommendation re-audit
   - R42 NEW (recommended): Δ%-reproducibility as primary STRICT gate when Welch t shows tail noise
   - R42 NEW (validated): decode geometry context is distinct from prefill
   - R42 NEW (recommended): SNR gate revision when kernel is mathematically equivalent (max|delta|=0 parity)
6. **【closed】**: 43 levers per cumulative tally. Decode shapes now COVERED (was 0/6 → 6/6). HB-* CLOSED. Tile-area-conservation 4-confirm. B-operand alt-layout 3-sub-class. `buffer_load_dword_lds` REFUTED-IN-TREE. 8B Up V2-RRR BOUNDARY-LOCK

### R42 Cherry-pick status

Cherry-picked to feat/mxfp8-only (in causal order):
- `0c96d60a` (R42 Dev B — small-M MXFP8 fastpath M=32/128 STRICT SHIP; landed prior session before R42 dispatch this session)
- `0157bc62` (R42 Dev D — dispatcher design + shape coverage + LDS-direct audit; docs only)
- `a5ff3819` (R42 Dev C — buffer_load_dword_lds REFUTED-IN-TREE; 1 findings doc)
- `501e19c6` (R42 Dev A — small-M MXFP8 fastpath M=1 SHIP-LITE; new .inc + dispatcher edit + bench scaffold)
- `be4cf007` (R42 Reviewer — 12th-cycle baseline + 2 STRICT RECONFIRM + 4 gold-standard re-bench + 3 methodology checks)

No conflicts in cherry-pick (Dev A's dispatcher edit at line ~5464 vs Dev B's at line ~5545 auto-merged cleanly). Default 8192³ MXFP8 build remains byte-identical (MXFP8_DECODE_M1_ENABLE off + MXFP8_SMALLM_B32_FASTPATH=0 → 0 smallm symbols; nm-gate PASS).
