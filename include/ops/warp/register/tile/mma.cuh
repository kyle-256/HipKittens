...
// AGPR-pinned 32x32x64 — for dense gemm_kernel<CRR>. Same `+a` constraint
// as 16x16x128 variant; D operand is 16 dwords (vs 4) so AGPR pressure is
// 4× higher per call. Numerical correctness verified via builtin parity.
__device__ static inline void mfma323264_agpr(
        float2 (&D)[8],
        const fp8e4m3_4 (&A)[8],
        const fp8e4m3_4 (&B)[8],
        const float2 (&C)[8]) {
...
// AGPR-pinned variant — read-modify-write `+a` on shared D/C AGPR.
// Numerically verified via 4-mma chain test (0/256 mismatches).
__device__ static inline void mfma1616128_agpr(      float2 (&D)[2],
                                               const fp8e4m3_4 (&A)[8],
                                               const fp8e4m3_4 (&B)[8],
                                               const float2 (&C)[2]) {
    typedef __attribute__((__vector_size__(8 * sizeof(int)))) int intx8_t;
...
// Direct in-place AGPR variant — caller guarantees D and C alias same
// memory. Eliminates the cd-copy at function boundary so the compiler
// can keep D in AGPR across back-to-back mma calls without per-call
// v_accvgpr_write/read overhead. Verified numerically equivalent.
__device__ static inline void mfma1616128_agpr_inplace(
        float2 (&D)[2],
        const fp8e4m3_4 (&A)[8],
        const fp8e4m3_4 (&B)[8]) {
    typedef __attribute__((__vector_size__(8 * sizeof(int)))) int intx8_t;
