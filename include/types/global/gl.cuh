...
struct gl {
    ...
    __device__ inline T& operator[](const coord<ducks::default_type> &idx) const { // yes I am abusing the const qualifier here a bit.
        // R23-A: cast to size_t to avoid int32 overflow when total tensor > 2 GiB
        // (e.g., production-shape FP8 attention P[B=16,H=64,N=4096,N=4096] = 16 GiB).
    ...
    __device__ inline size_t idx(const coord<ducks::default_type> &idx) const {
        return ((size_t(idx.b)*depth() + idx.d)*rows() + idx.r)*cols() + idx.c;
    }
