...
- **MXFP8 8-wave KPAIR+SRD+SCALE_PIPE**: 2897.66 TFLOPS (SNR 49.60 dB)
- **Gap**: +173.27 TFLOPS / +5.64%
...
### Current best MXFP8 (8-wave, KPAIR_LOOP + SGPR SRD + scale pipeline)
```bash
...
- **Dev C** — Python `.s` rewriter. Target: redistribute 8-wave loop's 6 `v_lshr` + 6 scale `buffer_load` into MFMA shadows; current `rewrite_mxfp8.py` only moves AGPRs and is therefore a no-op on the 8-wave PQ kernel (it has 0 AGPRs).
...
## Current Kernel Resource Picture (8-wave PQ=1, from compile remarks)
- VGPR: **256 (hard limit)** — skill-level: you cannot grow VGPR-resident state any further
- AGPR: 0 — accumulators already in VGPRs
- Spills: 0, Scratch: 0
- LDS: 135168 bytes (double-buffer A+B tiles)
- Occupancy: 2 waves/SIMD

## Inner-Loop ASM Diff (8-wave, per kpair)
| metric | FP8 | MXFP8 | Δ |
| --- | ---: | ---: | ---: |
| total lines | 403 | 431 | +28 |
| MFMAs | 64 | 64 | 0 |
| `v_mfma_scale` | 0 | 64 | +64 (all MFMAs are scaled) |
| `buffer_load` | 16 | 22 | **+6 (scale loads)** |
| `ds_read` | 48 | 48 | 0 |
| `s_waitcnt` | 10 | 12 | +2 |
| `s_barrier` | 16 | 16 | 0 |
| `v_lshrrev_b32` | 0 | 6 | **+6 (scale remap, on critical path)** |
| `sched_barrier` | 4 | 4 | 0 |

The +6 `v_lshr` instructions extract the "hi half" of 32-bit scale packs; their outputs feed the following MFMAs immediately (RAW), so they cannot be moved past the MFMAs they serve. They CAN potentially be hoisted earlier into the previous phase's MFMA shadow — this is the core optimization target.
...
- `phase1 op_sel_hi` inline-ASM variants: semantics correct but long-run perf regressed.
