#!/usr/bin/env python3
"""R56 Opt G-1 / L1 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 14336).

R56 G-1: very-high-confidence 64x1024 -> 256x256 swap on the 64x1024 LOSE cluster.
L1 current: R53D3B_1_AITER (tile=64x1024, pct_comp=65.68%).
...
Per-shape grid: gdx=ceil(N/256)=128, gdy=ceil(M/256)=16, gdz=1, bdx=256
"""
import argparse
...
def run_one_seed(seed: int, gpu_id, do_perf: bool, timeout: int = 1200):
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G1_L1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-1 L1 bench - mode={args.mode}, gpu={gpu_list}, "
