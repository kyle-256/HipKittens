#!/usr/bin/env python3
"""R14 OptA 5-run verify on the most plausible single-shot candidates.
None met the +0.5pp gate; the top-of-noise picks were:
  - P1 regclassglob: +0.38pp on 28672x4096x16384
  - DLA1 dppc: +0.20pp on 4096x32768x128256
  - DLA7 nomisched: +0.12pp on 28672x32768x4096
  - DLA2 regclassglob: +0.03pp on 128256x32768x4096 (control)

Round 6 methodology: candidate mean must >= baseline.MAX to count as WIN.
GPU 1, warmup=200, iters=500, trim=10%, 5 runs each.
"""
...
TARGETS = [
    ("P1",   28672,  4096, 16384, 5273.0, "_ts_gm8",              "regclassglob"),
    ("DLA1", 4096,  32768, 128256, 5781.1, "_ts_pf6_6_v12_memc",  "dppc"),
    ("DLA7", 28672, 32768,  4096, 4466.6, "_ts_lgk2_v12_memc",    "nomisched"),
    ("DLA2", 128256, 32768,  4096, 4536.4, "_ts_gm2_v12_memc_dc", "regclassglob"),
]
...
def main():
    ...
    with open(os.path.join(SCRIPT_DIR, "bench_round14_optA_verify.json"), "w") as f:
        json.dump(out, f, indent=2)
    print("\nSaved bench_round14_optA_verify.json")
