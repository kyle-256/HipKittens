#!/usr/bin/env python3
"""R54 Opt D-4A/1 — bench harness for the aiter `.co` dlopen shim on (4096, 14336, 16384).

Reuses R50D shim AS-IS (bdx=256, kd max_flat_workgroup_size=256).
Tile = 256x256 (aiter heuristic pick). Sub-90% HK-VC rescue candidate.
Sister of R51D1 (14336,4096,32768) which scored 103.17% — expect strong PROMOTE.
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 16384, 6144, 2048
COMPETITOR_TFLOPS = 3236.1  # R54 consensus HK tflops_p50 for (16384,6144,2048); pct_comp = AITER / HK
...
def main():
    ...
    ap.add_argument("--tag", default="R55_OPT_D5A_2")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R54 Opt D-4A bench — mode={args.mode}, gpu={gpu_list}, "
