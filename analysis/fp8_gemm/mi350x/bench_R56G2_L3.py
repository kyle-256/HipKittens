#!/usr/bin/env python3
"""R56 Opt G-2 / L3 — alt-tile 256x256 swap on (128256, 32768, 4096).

Current baseline: R53D3B_3_AITER 64x1024 -> 86.93% pct_comp.
Alt: AITER 256x256 via R50D shim AS-IS (no rebuild). Heuristic eff 60.2 -> 128.0
on identical local_round=251.

Grid: bdx=256, gdx=ceil(N/256)=128, gdy=ceil(M/256)=501, gdz=1.
"""
import argparse
...
COMPETITOR_TFLOPS = 4536.4  # bench_all_42.py
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G2_L3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-2 / L3 — mode={args.mode}, gpu={gpu_list}, "
