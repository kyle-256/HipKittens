"""R27 Dev A: preheat once, then run the bench 5x in same process for one
already-built variant. Prints CSV-style lines we can aggregate.

Usage: HIP_VISIBLE_DEVICES=0 python3 r27a_bench5x.py <layout> M N K
Env:   N_RUNS (default 5), MXFP8_WARMUP, MXFP8_ITERS
"""
import math
import os
import statistics
...
if layout == "rcr":
    ...
    Bs = preshuffle_v2_b(Bse)
    C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
    fn = lambda: tk_mxfp8_layouts.gemm_rrr_pq_v2(A, B, As, Bs, C)
    A_ref = A.float() * expand_row(Ase, K)
    ...
    Bs = preshuffle_v2_b(Bse)
    C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
    fn = lambda: tk_mxfp8_layouts.gemm_crr_pq_v2(A, B, As, Bs, C)
...
print(f"SUMMARY median={med:.2f} stdev={sd:.2f} runs={n_runs}", flush=True)
