#!/usr/bin/env python3
"""R55 Opt D-5B/1 — bench harness for the aiter `.co` dlopen shim on (32768, 14336, 2048).

Reuses R50D shim AS-IS (bdx=256, kd max_flat_workgroup_size=256).
Tile = 256x256 (aiter heuristic pick). Marginal HK-VC perf claw-back probe.
HK R54 PASS_10/10 at 103.0% — D-3A-1 risk HIGH. AITER must clearly win.
...
Per-shape grid:
  gdx = ceil(N/256) = ceil(14336/256) = 56
  gdy = ceil(M/256) = ceil(32768/256) = 128
  gdz = 1, bdx = 256
"""
import argparse
...
COMPETITOR_TFLOPS = 3351.4  # from R54_INTEGRATION_10RUN.json for (32768, 14336, 2048)
HK_PCT_COMP = 103.03  # R54 HK PASS_10/10 baseline
...
def run_one_seed(seed: int, gpu_id, do_perf: bool, timeout: int = 1500):
...
def main():
    ...
    ap.add_argument("--tag", default="R55_OPT_D5B_1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R55 Opt D-5B/1 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K}), tile={TILE_M}x{TILE_N}, HK_pct={HK_PCT_COMP}")
    log_lines.append("=" * 90)
    ...
    summary = {
        ...
        "hk_pct_comp_baseline": HK_PCT_COMP,
        "n_runs": len(results),
        "n_OK": n_ok,
