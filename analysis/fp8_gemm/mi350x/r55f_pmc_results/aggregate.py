#!/usr/bin/env python3
"""R55F aggregator — adapted from R54B/aggregate.py.

Targets RRR layout (V2 scaled kernel for MXFP8) on:
  - 8B_GateUp  (4096x14336x4096)  — primary HEADROOM cell (94.0% MX/FP8)
  - 8B_QO      (4096x4096x4096)   — cross-shape, K=4096
  - 70B_QO     (4096x8192x8192)   — cross-shape, K=8192
  - 70B_Down   (4096x8192x28672)  — cross-shape, K=28672 (matches R54B primary)

The cross-shape sweep tests K-scaling of the bottleneck class.
"""
import csv, os, statistics, json, glob

ROOT = os.path.dirname(os.path.abspath(__file__))

# MXFP8 RRR uses V2 (rrr_exact_8wave_scaled_kernel<true,2>) when MXFP8_PRESHUFFLE_QUANT=1
# FP8 RRR baseline kernel is rrr_exact_8wave_kernel
KERNEL_TAGS = {
    "mxfp8": ["rrr_exact_8wave_scaled_kernel"],
    "fp8":   ["rrr_exact_8wave_kernel"],
}
...
def merge_sets(label, dtype):
    ...
    knames = set()
    for s in SETS:
        d = load_cell(f"{label}_{dtype}_rrr_{s}", dtype)
        if d is None:
            continue
...
print()
print("=" * 100)
print("=== SUMMARY MXFP8/FP8 ratio per cell (RRR layout) ===")
print("=" * 100)
