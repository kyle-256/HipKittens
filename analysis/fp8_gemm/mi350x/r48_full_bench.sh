#!/bin/bash
# R48 Full bench: strict SCLK protocol per Dev A finding + Reviewer recommendation
# - 5x runs per cell (instead of R45's 1x)
# - 30s cooldown between runs
# - 60s cooldown between rebuilds
# - Reports median + range per cell
set -euo pipefail
cd "$(dirname "$0")"
...
# One MXFP8_CHECK=1 run per (shape, layout) for SNR + det
run_check() {
...
for i in "${!SHAPES[@]}"; do
    ...
    # MXFP8 build + bench all 3 layouts
    ...
    # MXFP8 correctness check (1 run per layout, MXFP8_CHECK=1 with det)
    for LAY in rcr rrr crr; do
...
def get_tf_runs(label, kind, lay):
    ...
    for r in range(1, RUNS+1):
        ...
        try:
            t = open(p).read()
            # extract TFLOPS from final results section
...
print()
print(f"{'Shape':<20} | {'Layout':>6} | {'FP8 med':>8} {'MX med':>8} {'MX/FP8':>7} | {'MX spread':>9} | {'SNR':>6} {'Det':>4} | {'Status':>6}")
print("-"*110)
...
for label in order:
    for lay in ["rcr", "rrr", "crr"]:
        ...
        line = (f"{names[label]:<20} | {lay.upper():>6} | "
                ...
                f"{mx_spread:8.2f}% | {snr_s:>6} {det:>4} | {status:>6}")
        print(line); lines.append(line)
    print()

with open(f"{D}/SUMMARY.txt", "w") as f:
    f.write(f"R48 strict-protocol baseline ({RUNS} runs/cell, median)\n")
    f.write(f"{'Shape':<20} | {'Layout':>6} | {'FP8 med':>8} {'MX med':>8} {'MX/FP8':>7} | {'MX spread':>9} | {'SNR':>6} {'Det':>4} | {'Status':>6}\n")
    f.write("-"*110 + "\n")
