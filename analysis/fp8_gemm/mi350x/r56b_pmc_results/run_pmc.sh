#!/bin/bash
# R56B PMC harness — adapted from R55F/run_pmc.sh, targeting CRR layout.
# Primary: 70B Gate/Up CRR (the only un-PMC'd open HEADROOM cell, MX/FP8=88.95%).
# Cross-shape: 70B Q/O CRR (CONFIRMED PASS at 94.59%) and 70B Down CRR
# (closes the K-axis side, references the 6x v_lshrrev floor known on CRR).
# GPU 1, MXFP8_WARMUP=100 ITERS=200, 30s cooldown between rocprof invocations.
set -uo pipefail

WORKSPACE="/shared_nfs/kyle/test/Hipkittens2/analysis/fp8_gemm/mi350x/r56b_workspace"
OUTROOT="/shared_nfs/kyle/test/Hipkittens2/analysis/fp8_gemm/mi350x/r56b_pmc_results"
...
# 70B GU CRR is the un-PMC'd HEADROOM cell.
# 70B QO CRR (K=8192, smaller N) — same K, different N tile bookkeeping
# 70B Down CRR (K=28672) — large-K reference (CRR floor known here)
CELLS=(
...
build_for_shape() {
    ...
    mkdir -p "$shape_dir"
    # Symlink stable source/include files
    ...
    # for sys.path[0], pulling stale .so via parent mi350x dir. (R55F fix)
    for f in test_python.py test_mxfp8_python.py; do
        if [ ! -f "$shape_dir/$f" ] || [ -L "$shape_dir/$f" ]; then
            rm -f "$shape_dir/$f"
            cp "$WORKSPACE/$f" "$shape_dir/$f"
        fi
    done
    ...
    if [ "$kind" = "fp8" ]; then
        ...
        for attempt in 1 2 3; do
            make TARGET=tk_fp8_layouts SRC=kernel_fp8_layouts.cpp \
                CXXFLAGS="-w -DM_DIM=$M -DN_DIM=$N -DK_DIM=$K" > "$LOG" 2>&1 && break
            echo "  build retry $attempt for fp8 $label"
            sleep 5
        done
        ...
        for attempt in 1 2 3; do
            make TARGET=tk_mxfp8_layouts SRC=kernel_mxfp8_layouts.cpp \
                CXXFLAGS="-w -DM_DIM=$M -DN_DIM=$N -DK_DIM=$K" > "$LOG" 2>&1 && break
            echo "  build retry $attempt for mxfp8 $label"
            sleep 5
        done
        if [ -f "$SO" ]; then echo "${M}x${N}x${K}" > "${SO}.shape"; fi
    fi
...
run_pmc() {
    ...
    local outdir="$OUTROOT/${label}_${kind}_crr_${pmcset}"
    mkdir -p "$outdir"
    ...
    if [ "$kind" = "fp8" ]; then
        ...
        FP8_WARMUP=$WARMUP FP8_ITERS=$ITERS FP8_CHECK=0 FP8_LAYOUTS=crr \
        ...
        MXFP8_CRR_PRESHUFFLE_V2_RUNTIME=1 \
        rocprofv3 -i "$pmcfile" --output-format csv -d "$outdir" \
