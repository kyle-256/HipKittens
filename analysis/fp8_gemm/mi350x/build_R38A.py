#!/usr/bin/env python3
"""R38 Opt A: like build_R37.py but compiled with -DR38A_INLINE_BUFLOAD_LDS=1.

R38A_INLINE_BUFLOAD_LDS=1 replaces the `__builtin_amdgcn_raw_buffer_load_lds`
intrinsic call inside `emit_one_pf` with an `asm volatile("buffer_load_dwordx4
... offen lds")` block carrying a "memory" clobber. The intent is to defeat any
LLVM scheduler-aggressive flag (`max-memory-clause` or implicit clang
reordering) that breaks the fused step34 path on the 19 WRONG_OUTPUT shapes.

Module names use a `_R38A` suffix to keep the .so files distinct from R37.
"""
import json
import os
import re
...
BUILD_DIR = os.path.join(SCRIPT_DIR, "build_R38A")
...
# R37 stripped this; we keep it stripped for R38A as well (defense in depth).
SCHED_BAD = "-mllvm -amdgpu-sched-strategy=max-memory-clause"

# R38A: extra define to enable the inline-asm path.
R38A_DEFINE = "-DR38A_INLINE_BUFLOAD_LDS=1"
...
def build_one(n_dim, k_dim, parent_tag, base_cppflags):
    module_name = f"tk_mxfp4_gluon_cpp_n{n_dim}_k{k_dim}_{parent_tag}_R38A"
    out_file = f"{module_name}{EXT_SUFFIX}"
    out_path = os.path.join(BUILD_DIR, out_file)
    key = f"N={n_dim},K={k_dim},{parent_tag}_R38A"
    ...
    wrapper_src = os.path.join(BUILD_DIR, f"wrap_n{n_dim}_k{k_dim}_{parent_tag}_R38A.cpp")
    with open(wrapper_src, "w") as f:
        f.write(patched)

    extra_cppflags = strip_bad_sched(base_cppflags)
    extra_cppflags = f"{extra_cppflags} {R38A_DEFINE}".strip()
...
def main():
    ...
    only_filter = sys.argv[2] if len(sys.argv) > 2 else None  # "all" or "MxNxK"
    ...
    for (m, n, k), parent_tag in BEST_VARIANTS.items():
        if only_filter and only_filter != "all":
            tag = f"{m}x{n}x{k}"
            if tag != only_filter:
                continue
    ...
    print(f"R38A builder: {len(plan)} shape entries, {len(unique_tasks)} unique builds")
    print(f"  (R38A_INLINE_BUFLOAD_LDS=1; -mllvm -amdgpu-sched-strategy=max-memory-clause stripped)")
    ...
    manifest = {
        ...
        "shapes_to_R38A": {
            f"{m}x{n}x{k}": f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38A"
    ...
    with open(os.path.join(SCRIPT_DIR, "R38A_BUILD_MANIFEST.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print("Manifest: R38A_BUILD_MANIFEST.json")
