#!/usr/bin/env python3
"""R33-OptD bench: 5-rep bench at warmup=200 iters=500 trim=10% on L6.

Per benchmark-rules.md (MANDATORY):
  warmup=200, iters=500, trim=10%, single-GPU isolation.

Usage:
  python3 bench_R33_optD.py --gpu N --reps 5
  python3 bench_R33_optD.py --gpu N --variants V1_aitersrd --reps 1
"""
...
VARIANTS = [
    ...
    ("V0_legacy_fork",    "tk_mxfp4_gluon_cpp_n32768_k128256_R33D_v12_memc_btw_all_legacyfork"),
    ("V1_aitersrd",       "tk_mxfp4_gluon_cpp_n32768_k128256_R33D_v12_memc_btw_all_aitersrd"),
    ("V2_aitersrd_vm15",  "tk_mxfp4_gluon_cpp_n32768_k128256_R33D_v12_memc_btw_all_aitersrd_vm15"),
    ("V3_aitersrd_vm25",  "tk_mxfp4_gluon_cpp_n32768_k128256_R33D_v12_memc_btw_all_aitersrd_vm25"),
]
...
def main():
    ...
    p.add_argument("--out", default="R33_OPT_D_BENCH_RESULTS.json")
    args = p.parse_args()

    selected = VARIANTS if args.variants is None else [v for v in VARIANTS if v[0] in args.variants]
    print(f"Bench R33-D: {len(selected)} variants × {args.reps} reps on GPU{args.gpu}")
    ...
    for tag, mod_name in selected:
        ...
        for r in range(args.reps):
            ...
            if res.get("status") == "OK":
                ...
                print(f"  rep{r+1}: {tf:.1f} TFLOPS  ({marker})  wall={res['wall_s']}s", flush=True)
            else:
                print(f"  rep{r+1}: FAIL {res.get('status')}  wall={res.get('wall_s','?')}s err={res.get('error', res.get('stderr','')[:200])}", flush=True)
