#!/usr/bin/env python3
"""R51 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R50_INTEGRATION.py with these changes:
  ...
  3. Per-shape backend dispatch — 4 aiter shim cells (all reuse R50D shim AS-IS):
       ...
       - all other 38 shapes -> R44/R50 baseline manifest entries unchanged
  4. Output defaults: R51_INTEGRATION_10RUN.{json,log}
  5. Round label = R51_INTEGRATION
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R51 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R51_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R51_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R51_INTEGRATION_10RUN.log")
    args = ap.parse_args()
    ...
    log(f"R51 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
