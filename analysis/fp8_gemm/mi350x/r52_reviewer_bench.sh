#!/bin/bash
# R52 Reviewer — refresh of the 21-cell strict-SCLK MXFP8 baseline at HEAD.
# Modeled on r48_full_bench_resume.sh. Pure HEAD baseline (no extra -D).
#
# Protocol:
#   - 5 runs per cell, 30 s cooldown between runs
#   - 60 s rebuild cooldown between cells
#   - MXFP8_WARMUP=100, MXFP8_ITERS=200
#   - HIP_VISIBLE_DEVICES=7 (exclusive)
#   - Median scoring; 5-number summary in SUMMARY.txt
set -uo pipefail
cd "$(dirname "$0")"
...
for i in "${!SHAPES[@]}"; do
    ...
    BUILD_LOG="$OUTDIR/${L}_mxfp8_build.log"
    make TARGET=tk_mxfp8_layouts SRC=kernel_mxfp8_layouts.cpp \
        CXXFLAGS="-w -DM_DIM=$M -DN_DIM=$N -DK_DIM=$K" > "$BUILD_LOG" 2>&1
    echo "  MXFP8 built (log: $BUILD_LOG)"
    for LAY in rcr rrr crr; do
...
with open(f"{D}/SUMMARY.txt", "w") as f:
    f.write(f"R52 Reviewer strict-protocol baseline (target {RUNS} runs/cell, median)\n")
    f.write(hdr + "\n")
    f.write("-"*len(hdr) + "\n")
