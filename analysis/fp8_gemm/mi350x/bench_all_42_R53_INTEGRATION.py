#!/usr/bin/env python3
"""R53 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R52_INTEGRATION.py with these changes:
  ...
  3. Per-shape backend dispatch — 15 aiter shim cells (all reuse R50D shim AS-IS):
        FROM R52 (carried, 256x256 tile):
          - (4096,  32768,  28672)   R50D
          - (14336, 4096,   32768)   R51D1
          - (16384, 4096,   28672)   R51D2
          - (28672, 4096,   16384)   R51D3
          - (4096,  28672,  32768)   R52D2A
          - (4096,  32768,  128256)  R52D2B
          - (4096,  4096,   32768)   R52D2C
        NEW IN R53 (8 new aiter overrides, non-256x256 tiles):
          D-3A (96x640 tile):
          - (6144,  4096,   16384)   R53D3A_2 (HK-VC perf claw-back +22.65pp)
          - (4096,  6144,   32768)   R53D3A_3 (HK-VC perf claw-back +48.82pp)
          D-3B (64x1024 tile):
          - (4096,  32768,  14336)   R53D3B_1 (NO_VC -> +1 NET VC rescue)
          - (32768, 4096,   14336)   R53D3B_2 (NO_VC -> +1 NET VC rescue)
          - (128256,32768,  4096)    R53D3B_3 (HK-VC marginal +0.21pp)
          D-3C (64x1024 tile):
          - (14336, 32768,  4096)    R53D3C_1 (HK-VC marginal +1.28pp)
          - (28672, 32768,  4096)    R53D3C_2 (HK-VC marginal +0.53pp)
          - (16384, 4096,   14336)   R53D3C_3 (HK-VC marginal +2.37pp)
       all other 27 shapes -> R44/R50 baseline manifest entries unchanged
  4. Output defaults: R53_INTEGRATION_10RUN.{json,log}
  5. Round label = R53_INTEGRATION
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R53 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R53_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R53_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R53_INTEGRATION_10RUN.log")
    args = ap.parse_args()
    gpus = [int(g) for g in args.gpus.split(",")]
    assert args.runs <= len(RANDOM_SEEDS), f"runs={args.runs} exceeds {len(RANDOM_SEEDS)} seeds"
    ...
    log(f"R53 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
    log(f"  warmup={WARMUP}, iters={ITERS}, trim={TRIM}, seeds={RANDOM_SEEDS[:args.runs]}")
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "round": "R53_INTEGRATION",
            "warmup": WARMUP, "iters": ITERS, "trim_frac": TRIM,
            "snr_threshold_db": SNR_THRESHOLD_DB,
            "wrong_cell_gate": WRONG_CELL_GATE,
            "finite_gate": FINITE_GATE,
            "random_seeds": RANDOM_SEEDS[:args.runs],
            "n_shapes": len(jobs),
            "n_runs": args.runs,
            "ok_threshold": OK_TH,
            "gpus": gpus,
