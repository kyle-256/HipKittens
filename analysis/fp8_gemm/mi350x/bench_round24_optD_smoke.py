#!/usr/bin/env python3
"""R24D smoke bench (warmup=200 iters=500 trim=10%) — A-only non-temporal hint.

Per benchmark-rules.md MANDATORY: warmup=200, iters=500, 10% trim.
GPUs 0-3 (avoid 4-7; another agent uses those).
"""
...
def main():
    ...
    print(f"R24D smoke: {len(args_list)} bench jobs (3 shapes x 3 variants), GPUs {GPU_POOL}")
    ...
    with open(os.path.join(SCRIPT_DIR, "bench_round24_optD_smoke.json"), "w") as f:
        json.dump(out, f, indent=2)

    # Summary: deltas vs each shape's _r24d_baseline
    print("\n" + "=" * 110)
    print("SUMMARY (Δ vs _r24d_baseline, +1.5pp gate)")
    print("=" * 110)
    for (lab, *_rest) in SHAPES:
        b = out["shapes"][lab].get("_r24d_baseline", {})
        bt = b.get("tflops")
        if bt is None:
