#!/usr/bin/env python3
"""R36: Build _f34 (FUSED_STEP34=1) versions of every BEST_VARIANTS entry.

For each (M,N,K) -> variant_tag in BEST_VARIANTS:
  Look up the CPPFLAGS for variant_tag from bench_all_42.py's `variants` list,
  append " -DFUSED_STEP34=1", and build a module named
  tk_mxfp4_gluon_cpp_n{N}_k{K}_{variant_tag}_f34.

Builds in parallel (default 24 workers — uses host CPU cores; build is independent
of GPUs).
"""
import json
import os
...
def parse_variants_from_bench():
    """Pull `variants = [...]` out of bench_all_42.py and exec to dict."""
...
def build_one(n_dim, k_dim, parent_tag, base_cppflags):
    """Compile the _f34 module for a (N, K, parent_tag) triple."""
    f34_suffix = f"_{parent_tag}_f34"
    module_name = f"tk_mxfp4_gluon_cpp_n{n_dim}_k{k_dim}{f34_suffix}"
    out_file = f"{module_name}{EXT_SUFFIX}"
    out_path = os.path.join(BUILD_DIR, out_file)
    key = f"N={n_dim},K={k_dim},{parent_tag}_f34"

    if os.path.exists(out_path):
        return (key, "cached", 0.0)

    # Patch PYBIND11 module name
    ...
    wrapper_src = os.path.join(BUILD_DIR, f"wrap_n{n_dim}_k{k_dim}{f34_suffix}.cpp")
    with open(wrapper_src, "w") as f:
        f.write(patched)

    extra_cppflags = base_cppflags + " -DFUSED_STEP34=1"
    ...
    if result.returncode != 0 or not os.path.exists(out_path):
        return (key, "FAILED", elapsed, result.stderr[-1000:])
...
def main():
    ...
    # Construct task list: (n, k, parent_tag, cppflags) for every BEST_VARIANTS entry
    tasks = []
    plan = []
    ...
    # De-duplicate (same N,K,tag from multiple shapes share .so)
    unique_tasks = list(set(tasks))
    print(f"R36 _f34 builder: {len(plan)} shape entries, {len(unique_tasks)} unique builds")
    ...
    manifest = {
        ...
        "shapes_to_f34": {
            f"{m}x{n}x{k}": f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_f34"
    ...
    with open(os.path.join(SCRIPT_DIR, "R36_F34_BUILD_MANIFEST.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print("Manifest: R36_F34_BUILD_MANIFEST.json")
