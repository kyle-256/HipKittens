#!/usr/bin/env python3
"""R43 Opt A bench harness — A.fix1 (R43A_GATE_PF_TAIL_KBOUND) on K=28672 CRASH shapes.

Bench rules: warmup=200, iters=500, trim=0.10, FINITE_GATE=0.98 (R42 Opt A1
relaxed gate, matches bench_all_42_R42A1.py). Random scales (R39B), seed=42.

Modes:
  --runs 1   = smoke   (1 rep per cell)
  --runs 5   = full 5-run consensus

A 5-run "VC" is: n_OK >= 4 AND wcf_max < 0.02 AND wcf_std < 0.01 AND fin_min >= 0.98.
"""
import json
import os
import re
...
FINITE_GATE      = 0.98   # R42 Opt A1 relaxed gate
RANDOM_SEED = 42
...
ALL_CELLS = ["p2b_ctrl", "p2b_fix1", "p2b_fix1b", "p2b_fix1c", "p2b_fix1d", "p2b_fix1e", "p2b_fix1f"]


def make_runner_script(module_name, so_path, m, n, k, comp, run_seed):
    return f"""\
...
def find_so(m, n, k, cell):
    ...
    mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R43A_{cell}"
...
def bench_one_shape(m, n, k, comp, cell, gpu_id, run_seed):
    ...
    if not so_path:
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                ...
                "best_variant": f"R43A_{cell}", "cell": cell}

    script = make_runner_script(module_name, so_path, m, n, k, comp, run_seed)
    env = os.environ.copy()
...
def run_one_pass(gpus, run_label, shape_cells, run_seed):
    print(f"R43A Bench [{run_label}, seed={run_seed}] - GPUs: {gpus}, n_jobs={len(shape_cells)}")
    print(f"warmup={WARMUP}, iters={ITERS}, trim={TRIM}, "
          f"SNR>={SNR_THRESHOLD_DB}dB AND wcf<{WRONG_CELL_GATE} AND "
          f"finite>={FINITE_GATE}")
    print("=" * 110)
    ...
    def run_gpu(gpu_id):
        for (i, (m, n, k, comp, cell)) in gpu_tasks[gpu_id]:
            r = bench_one_shape(m, n, k, comp, cell, gpu_id, run_seed)
            with lock:
                results[i] = r
                if r["status"] == "OK":
...
def main():
    ...
    ap.add_argument("--out", default="R43_OPT_A_SMOKE.json")
    ...
    print(f"R43A bench: {len(shapes)} shapes x {len(cells)} cells = "
    ...
    for ri in range(args.runs):
        # Use distinct seeds per run for true cross-run statistical sample.
        run_seed = base_seed + ri
        results, elapsed = run_one_pass(gpus, f"run{ri+1}/{args.runs}", shape_cells, run_seed)
        all_runs.append({"run": ri+1, "seed": run_seed,
    ...
    for i in range(n_jobs):
        ...
        finites = [r.get("kernel_finite") for r in per_run if r.get("kernel_finite") is not None]
        statuses = [r.get("status") for r in per_run]

        wcf_max = max(wcfs) if wcfs else None
        wcf_std = statistics.pstdev(wcfs) if len(wcfs) > 1 else 0.0
        fin_min = min(finites) if finites else None
        ok_tflops = [r["tflops"] for r in oks if r.get("tflops") is not None]
        if ok_tflops:
            ok_tflops_sorted = sorted(ok_tflops)
            tflops_p50 = ok_tflops_sorted[len(ok_tflops_sorted) // 2]
        else:
            tflops_p50 = None

        # 5-run "VC" gate: n_OK>=4 AND wcf_max<0.02 AND wcf_std<0.01 AND fin_min>=0.98
        vc = (
            n_OK >= max(1, args.runs - 1)  # >= 4 of 5 (>= 1 of 1 etc)
        ...
        if n_OK > args.runs // 2:
            chosen = dict(sorted(oks, key=lambda r: r["tflops"])[len(oks) // 2])
            chosen["consensus"] = f"PASS_{n_OK}/{args.runs}"
        elif len(crashes) > args.runs // 2:
            chosen = dict(crashes[0]); chosen["consensus"] = f"CRASH_{len(crashes)}/{args.runs}"
        elif len(wrongs) > args.runs // 2:
            chosen = dict(wrongs[0]); chosen["consensus"] = f"WRONG_{len(wrongs)}/{args.runs}"
        else:
            chosen = dict(per_run[0])
            chosen["consensus"] = f"MIXED_oks={n_OK}_wrongs={len(wrongs)}_crashes={len(crashes)}"

        chosen["wcf_per_run"] = [r.get("wrong_cell_frac") for r in per_run]
        chosen["finite_per_run"] = [r.get("kernel_finite") for r in per_run]
        chosen["status_per_run"] = statuses
        if any(r.get("status") == "OK" for r in per_run):
            chosen["tflops_per_run"] = [r.get("tflops") for r in per_run]
        chosen["n_OK"] = n_OK
        chosen["wcf_max"] = wcf_max
        chosen["wcf_std"] = round(wcf_std, 6)
        chosen["fin_min"] = fin_min
        chosen["tflops_p50"] = tflops_p50
        chosen["VC"] = vc
    ...
    for c in consensus:
        ...
        fin_s = f"{fin:.4f}" if fin is not None else "  N/A "
        tf = c.get("tflops_p50")
        tf_s = f"{tf:.1f}" if tf is not None else "   N/A"
        print(f"  {c['M']:>6}x{c['N']:>6}x{c['K']:>6}  {cell:<14s}  {cons:<14s}  "
              f"n_OK={n_OK} wcf_max={wcf_s} fin_min={fin_s} tflops_p50={tf_s} {vc}")
