#!/usr/bin/env python3
"""R44 Opt A Phase 3 — bench Opt C back-edge vmcnt(0) drain fix variants.

Test the R44A_BACKEDGE_VMCNT_DRAIN macro on FUSED_STEP34 + TAIL_SPLIT path.
"""
import json
import math
import os
...
ALL_CELLS = [
    "fused_ts_drain", "fused_ts_drain_pfoff104", "fused_ts_drain_pfoff32",
    "fused_ts_drain_pfoff8", "nonfused_ts_drain_R38B",
    "fused_ts_drain_R40A", "fused_ts_drain_R40A_pfoff104",
    "fused_ts_drain_memc_pfoff104",
]


def find_so(m, n, k, cell):
    mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_ts_lgk2_gm7_kx28672_btw_all_p3_R44A_{cell}"
...
def dequant_fp4_rows(packed_uint8, K, row_lo, row_hi):
    ...
    lo = (pk & 0x0F).to(torch.int64); hi = ((pk >> 4) & 0x0F).to(torch.int64)
    rows, cols = pk.shape
...
def main():
    ...
    ap.add_argument("--out", default="R44_OPT_A_PHASE3_5RUN.json")
    args = ap.parse_args()
    ...
    print(f"R44 Opt A Phase 3: {len(jobs)} jobs across {len(cells)} cells x {len(ALL_SHAPES)} shapes x {len(SEEDS)} seeds")
    print(f"  cells: {cells}")
    print(f"  GPUs: {gpus}")
    print("=" * 110)
    ...
    def run_gpu(gpu_id):
        for (i, (m, n, k, comp, cell, seed)) in gpu_tasks[gpu_id]:
            ...
            with lock:
                ...
                if r["status"] == "OK":
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<32s} seed={seed} {r['tflops']:>7.1f} TF fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
                else:
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<32s} seed={seed} {r['status']} fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
    threads = []
    for g in gpus:
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "n_shapes": len(ALL_SHAPES), "n_cells": len(cells), "n_runs": len(SEEDS),
            "gpus": gpus, "elapsed_minutes": round(elapsed / 60, 1),
            "consensus": consensus,
        }, f, indent=2)

    print(f"\n=== R44 Opt A Phase 3 SUMMARY ===")
    ...
    for c in consensus:
        ...
        print(f"  {c['M']:>5}x{c['N']:>5}x{c['K']:>5} {c['cell']:<32s} n_OK={c['n_OK_5']}/5 "
