#!/usr/bin/env python3
"""R59 Opt J-2/S-2 — 64x1024 alt-tile probe on HK cell (32768, 14336, 2048).

R59 J-2/S-2: 64x1024 aiter alt-tile via R50D shim AS-IS (HK baseline 100.49% R40B, VC-flipped).
Grid: gdx=ceil(32768/64)=512, gdy=ceil(14336/1024)=14, gdz=1, bdx=256.
Hypothesis: low-medium (eff=60.2 vs HK 256x256). PROMOTE only if SMOKE > 101.49% (HK + 1.0pp).
Else: ACCEPT_FALLBACK (axis closure for L3 64x1024).

Modes:
  --mode smoke    5 seeds [101,202,303,404,505], full correctness gate + perf measurement
  --mode 10run    10 INDEPENDENT seeds, n_OK / wcf_max / wcf_std / fin_min stats
...
KERNEL_NAME = "_ZN5aiter42f4gemm_bf16_per1x32Fp4_BpreShuffle_64x1024E"
TILE_M = 64
TILE_N = 1024
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 32768, 14336, 2048
COMPETITOR_TFLOPS = 3351.4  # bench_all_42.py competitor for (32768, 14336, 2048)
...
def main():
    ...
    ap.add_argument("--tag", default="R59_OPT_S2")
    ...
    if args.mode == "smoke":
        ...
        seeds = args.seeds or [101, 202, 303, 404, 505]
        do_perf = True
    else:
    ...
    results = []
    log_lines = []
    log_lines.append(f"R59 Opt J-2/S-2 bench - mode={args.mode}, gpu={gpu_list}, "
