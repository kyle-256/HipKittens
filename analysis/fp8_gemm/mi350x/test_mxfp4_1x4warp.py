"""Test/benchmark: MXFP4 1x4 warp layout kernel."""
import math, os, sys, torch
torch.manual_seed(0)

import tk_mxfp4_1x4warp
...
num_warmup = int(os.environ.get("MXFP4_WARMUP", "200"))
num_iters  = int(os.environ.get("MXFP4_ITERS", "500"))
check      = os.environ.get("MXFP4_CHECK", "1") != "0"

k_blocks = K // 32

print(f"=== MXFP4 1x4 Warp: M={M}, N={N}, K={K} ===")
...
run = lambda: tk_mxfp4_1x4warp.gemm_rcr(A, B, A_sc, B_sc, C)

for _ in range(num_warmup):
    C.zero_(); run()
torch.cuda.synchronize()

# Timed iterations using hipEvent with trimmed mean
times_ms = []
...
times_ms.sort()
trim_count = int(len(times_ms) * 0.10)
if trim_count > 0:
