#!/usr/bin/env python3
"""R51 Opt D-3 — bench harness for the aiter `.co` dlopen shim on (28672, 4096, 16384).
...
COMPETITOR_TFLOPS = 5350.6  # from bench_all_42.py for (28672,4096,16384)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D3_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D3_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D3_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D3_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R51 Opt D-3 bench — mode={args.mode}, gpu={args.gpu}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
