"""R32 Dev C C4: BABA-paired V2-RRR vs V2-CRR for K-large 70B Down.

CRR uses A=(K,M), B=(K,N); entry gemm_crr_pq_v2.
RRR uses A=(M,K), B=(K,N); entry gemm_rrr_pq_v2.
Both produce C=(M,N) bf16. Same A/B preshuffles for scales.

Env: M, N, K, SO_CRR, SO_RRR, PHYS_GPU, N_PAIRS, MXFP8_WARMUP, MXFP8_ITERS
"""
...
so_crr = os.environ['SO_CRR']
so_rrr = os.environ['SO_RRR']
mod_crr_name = os.environ.get('MOD_CRR', 'tk_mxfp8_layouts_crr')
...
mod_crr = load(mod_crr_name, so_crr)
...
A_crr = gen_fp8(K, M); B_crr = gen_fp8(K, N)
Ase_c = gen_scale(M, k_blocks); Bse_c = gen_scale(N, k_blocks)
As_c = preshuffle_v2_a(Ase_c); Bs_c = preshuffle_v2_b(Bse_c)
C = torch.zeros(M, N, dtype=torch.bfloat16, device='cuda')
A_crr_ref = A_crr.float() * expand_col(Ase_c, K)
B_crr_ref = B_crr.float() * expand_col(Bse_c, K)
C_crr_ref = A_crr_ref.T @ B_crr_ref
...
def fn_crr(): mod_crr.gemm_crr_pq_v2(A_crr, B_crr, As_c, Bs_c, C)
def fn_rrr(): mod_rrr.gemm_rrr_pq_v2(A_rrr, B_rrr, As_r, Bs_r, C)

# Correctness
for fn, name, ref in [(fn_crr, 'CRR', C_crr_ref), (fn_rrr, 'RRR', C_rrr_ref)]:
...
for w in range(warmup_pairs):
    avg_c = bench_fn(fn_crr); tc = flops / (avg_c * 1e9)
    avg_r = bench_fn(fn_rrr); tr = flops / (avg_r * 1e9)
    print(f'WARMUP {w} CRR={tc:.2f} RRR={tr:.2f} sclk={read_sclk()}', flush=True)
for r in range(n_pairs):
    avg_c = bench_fn(fn_crr); tc = flops / (avg_c * 1e9)
    avg_r = bench_fn(fn_rrr); tr = flops / (avg_r * 1e9)
    avg_c2 = bench_fn(fn_crr); tc2 = flops / (avg_c2 * 1e9)
    avg_r2 = bench_fn(fn_rrr); tr2 = flops / (avg_r2 * 1e9)
    crr_tf.append(tc); crr_tf.append(tc2); rrr_tf.append(tr); rrr_tf.append(tr2)
    print(f'PAIR {r} CRR_1={tc:.2f} RRR_1={tr:.2f} CRR_2={tc2:.2f} RRR_2={tr2:.2f}', flush=True)
...
print(f'CRR median={statistics.median(crr_tf):.2f} mean={statistics.mean(crr_tf):.2f} stdev={statistics.stdev(crr_tf):.2f} n={len(crr_tf)}')
...
t = welch(crr_tf, rrr_tf)
print(f'Welch t (CRR vs RRR) = {t:.3f}  (positive = RRR faster)')
delta_pct = (statistics.median(rrr_tf) - statistics.median(crr_tf)) / statistics.median(crr_tf) * 100
print(f'DELTA_MEDIAN_PCT RRR_vs_CRR = {delta_pct:+.3f}%')
print(f'CRR_TFLOPS_LIST {",".join(f"{x:.4f}" for x in crr_tf)}')
