#!/usr/bin/env python3
"""R53 Opt D-3B/1 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 14336).

Reuses R50D shim AS-IS (bdx=256, kd max_flat_workgroup_size=256 for 64x1024 verified).
Tile = 64x1024 (aiter heuristic pick). NO_VC rescue candidate.
...
COMPETITOR_TFLOPS = 5296.1  # from bench_all_42.py for (4096, 32768, 14336)
...
def main():
    ...
    ap.add_argument("--tag", default="R53_OPT_D3B_1")
    args = ap.parse_args()
    ...
    results = []
    log_lines = []
    log_lines.append(f"R53 Opt D-3B bench — mode={args.mode}, gpu={gpu_list}, "
