...
// R33-D AITER SRD MODE — when AITER_SRD_MODE=1, replace flat-buffer SRD with
// aiter's CDNA `ADD_TID_ENABLE + INDEX_STRIDE=01` mode (per R33_AITER_ARCHAEOLOGY):
//   word1 |= 0x40000 (ADD_TID_ENABLE on bit 18 of word1)
//   word2  = 0xFFFFFFF0 (-16, structured per-lane envelope)
//   word3  = 0x00020000 (INDEX_STRIDE=01)
// This unlocks safe vmcnt(15) operation by switching the OOB envelope from
// per-buffer-flat to per-lane-stripe (~4 GiB minus 16 bytes per lane).
...
__device__ __forceinline__ i32x4 make_scale_srd(const uint8_t* ptr) {
    ...
    i32x4 srd = std::bit_cast<i32x4>(make_buffer_resource(
        base_ptr,
        MXFP4_SRD_NUM_RECORDS, MXFP4_SRD_CONFIG));
...
#define GROUP_SIZE_M 4
    ...
    // Tile SRDs
    // R33-D AITER_SRD_MODE: see make_scale_srd above for mode details.
    auto make_srd = [](const void* raw_ptr) {
        uint64_t base_ptr = static_cast<uint64_t>(reinterpret_cast<std::uintptr_t>(raw_ptr));
        base_ptr |= MXFP4_SRD_PTR_OR;
