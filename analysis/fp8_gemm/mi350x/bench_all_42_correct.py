#!/usr/bin/env python3
"""R36: Bench every shape's _f34 module with a correctness gate.

For each (M,N,K) → variant_tag in BEST_VARIANTS:
  1. Load module tk_mxfp4_gluon_cpp_n{N}_k{K}_{variant_tag}_f34
  2. Run kernel ONCE on random scaled FP4 inputs
  3. Compute kernel_finite (fraction of finite output cells)
  4. If kernel_finite < 0.995 → status="WRONG_OUTPUT", tflops=None
  5. Otherwise warmup=200, iters=500 trim=10% timed run

Per-shape execution is in its own subprocess for isolation; runs are
distributed across multiple GPUs.

Bench rules: warmup=200, iters=500, trim=10%, GPU isolation via HIP_VISIBLE_DEVICES.

Usage:
    python3 bench_all_42_correct.py [gpus_csv]   # default: 0,1,2,3,4,5,6,7
"""
import json
import os
import subprocess
...
ITERS = 500
TRIM = 0.10
SHAPE_TIMEOUT = 600  # 10 min/shape
CORRECTNESS_GATE = 0.995  # require >= 99.5% finite output cells
...
def make_runner_script(module_name, so_path, m, n, k, comp):
    """Render a self-contained runner that does correctness gate + timed bench."""
    return f"""\
...
try:
    ...
    # --- CORRECTNESS GATE INPUTS: constant scale -4 (matches R35 / snr_all_42_shapes.py)
    # Using random scale [-2,3] would cause bf16 overflow (max_out ~ 6*6*K * 2^6 >> 65k),
    # producing non-finite output even from a CORRECT kernel. Constant -4 keeps outputs bounded.
    A = gen_fp4(M, K)
    B = gen_fp4(N, K)
    ...
    A_sc_correct = preshuffle(sc_a_correct)
    B_sc_correct = preshuffle(sc_b_correct)
    # --- TIMING INPUTS: random scale (matches existing bench harness)
    ...
    # --- CORRECTNESS GATE: single run with bounded scales, measure finite frac ---
    C.zero_()
    run_correct()
    ...
    if finite_frac < GATE:
        out = {{
            "M": M, "N": N, "K": K,
            "tflops": None, "avg_ms": None, "comp": COMP,
            "status": "WRONG_OUTPUT",
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    module_name = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{parent_tag}_f34"
    so_path = os.path.join(BUILD_DIR, f"{module_name}{EXT_SUFFIX}")

    if not os.path.exists(so_path):
        return {
            "M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
            "comp": comp, "status": "MISSING_SO",
            "kernel_finite": None,
            "best_variant": parent_tag + "_f34",
        }
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_f34"
        return out
...
def main():
    ...
    print(f"R36 _f34 Correctness-Gated Bench - GPUs: {gpus}")
    print(f"Shapes: {len(ALL_SHAPES)}, "
          f"warmup={WARMUP}, iters={ITERS}, trim={TRIM}, "
          f"gate kernel_finite>={CORRECTNESS_GATE}")
    print("=" * 110)

    # Distribute shapes across GPUs round-robin
    ...
    for r in results:
        ...
        if r["status"] == "OK":
            if r["tflops"] >= r["comp"]:
                ...
                losses += 1
        elif r["status"] == "WRONG_OUTPUT":
            wrong += 1
        else:
            errs += 1

    print(f"WIN: {wins}/42  LOSE: {losses}/42  "
          f"WRONG_OUTPUT: {wrong}/42  ERR: {errs}/42  "
          f"({elapsed/60:.1f} min)")

    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R36_f34.json")
