#!/bin/bash
# R35 Dev A: 5th V2-RRR autotune predicate (8B Gate/Up shape 4096x14336x4096).
#
# Phases:
#  1. Source patch (already applied to kernel_mxfp8_layouts.cpp).
#  2. Build sanity (default 8192^3 + per-shape advisory verification).
#  3. Per-shape numerics + perf re-verify (c5).
#  4. Cross-GPU verify (GPU4) (c5).
#  5. Regression smoke test (re-bench c0/c1/c4/c8).
#
# Per R31/R32/R33/R34 closures:
...
build_so() {
  ...
  local mod_name="tk_mxfp8_r35a_${tag}"
  local out_so="${mod_name}${PYEXT}"
  local log="$HERE/r35a_build_${tag}.log"
...
# Target cells:
#   c5_8b_gate is THE new predicate target (4096x14336x4096)
# Regression smoke test cells (R34 Dev A SHIPs):
#   c0 c1 c4 c8 — must still emit 1 advisory each, Δ% must remain ≥+5%
...
if [[ "$phase" == "bench" || "$phase" == "all" ]]; then
  echo "########## BENCH PHASE — TARGET CELL c5 GPU${PHYS_GPU} ##########"
  for row in "${TARGET_CELLS[@]}"; do
    read -r tag M N K LA LB <<< "$row"
  ...
  echo "ALL R35A BENCHES DONE FOR GPU${PHYS_GPU}"
fi
