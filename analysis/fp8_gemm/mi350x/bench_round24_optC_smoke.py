#!/usr/bin/env python3
"""R24C smoke bench (warmup=200 iters=500 trim=10%) — outer-K pull-forward L2 prefetch.

Per benchmark-rules.md MANDATORY: warmup=200, iters=500, 10% trim.
Uses GPUs 0,1,2,4,6,7 (avoid 3 reserved for R24B / 5 reserved for sister mxfp8).
"""
...
# C-coverage check: warn if all-zero
nz = int(C.abs().sum().item() > 0)
print(json.dumps({{"tflops": round(t,2), "ms": round(avg,4), "n_iters_kept": len(times), "C_nonzero": nz}}))
"""
...
def main():
    ...
    print(f"R24C smoke: {len(args_list)} bench jobs ({len(SHAPES)} shapes x {len(VARIANT_SUFFIXES)} variants), GPUs {GPU_POOL}")
    ...
    with ProcessPoolExecutor(max_workers=len(GPU_POOL)) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if "error" in res:
                ...
                tag = "" if res.get("C_nonzero", 1) else "  [WARN: C all-zero]"
                print(f"  {lab:6s} {vs:30s} gpu={gpu}  {res['tflops']:>8.2f} TFLOPS{tag}", flush=True)

    print(f"\nElapsed: {time.time()-t0:.1f}s")
    with open(os.path.join(SCRIPT_DIR, "bench_round24_optC_smoke.json"), "w") as f:
        json.dump(out, f, indent=2)

    # Summary: deltas vs each shape's _r24c_baseline
    print("\n" + "=" * 110)
    print("SUMMARY (Δ vs _r24c_baseline, +1.5pp gate)")
    print("=" * 110)
    for (lab, *_rest) in SHAPES:
        b = out["shapes"][lab].get("_r24c_baseline", {})
        bt = b.get("tflops")
        if bt is None:
        ...
        for vs in VARIANT_SUFFIXES:
            ...
            print(f"    {vs:30s}  {vt:>8.2f}  Δ={d:+.2f} ({d_pp:+.2f}%)  {verdict}")
