#!/usr/bin/env python3
"""R57 Opt H-3 / K1 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 128256).

R57 H-3 / K1: 224x256 alt-tile probe on L8 (currently AITER 256x256, pct_comp=98.30%).
Alt: 224x256 via R50D shim AS-IS.

D-3A-1 RISK HIGH: must beat 99.30% pct_comp to PROMOTE (256x256 98.30 + 1.0pp).
...
Per-shape grid: gdx=ceil(N/256)=128, gdy=ceil(M/224)=19, gdz=1, bdx=256
"""
import argparse
...
KERNEL_NAME = "_ZN5aiter42f4gemm_bf16_per1x32Fp4_BpreShuffle_224x256E"
TILE_M = 224
TILE_N = 256
...
def main():
    ...
    ap.add_argument("--tag", default="R57_OPT_K1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-4 C3 bench - mode={args.mode}, gpu={gpu_list}, "
