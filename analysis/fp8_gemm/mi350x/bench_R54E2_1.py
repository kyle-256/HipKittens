#!/usr/bin/env python3
"""R54 Opt E-2/1 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 4096).

Reuses R50D shim AS-IS (bdx=256). Tile = 256x256 (aiter heuristic pick).
Cohort-race rescue candidate: R52 VC at 91.8%, R53 cohort LOSS PASS_9/10 fin_min=0.951.
...
COMPETITOR_TFLOPS = 4166.5  # from R53_INTEGRATION_10RUN.json for (4096, 32768, 4096)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_E2_1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R54 Opt E-2/1 bench — mode={args.mode}, gpu={gpu_list}, "
