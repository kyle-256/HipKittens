#!/usr/bin/env python3
"""R56 Opt G-4 / C3 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 128256).

R56 G-4 / C3: 192x256 alt-tile probe on L8 (currently AITER 256x256, pct_comp=98.32%).
Alt: 192x256 via R50D shim AS-IS.

D-3A-1 RISK HIGH: must beat 99.32% pct_comp to PROMOTE (256x256 98.32 + 1.0pp).
...
Per-shape grid: gdx=ceil(N/256)=128, gdy=ceil(M/192)=22, gdz=1, bdx=256
"""
import argparse
...
def main():
    ...
    ap.add_argument("--tag", default="R57_OPT_L3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R57 Opt L-3 bench - mode={args.mode}, gpu={gpu_list}, "
