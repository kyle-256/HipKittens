...
// ═══════════ V6 SPLIT-K (R32 Opt B POC) ═══════════
// Kernel-level K-split. Each launch processes a contiguous slice of the K-loop:
//   bt ∈ [BT_START, BT_END)   where BT_START = S_IDX * K_PER_SPLIT_ITERS
//   BT_END  = (S_IDX + 1) * K_PER_SPLIT_ITERS for non-last splits
//   BT_END  = k_byte_iters                    for the last split (absorbs remainder)
// Per-split kernels each write to their own bf16 output buffer (g.c). A separate
// epilogue kernel adds the per-split tiles to produce the final output.
//
// Defaults: K_SPLIT=1 (no split), S_IDX=0 → identical behavior to v1 baseline.
// Built variants for L6 use K_SPLIT=2, S_IDX={0,1}.
#ifndef K_SPLIT
#define K_SPLIT 1
#endif
#ifndef S_IDX
...
#endif
#if S_IDX >= K_SPLIT
#error "S_IDX must be < K_SPLIT"
#endif
// V6 split-K POC limits: only TAIL_SPLIT=1 + non-SWAP_STEP34 + non-FUSED main loop
// has been adapted. Reject other combos at compile time when K_SPLIT > 1.
#if K_SPLIT > 1
  #if defined(SWAP_STEP34_MAIN) && SWAP_STEP34_MAIN
    #error "V6 K_SPLIT>1 only supports SWAP_STEP34_MAIN=0 in POC"
  #endif
  #if defined(FUSED_STEP34) && FUSED_STEP34
    #error "V6 K_SPLIT>1 only supports FUSED_STEP34=0 in POC"
  #endif
  #if !defined(TAIL_SPLIT) || TAIL_SPLIT == 0
    #error "V6 K_SPLIT>1 requires TAIL_SPLIT=1 in POC"
  #endif
#endif
...
// V6 split-K: per-split bt range. Last split absorbs the remainder when
// k_byte_iters is not divisible by K_SPLIT.
constexpr int K_PER_SPLIT_ITERS = k_byte_iters / K_SPLIT;
constexpr int BT_START = S_IDX * K_PER_SPLIT_ITERS;
constexpr int BT_END   = (S_IDX == K_SPLIT - 1) ? k_byte_iters
                                                : (S_IDX + 1) * K_PER_SPLIT_ITERS;
constexpr int K_SPLIT_LEN = BT_END - BT_START;
static_assert(K_SPLIT_LEN >= 2, "K_SPLIT too large for K_DIM (need >= 2 K-tiles per split)");
...
#define GROUP_SIZE_M 4
    ...
    // ═══════════ Prologue ═══════════
    // V6 split-K: prologue starts at BT_START (= 0 for K_SPLIT=1).
    load_tiles(BT_START, 0);
    if (BT_END > BT_START + 1) load_tiles(BT_START + 1, 1);

    fp8e8m0_4 pf_a0[a_packs], pf_a1[a_packs], pf_bl[b_packs], pf_br[b_packs];
    {
        // V6 split-K: scale offset must match starting bt (= BT_START).
        constexpr uint32_t init_scale = static_cast<uint32_t>(BT_START) << 9;
        load_pq_scale_x2_async(a0_srd, lane_soff_x2, init_scale, pf_a0[0], pf_a0[1]);
        load_pq_scale_x2_async(a1_srd, lane_soff_x2, init_scale, pf_a1[0], pf_a1[1]);
        load_pq_scale_x2_async(bl_srd, lane_soff_x2, init_scale, pf_bl[0], pf_bl[1]);
        load_pq_scale_x2_async(br_srd, lane_soff_x2, init_scale, pf_br[0], pf_br[1]);
    }
    ...
    load_bl_direct_async(bl_init_vmem, srd_b_ps, bl_voff_ps,
        bl_ps_n0_soff + static_cast<uint32_t>(BT_START) * 2 * BL_K0_STRIDE);  // V6: start at BT_START
...
#define TAIL_SPLIT 0
    ...
    // Steady-state loop: bt = BT_START .. BT_END-2
    // Tail: bt = BT_END-1 (no prefetch, no next-iter scale/LDS loads)
    // V6 split-K: unroll selection is based on per-split iter count, not full k_byte_iters.
#ifdef UNROLL_K
  #if UNROLL_K == 0
    #pragma unroll
  #else
...
#elif K_SPLIT_LEN <= 32
    #pragma unroll 16
#elif K_SPLIT_LEN <= 128
    // V6 split-K: at K_SPLIT_LEN ≤ 128 (e.g. split=4 on L6), pragma unroll 16 fires
    // and R25C tail-pf-off mechanism becomes effective per R25-F.
    #pragma unroll 16
#else
    #pragma unroll 8
#endif
    for (int bt = BT_START; bt + 1 < BT_END; ++bt) {
        const int cur = bt & 1;
        const int nxt = 1 - cur;
        ...
        // V6 split-K: clamp prefetches to BT_END so we never prefetch the next split's data.
        const int pf_bt = (bt + 2 < BT_END) ? (bt + 2) : (BT_END - 1);
        tile_pf_params pf_a0_p = make_pf_params(A0_db[cur], g.a, coord<ST_tile>(0,0,br*2,     pf_bt), so_a, srd_a, base_a, lb_a0[cur], R22B_A_HINT_VAL);
        tile_pf_params pf_a1_p = make_pf_params(A1_db[cur], g.a, coord<ST_tile>(0,0,br*2+1,   pf_bt), so_a, srd_a, base_a, lb_a1[cur], R22B_A_HINT_VAL);
#if !DIRECT_BL
        tile_pf_params pf_bl_p = make_pf_params(Bl_db[cur], g.b, coord<ST_tile>(0,0,bc*2,     pf_bt), so_b, srd_b, base_b, lb_bl[cur], R22B_B_HINT_VAL);
        ...
        // We rebuild pf params at pf_bt+(OUTER_K_PF_DEPTH-1) clamped to BT_END-1
        ...
        {
            const int pf2_bt = (pf_bt + (OUTER_K_PF_DEPTH - 1) < BT_END)
        ...
        {
            const int pf3_bt = (pf_bt + 1 < BT_END) ? (pf_bt + 1) : (BT_END - 1);
#if (L2_PF_A > 0)
        ...
        // otherwise.
#if R25C_ACTIVE
        // V6 split-K: tail-no-pf is per-split (BT_END is the split's end).
        const bool _r25c_tail_no_pf = (bt >= BT_END - 1 - R25C_TAIL_PF_OFF_ITERS);
#else
    ...
    // V6 split-K: tail is the LAST iter of THIS split, not of the full K-loop.
    {
        const int bt = BT_END - 1;
        const int cur = bt & 1;
        const uint32_t sel_br_p0 = cur ? br_1_p0 : br_0_p0;
        const uint32_t sel_br_p1 = cur ? br_1_p1 : br_0_p1;
        const uint32_t sel_a1_p0 = cur ? a1_1_p0 : a1_0_p0;
...
// ═══════════ V6 split-K epilogue kernel ═══════════
// add-and-cast: C_out = C0 + C1 (all bf16). Simple element-wise kernel.
// Each thread handles 8 bf16 elements (one dwordx4 load per input).
// Grid: (M*N + ELEMS_PER_BLOCK - 1) / ELEMS_PER_BLOCK blocks of 256 threads.
struct epilogue_globals {
    _gl_bf16 c0, c1, c_out;
};

constexpr int EPI_THREADS = 256;
constexpr int EPI_ELEMS_PER_THREAD = 8;
constexpr int EPI_ELEMS_PER_BLOCK = EPI_THREADS * EPI_ELEMS_PER_THREAD;

__global__ __launch_bounds__(EPI_THREADS, 4)
void mxfp4_v6_epilogue_kernel(const epilogue_globals g) {
    const size_t total = static_cast<size_t>(g.c_out.rows()) * static_cast<size_t>(g.c_out.cols());
    const size_t base = (static_cast<size_t>(blockIdx.x) * EPI_ELEMS_PER_BLOCK)
                      + static_cast<size_t>(threadIdx.x) * EPI_ELEMS_PER_THREAD;
    if (base + EPI_ELEMS_PER_THREAD > total) {
        // Tail: scalar fallback
        for (size_t i = base; i < base + EPI_ELEMS_PER_THREAD && i < total; ++i) {
            float a = base_types::convertor<float, bf16>::convert(g.c0.raw_ptr[i]);
            float b = base_types::convertor<float, bf16>::convert(g.c1.raw_ptr[i]);
            g.c_out.raw_ptr[i] = base_types::convertor<bf16, float>::convert(a + b);
        }
        return;
    }
    // Vectorized fast path: 8 bf16 = 16 bytes = 1 dwordx4 load
    using vec16 = float4;
    vec16 v0 = *reinterpret_cast<const vec16*>(g.c0.raw_ptr + base);
    vec16 v1 = *reinterpret_cast<const vec16*>(g.c1.raw_ptr + base);
    bf16 out[8];
    const bf16* p0 = reinterpret_cast<const bf16*>(&v0);
    const bf16* p1 = reinterpret_cast<const bf16*>(&v1);
    #pragma unroll
    for (int i = 0; i < 8; ++i) {
        float a = base_types::convertor<float, bf16>::convert(p0[i]);
        float b = base_types::convertor<float, bf16>::convert(p1[i]);
        out[i] = base_types::convertor<bf16, float>::convert(a + b);
    }
    *reinterpret_cast<vec16*>(g.c_out.raw_ptr + base) = *reinterpret_cast<vec16*>(out);
}

void dispatch_epilogue_add(epilogue_globals g) {
    const size_t total = static_cast<size_t>(g.c_out.rows()) * static_cast<size_t>(g.c_out.cols());
    const size_t blocks = (total + EPI_ELEMS_PER_BLOCK - 1) / EPI_ELEMS_PER_BLOCK;
    mxfp4_v6_epilogue_kernel<<<dim3((unsigned int)blocks), dim3(EPI_THREADS), 0>>>(g);
}

PYBIND11_MODULE(tk_mxfp4_gluon_cpp, m) {
    m.doc() = "MXFP4 Gluon-arch kernel (V6 split-K POC)";
#if DIRECT_BL
    ...
    py::bind_function<dispatch_gluon_cpp>(m, "gemm_rcr",
        ...
        &gluon_globals::c);
#endif
    py::bind_function<dispatch_epilogue_add>(m, "epilogue_add_bf16",
        &epilogue_globals::c0, &epilogue_globals::c1, &epilogue_globals::c_out);
}
