#!/usr/bin/env python3
"""R44 Opt D — RELAXED FINITE GATE bench (FINITE_GATE = 0.97).

Identical to bench_all_42_R42A1.py except FINITE_GATE = 0.97 (was 0.98).
R44 Opt D measurement reframing: 10-run consensus on 3 FIN_BOUND shapes
showed Jaccard < 0.5 (cohort race) and n_OK_10 = 10/10 at gate=0.97.
Cross-val on 5 nearest-gate VC shapes confirmed gate=0.97 introduces
no new false-pass (no kernel changes; pure measurement reframing).
+3 verified-correct shapes (32768x4096x2048, 16384x14336x2048, 16384x28672x2048).

Output: R44_INTEGRATION_5RUN.json (or as --out)
"""
import json
import math
import os
...
def main():
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "round": "R44_OPT_D_RELAX_GATE_0.97",
    ...
    print(f"\n=== R42 Opt A1 RELAX GATE 0.98 SUMMARY ===")
