#!/usr/bin/env python3
"""R56 Opt G-3 / L6 — 128x512 falsification probe on (16384, 4096, 14336).

Reuses R50D shim AS-IS. Tile = 128x512 (alt eff=102.4 vs current 64x1024 eff=60.2).
Per-shape grid (bdx=256, gdz=1):
  gdx = ceil(N/512) = ceil(4096/512)   = 8
  gdy = ceil(M/128) = ceil(16384/128)  = 128
"""
import argparse
...
COMPETITOR_TFLOPS = 5142.1  # L6 baseline from bench_all_42.py
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G3_L6")
