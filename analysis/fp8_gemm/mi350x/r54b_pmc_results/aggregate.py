#!/usr/bin/env python3
"""R54B aggregator — aggregates rocprofv3 PMC outputs across 3 cells x 2 dtypes
x 2 PMC sets. Adapted from R53C/aggregate.py.

Cells: 70B_Down (4096x8192x28672), 8B_Down (4096x4096x14336), 70B_QO (4096x8192x8192)
Dtypes: fp8, mxfp8 (both RCR layout)
...
# Identify the kernel of interest in each dtype.
# MXFP8: rcr_exact_8wave_scaled_kernel<true, 2>  -- correctly labelled
# FP8 per-tensor: rcr_exact_8wave_kernel         -- correctly labelled at 8B_Down
# rocprofv3 quirk at 70B_Down/QO FP8: the same kernel is reported as
# `gemm_kernel<(Layout)0>`. We confirmed by timing (641us @ 2997 TFLOPS at
# 70B_Down = fastpath speed) and via x86 disasm of dispatch<L0>: the
# fastpath jump fires unconditionally for these shapes.
# rocprofv3 quirk at 8B_Down MXFP8: scaled fastpath labelled as
# `gemm_tail_kernel<(Layout)0, true>`. We accept both as long as the
# dispatch path is verified single-kernel.
KERNEL_TAGS = {
    "mxfp8": ["rcr_exact_8wave_scaled_kernel"],
    "fp8":   ["rcr_exact_8wave_kernel"],
}
# Hard duration ceiling (us) — anything slower than this is the fallback
# `gemm_tail_kernel` path which we explicitly do NOT want to compare
# against. Fastpath kernels at the 4096 x {4096,8192} x {8192,14336,28672}
# sweep run in 150-700 us. We use 5_000 us as a generous ceiling.
MAX_DURATION_US = 5_000.0
...
def load_cell(dirname, dtype):
    """Load all PMC csv files in a cell dir, filter by kernel tag."""
    by_disp = {}
    timing = {}
    knames = set()
            ...
            with open(csvp) as f:
                ...
                for row in rdr:
                    ...
                    if dur_us > MAX_DURATION_US:
                        continue  # drop fallback gemm_tail_kernel runs
...
def merge_sets(label, dtype):
    """Merge set1+set3 PMC counters into one combined dict."""
    merged = {}
    durations = []
    knames = set()
    for s in SETS:
        d = load_cell(f"{label}_{dtype}_rcr_{s}", dtype)
        if d is None:
            continue
        # skip sets where no fastpath kernel was captured
...
# === Print per-cell MXFP8 vs FP8 comparison table ===
def fmt(v):
...
print()
print("=" * 100)
print("=== SUMMARY MXFP8/FP8 ratio per cell ===")
print("=" * 100)
print(f"{'Cell':<14}{'K':>8}{'FP8 TFLOPS':>14}{'MX TFLOPS':>14}{'MX/FP8':>10}{'MfmaUtil FP8':>14}{'MfmaUtil MX':>14}{'L2hit FP8':>12}{'L2hit MX':>12}")
for label, M, N, K in CELLS:
    ...
    print(f"{label:<14}{K:>8}{fp8d['achieved_TFLOPS']:>14.1f}{mxd['achieved_TFLOPS']:>14.1f}{ratio:>9.1f}%{fp8d['MfmaUtil_pct']:>14.2f}{mxd['MfmaUtil_pct']:>14.2f}{fp8d['L2CacheHit_pct']:>12.2f}{mxd['L2CacheHit_pct']:>12.2f}")
