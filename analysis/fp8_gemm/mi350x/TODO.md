...
## Current State (2026-04-20, post-R61 — METHODOLOGY + MONITORING ROUND, **42/42 STRICT VC HELD (4TH CONSECUTIVE)**, **41/42 WIN RESTORED (+1 vs R60; L1 noise-edge crossed back to WIN at 100.15%; STRETCH TIER MET)**, **40/42 AITER BIT-DETERMINISTIC SHARE HELD (LARGEST EVER, 4TH CONSECUTIVE)**, **HK POOL 2 HELD (SMALLEST EVER, 4TH CONSECUTIVE)**, **13TH CONSECUTIVE R50D AS-IS REUSE**, 0 PROMOTE / 0 SMOKE_DEAD / 1 POLICY_ONLY (Opt U₂ pub outline) / 1 ARTIFACT (Opt Z per-shape decomp) / 1 MEASUREMENT_ONLY (Opt Y₂), 0 COHORT-RACE CHURN ON 42 UNCHANGED-BINARY CELLS (3RD CONSECUTIVE CLEAN-CHURN OUTCOME ON DISJOINT SEED SET), **OPT W STRENGTHENED PERMANENTLY DEPRIORITIZED (4-OF-4 OPT Y PASS ON L3)**, 6TH 100% LEADERBOARD ROUND IN PROJECT HISTORY (NON-CONSECUTIVE — R55, R56, R57, R59, R60, R61; INTERRUPTED ONLY BY R58))

**HEADLINE — R61 METHODOLOGY + MONITORING ROUND: 4-of-4 Opt Y PASS on L3 worst-margin HK survivor empirically tightens cohort-race tail-draw rate to ≤1/4 sweeps with high-confidence recovery — Opt W STRENGTHENED PERMANENTLY DEPRIORITIZED. L1 noise-edge crossed back to WIN at 100.15% restoring 41/42 WIN (Stretch tier MET). 5-round Opt R policy A oscillation envelope characterized.** R61 ran 3 worker cohorts (L-1 Opt Y₂ cohort-race surface monitoring on disjoint seed set [303..3030 step 303], L-2 Opt U₂ publication outline 602 lines, L-3 Opt Z per-shape decomposition 1091 lines). **0/3 PROMOTE / 1 POLICY_ONLY / 1 ARTIFACT / 1 MEASUREMENT_ONLY**. Manifest **byte-identical to R60 binary entries** (3rd consecutive binary-identical round; only metadata + axis_closures_R61 + policy_artifacts_R61 + cohort_race_validation_R61 + L1_noise_edge_oscillation_R61 fields added). Reviewer integration L-1 IS the bench (no separate re-bench step) on 4 GPUs (4-7), ITERS=500 default per Opt R policy A, INDEPENDENT seeds DISJOINT from R58/R59/R60, ~17 min wall, 420 runs: **42/42 strict VC HELD** (4th consecutive 42/42; 6th 100% leaderboard round); **41/42 WIN RESTORED** (R60 40/42; +1 from L1 noise-edge crossing back to WIN at 100.15% under same ITERS=500 protocol on UNCHANGED R57J1_L1 binary; STRETCH TIER MET); **40/40 AITER cells bit-deterministic** (wcf_max=0, wcf_std=0, fin_min=1.0; HELD); **2/2 HK cells PASS** (16384x4096x2048 R40B HOLD; 32768x14336x2048 R40B 100.55% **4TH CONSECUTIVE PASS** n_OK=10/10 fin_min=0.9883). 13th consecutive R50D shim AS-IS reuse round (no rebuild, no kernel modification, no new .co — 13-round stability claim).

**R61 mechanism findings**:
1. **L3 cohort-race surface classification STRENGTHENED via 4-of-4 Opt Y PASS on DISJOINT seed set (the central R61 question)**: 4-sweep history across R58 → R59 → R60 → R61 on UNCHANGED `(32768,14336,2048)` R40B HK 256×256 binary, with R61 using a 4th DISJOINT seed set [303..3030 step 303]. R58 PASS_9/10 fin_min=0.911 (lone tail-draw) → R59 PASS_10/10 fin_min=0.988 (RECOVERED) → R60 PASS_10/10 fin_min=0.985 (HELD on disjoint seeds) → R61 PASS_10/10 fin_min=**0.988** (HELD on 4th disjoint seeds). **Empirical claim TIGHTENED: L3 tail-draw probability ≤1/4 sweeps under ITERS=500 default; not intrinsic; no kernel rebuild justified. Opt W STRENGTHENED PERMANENTLY DEPRIORITIZED.**
2. **L1 noise-edge 5-round oscillation envelope characterized + WIN restored**: L1 (4096x32768x14336) R57J1_L1 AITER 256×256 across R57 → R58 → R59 → R60 → R61: 100.04% (ITERS=1000 WIN-edge) → 99.94% (ITERS=500 LOSE-edge) → 100.08% (ITERS=500 WIN) → 99.98% (ITERS=500 LOSE-edge) → **100.15% (ITERS=500 WIN)**. All 5 measurements bit-deterministic (wcf_max=0); envelope span 99.94% (R58) ↔ 100.15% (R61) = 0.21pp around WIN-line; alternating WIN-edge / LOSE-edge / WIN / LOSE-edge / WIN classification flip pattern. **Opt R policy A operationally validated for the 5th distinct measurement — full 5-round envelope characterized; per Opt R policy A no protocol intervention; 100.15% R61 reading is production answer; 41/42 WIN restored.**
3. **Cohort-race churn = 0 across 42 unchanged-binary cells under same ITERS=500 protocol on 4th DISJOINT seed sweep**: Mean perf drift -0.020pp/cell across 42 cells (range -1.17pp to +0.53pp); statistically indistinguishable from zero. R45+ ITERS=500 protocol stable across 4th independent seed sweep on UNCHANGED binaries — operational stability claim now spans 4 independent sweeps.
4. **Publication-prep artifacts delivered (~1700 lines combined)**: `R61_OPT_U2_PUBLICATION_OUTLINE.md` (602 lines, 5 sections covering pub outline + 11+ citation related-work skeleton + methods draft + 3 results tables + 9-subsection limitations) + `R61_OPT_Z_PER_SHAPE_DECOMPOSITION.md` (1091 lines, 42 H3 cell subsections + aggregate axis-closure summary + cells-by-cohort summary + closed-axis carry-forward + R43→R60 17-round arc digest). Both build on R60 Opt U doc pivot without duplication.
5. **R50D shim AS-IS reuse 13-round stability claim**: 13 consecutive rounds (R49 → R61) without shim rebuild, kernel modification, or new .co binary load. Operational reliability claim for SC/MICRO submission strengthened.

**R61 attempts summary** (0 PROMOTE; 0 SMOKE_DEAD; 1 POLICY_ONLY + 1 ARTIFACT + 1 MEASUREMENT_ONLY; 13th consecutive R50D AS-IS):
- **R61 Opt Y₂ — Cohort-race surface monitoring 4th sweep (worker L-1, GPUs 4-7; MEASUREMENT_ONLY PASS)**:
  - L3 (32768x14336x2048) R40B HK 256×256 on 4th disjoint seed set: **n_OK=10/10, wcf_max=0.0124, fin_min=0.9883, p50=100.55% — PASS** (4th consecutive successful sweep). Opt W STRENGTHENED PERMANENTLY DEPRIORITIZED.
  - All 42 cells: 42/42 strict VC HELD; 41/42 WIN RESTORED; 40/40 AITER bit-deterministic; 0 churn losses.
  - L1 noise-edge: 100.15% WIN (back from R60 99.98% LOSE-edge); +0.17pp seed-sweep drift within Opt R envelope; production reading per policy A.
- **R61 Opt U₂ — Publication outline (worker L-2, NO GPU; POLICY_ONLY)**:
  - `R61_OPT_U2_PUBLICATION_OUTLINE.md` (602 lines, 5 sections); 12-page SC25-style format; HPCA27/ISCA27/ASPLOS27 Q3 2026 deadline note flagged; 11+ citations across 7 topic clusters; 3 results tables (42-cell perf, R43→R60 18-round arc, R54→R60 AITER bit-det evolution); 9-subsection limitations.
- **R61 Opt Z — Per-shape decomposition appendix (worker L-3, NO GPU; ARTIFACT)**:
  - `R61_OPT_Z_PER_SHAPE_DECOMPOSITION.md` (1091 lines); 42 H3 cell subsections sorted by (M, N, K) with cohort/source/pct_comp/TFLOPS/wcf/fin_min/strict-VC/bit-det/tried-axes/closed-axes/why-best for each cell; 25-row aggregate axis-closure summary; cells-by-cohort summary; verbatim closed-axis carry-forward; R43→R60 17-round arc digest table.

**R61 reviewer integration (10-run @ 80%, DISJOINT INDEPENDENT seeds [303..3030 step 303] @ ITERS=500 default per Opt R policy A; 4 GPUs 4-7, ~17 min wall, 420 runs)**:
- Manifest: 40 AITER + 2 HK = 42 (**0 binary deltas vs R60** — byte-identical entries; 3rd consecutive binary-identical round)
- AITER cells: **40/40 PASS, all bit-deterministic** (4th consecutive at largest in project history)
- HK cells: **2/2 PASS** (L3 R40B 100.55% n_OK=10/10 fin_min=0.9883; 16384x4096x2048 HOLD WIN+VC)
- Cohort-race churn audit on 42 UNCHANGED-binary cells: **VC retention 42/42**; 0 lost VC; 0 gained VC; mean perf drift -0.020pp/cell
- WIN flip: 1 LOSE→WIN (L1 R57J1_L1 99.98% → 100.15%; per Opt R policy A oscillation); 0 WIN→LOSE
- **Final VC count: 42/42 strict 10-run (6th 100% leaderboard round in project history; non-consecutive — R55, R56, R57, R59, R60, R61; only R58 interrupted)**
- WIN cells (≥100% comp): 40 → **41 RESTORED** (+1 from L1 noise-edge crossback per Opt R policy A; STRETCH TIER MET)
- LOSE cells: 2 → **1** (only L8 4096x32768x128256 at 98.34% remains; structurally floored at aiter-internal ceiling)
- Files: `R61_INTEGRATION_VERDICT.md`, `R61_INTEGRATION_MANIFEST.json`, `bench_all_42_R61_INTEGRATION.py`, `R61_INTEGRATION_{10RUN,SMOKE1}.{json,log,console}`, `R61_DECIDER_PLAN.md`, `R61_OPT_U2_PUBLICATION_OUTLINE.md`, `R61_OPT_Z_PER_SHAPE_DECOMPOSITION.md`, `R61L1_Y2_INTEGRATION_FRAGMENT.json`

**R61 net result**: **4-of-4 Opt Y PASS on L3 (Opt W STRENGTHENED PERMANENTLY DEPRIORITIZED) + L1 noise-edge crossback to WIN restoring 41/42 WIN (STRETCH TIER MET) + 5-round Opt R policy A envelope characterized + Opt U₂ publication outline 602 lines + Opt Z per-shape decomposition 1091 lines (~1700 lines pub-prep artifacts) + 13th consecutive R50D AS-IS + 0 cohort churn + 42/42 strict VC HELD + AITER bit-det 40/40 4th consecutive HELD + HK pool 2 4th consecutive HELD + 6th 100% leaderboard round (non-consecutive)**.

### R61 LOSE/Attention cells remaining (1 hard LOSE structurally floored, 1 noise-edge oscillation cell now WIN)
- `(4096,32768,128256)` 98.34% (L8; R52D2B AITER 256×256; aiter alt-tile axis FULLY CLOSED + HK 256×256 axis CLOSED by correctness; only Opt T from-scratch HK build remains; 1.66pp from aiter-internal ceiling)
- `(4096,32768,14336)` 100.15% (L1; R57J1_L1 AITER 256×256; **currently WIN**; 5-round ±0.10pp oscillation around WIN-line under ITERS=500; alt-tile space EXHAUSTED; per Opt R policy A records per-sweep value without protocol intervention)
- L3 `(32768,14336,2048)` HK R40B is NOT attention cell after 4-of-4 PASS: 100.55% n_OK=10/10 fin_min=0.9883; per-sweep tail-draw rate ≤1/4 empirically tightened; Opt Y₃ optional every 2-3 rounds for monitoring.

### R62 candidates (post-R61, recommendation order)
1. **R62 Opt U₃ — Continued documentation pivot (RECOMMENDED ELECT)** — Methods-section deeper drafts (R50D shim flow diagram + Opt R policy A statistical justification + 10-run @ 80% gate analysis), Results section publication-quality charts (8 figure designs from R61 §1.4 figure list), Discussion section drafts. NO GPU, ~30-60 min/round.
2. **R62 Opt Z₂ — Per-shape decomposition table extensions (RECOMMENDED ELECT)** — Per-shape mechanism diagrams for the 3 attention cells (L1/L3/L8); per-cohort archetype analysis (K-bucket grouping); R50D shim invocation traces with kernel-launch invariants. NO GPU, ~30-60 min.
3. **R62 Opt Y₃ — 5th-consecutive cohort-race monitoring sweep (OPTIONAL ELECT)** — 5th sweep on R61 manifest with another DISJOINT seed set (e.g. [404, 808, 1212, 1616, 2020, 2424, 2828, 3232, 3636, 4040]). ~17 min wall. Every 2-3 rounds while no other work happening; builds longitudinal cohort-race dataset for publication appendix.
4. **R62 Opt T — L8 from-scratch HK kernel build for K=128256 (very low confidence, ~3 R-rounds, default DEFER)** — Port R39A TAIL_SCALE_CLAMP + R44A back-edge drain + R44D FINITE_GATE into new K=128256 HK build. Only path to closing L8 1.66pp gap. Defer unless explicit user election.

### R62+ axes NOT to attempt (R45-R61 closed)
- All R60 closure list plus:
- **Opt W — HK kernel rebuild with FINITE_GATE 0.97→0.95 STRENGTHENED PERMANENTLY DEPRIORITIZED** — R61 4-of-4 Opt Y PASS empirically tightens per-sweep tail-draw rate to ≤1/4 with high-confidence recovery; cost (break 13-round R50D AS-IS streak + ~1-2 R-rounds) not justified. 3rd-tier locked.
- **L1 ITERS=1000 one-off bump axis CLOSED by Opt R policy** — operationally validated across 5 sweeps R57/R58/R59/R60/R61.
- **L1 (4096x32768x14336) AITER alt-tile space EXHAUSTED** (256×256 R57J1_L1 best; oscillates ±0.10pp around WIN-line)
- **L3 (32768x14336x2048) AITER alt-tile space EXHAUSTED**
- **L8 HK 256×256 lgk2 v12 axis CLOSED by correctness** (R39A/R44A/R44D fixes never ported into K=128256 build)
- **L8 AITER alt-tile space CLOSED** (128×512, 192×256, 224×256, 96×640, 64×1024 all DEAD)

---

## Previous State (2026-04-20, post-R60 — METHODOLOGY ROUND, **42/42 STRICT VC HELD (3RD CONSECUTIVE)**, **40/42 WIN (-1 vs R59; predicted L1 noise-edge oscillation per Opt R policy A)**, **40/42 AITER BIT-DETERMINISTIC SHARE HELD (LARGEST EVER, 3RD CONSECUTIVE)**, **HK POOL 2 HELD (SMALLEST EVER, 3RD CONSECUTIVE)**, **12TH CONSECUTIVE R50D AS-IS REUSE**, 0 PROMOTE / 0 SMOKE_DEAD / 1 POLICY_ONLY (Opt U doc pivot) + 1 MEASUREMENT-ONLY (Opt Y), 0 COHORT-RACE CHURN ON 42 UNCHANGED-BINARY CELLS, **OPT W PERMANENTLY DEPRIORITIZED (3RD-CONSECUTIVE OPT Y PASS ON L3)**, 5TH 100% LEADERBOARD ROUND IN PROJECT HISTORY (NON-CONSECUTIVE — R55, R56, R57, R59, R60; INTERRUPTED BY R58))
...
**R58 mechanism findings**:
1. **P-2 mechanism (Opt P PROMOTE)**: HK R40B 256×256 cohort-race intensity is meaningful at (M=16384, N=4096, K=3072) but aiter 128×256 (eff=85.3) holds end-to-end perf — single swap delivers +4.37pp perf AND wcf_max 0.0134→0 AND fin_min 0.9955→1.0. **PROMOTE found in axis the decider rated "low confidence"** — supports principle that bounded-cost axis-closure probes are worth running.
2. **HK 256×256 axis on L8 K=128256 CLOSED by correctness (Opt O)**: BOTH R40B (build_R40B/) and R37 (build_R37/) HK candidates produce WRONG_OUTPUT (fin=0.78-0.80, wcf=0.07-0.13). The R39A TAIL_SCALE_CLAMP / R44A back-edge drain / R44D FINITE_GATE fixes were NEVER ported into the K=128256 build — explains why L8 is structurally stuck. R59+ cannot use existing HK binaries; would require a from-scratch K=128256 HK build with all post-R39 correctness fixes ported in.
3. **128×256 axis closed for 2 of 3 HK cells (Opt P P-1, P-3)**: P-1 (M=16384, N=4096, K=2048) -3.68pp; P-3 (M=32768, N=14336, K=2048) -25.04pp catastrophic. Combined with R57 H-2 192×256 closure, the alt-tile space for HK kept-cell pool is largely exhausted for K=2048.
4. **Opt N gate tightening informational delivered (analysis-only)**: 40/42 pass tightened gate (n_OK≥9/10, wcf_max<0.01, fin_min≥0.99); 2 droppers are HK survivors `16384x4096x3072` (now eliminated by P-2 PROMOTE) and `32768x14336x2048` (the lone R58 VC-flipper). Recommendation: KEEP R45+ default as canonical; carry Opt N as informational secondary gate. Post-R58 Opt N attention list = 1 cell.
5. **ITERS=500 revert effects predicted by R57 verdict**: L1 (R57J1) 100.04% → 99.94% (LOSE-edge re-emergence; predicted in R57 verdict §"ITERS=1000 protocol observation"); HK survivors' wider trim distribution shifts mean -1 to -2pp (16384x4096x2048 109.57→107.48%); fin_min on `32768x14336x2048` 0.9847 → 0.911 (under 0.97 gate; produces VC flip). **L3 VC drop is COHORT-RACE TAIL-DRAW on UNCHANGED binary, NOT a kernel regression.** REVERT was considered but does NOT apply — P-2 swap candidate strictly passes; L3 is governed by ITERS revert on a different (unchanged) cell.

**R58 attempts summary** (1 PROMOTE; 4 ACCEPT_FALLBACK; 10th consecutive R50D AS-IS):
- **R58 Opt N — Gate tightening pilot, analysis-only (worker I-1, NO GPU)**:
  - Re-classify R57_INTEGRATION_10RUN.json under (n_OK≥9, wcf_max<0.01, fin_min≥0.99): 40/42 pass; 2 HK survivors drop on wcf_max tail (`16384x4096x3072`, `32768x14336x2048`); 0 AITER drops (perfect bit-determinism). **Recommendation: do NOT adopt; keep informational.**
- **R58 Opt O — L8 HK kernel probe (worker I-2, GPUs 0-1, 0/2 PROMOTE 2 ACCEPT_FALLBACK)**:
  ...
  - O-1b same shape HK R37 256×256 memc fallback: WRONG_OUTPUT fin=0.775 wcf=0.066 SNR=31.79dB. **ACCEPT_FALLBACK**. **HK 256×256 axis on L8 K=128256 CLOSED by correctness**.
- **R58 Opt P — 128×256 alt-tile probe on 3 HK survivors (worker I-3, GPUs 2-3, 1/3 PROMOTE 2 ACCEPT_FALLBACK)**:
  ...
  - P-3 `(32768,14336,2048)`: HK 100.53% vs 128×256 SMOKE 75.49% (-25.04pp catastrophic). **ACCEPT_FALLBACK (STOP_DEAD)**.

The 1 PROMOTE worker AS-IS reuses **EXISTING R50D shim** at `build_R50D/R50D_aiter_shim.cpython-310-x86_64-linux-gnu.so` with new `tile_M=128, tile_N=256` kwargs + `f4gemm_bf16_per1x32Fp4_BpreShuffle_128x256.co`. NO shim rebuild. Kernel UNCHANGED.

**R58 reviewer integration (10-run @ 80%, INDEPENDENT seeds [101..1010] @ ITERS=500 DEFAULT REVERTED; 4 GPUs 4-7, ~17 min wall, 420 runs)**:
- Manifest: 40 AITER + 2 HK = 42 (1 binary delta vs R57: P-2 swap; only swap, no other manifest changes).
- All 1 R58 PROMOTE candidate RE-VERIFIED 10/10 PASS at reviewer with perfect bit-determinism (wcf_max=0, wcf_std=0, fin_min=1.0).
- AITER cells: **40/40 PASS (100% bit-deterministic)** — largest AITER bit-det share in project history.
- HK cells: **1/2 PASS** (`16384x4096x2048` HOLD VC+WIN at 107.48%; `32768x14336x2048` VC-flipped at n_OK=9/10 fin_min=0.911 but numerically WIN at 100.49%).
- Cohort-race churn audit on 41 UNCHANGED-binary cells: **VC retention 40/41**; mean perf drift +0.001pp on 39 unchanged AITER cells (range -1.09 to +1.15pp), -1.06pp on 2 unchanged HK cells (predicted ITERS=500 wider-trim effect).
- WIN flip cells under ITERS=500 revert: 1 (L1 R57J1 100.04% → 99.94% LOSE-edge; predicted in R57 verdict).
- **Final VC count: 41/42 strict 10-run (3-in-a-row 100% leaderboard streak ENDED at R57; structural wins compound)**.
- WIN cells (>=100% comp): 41 → 40 (-1; L1 LOSE-edge re-emergence; P-2 +1 cancels).
- LOSE cells: 1 → 2 (L1 99.94%, L8 98.28%).
- Files: `R58_INTEGRATION_VERDICT.md`, `R58_INTEGRATION_MANIFEST.json`, `bench_all_42_R58_INTEGRATION.py`, `R58_INTEGRATION_{10RUN,SMOKE1}.{json,log,console}`, `R58_DECIDER_PLAN.md`.

**R58 net result**: **+1 PROMOTE structural swap + AITER bit-det 39→40 (largest ever) + HK pool 3→2 (smallest ever) + 10th consecutive R50D AS-IS + Opt N informational delivered + 4 closed axes documented**, in exchange for: 3-in-a-row 100% streak ends at 41/42 VC + 40/42 WIN under ITERS=500 default. Cohort-race attention surface PERMANENTLY shrinks 3 cells → 1 cell. Net structural progress trumps the streak break.

### R58 LOSE/VC-flipped cells remaining (3 cells of attention; 1 hard LOSE, 1 ITERS-edge LOSE, 1 VC-flipped)
- `(4096,32768,128256)` 98.28% (L8; R52D2B AITER 256×256; aiter alt-tile axis FULLY CLOSED + HK 256×256 axis CLOSED by correctness; no remaining mechanism axis without from-scratch HK build)
- `(4096,32768,14336)` 99.94% (L1; R57J1 AITER 256×256; ITERS=500 revert pushed below WIN-line; was 100.04% under R57 ITERS=1000)
- `(32768,14336,2048)` 100.49% (HK R40B; numerically WIN but VC-flipped n_OK=9/10 fin_min=0.911 under ITERS=500; previously flagged as worst-margin survivor by R57 reviewer fin_min=0.9847 and R58 Opt N)

### R59 candidates (post-R58, ordered by mechanism-confidence)
1. **R59 Opt R — Mixed-protocol leaderboard policy decision (highest methodology value, lowest perf gain)** — Decide: do we permanently bump ITERS=1000 (cost: 2× wall per round; benefit: L1 +1 WIN cell stable + HK survivors +1.5pp)? OR maintain ITERS=500 default with documented L1 noise-edge LOSE? OR mixed-protocol L1-only ITERS=1000 (cost: leaderboard inconsistency)? **Recommend ITERS=500 default + documented L1 footnote** (current R58 disposition).
2. **R59 Opt S — `(32768,14336,2048)` HK survivor rescue (medium confidence)** — VC-flipped under R58. Try non-128×256 alt-tile (96×640 eff=83.5, 64×1024 eff=60.2) — both lower-eff than 128×256 which scored 75.49% on this cell. Very low confidence. OR try kernel-rebuild with R44D FINITE_GATE 0.97 → 0.95 (cohort-protocol relax). Estimated +0 to +1 NET VC.
3. **R59 Opt T — L8 from-scratch HK kernel build for K=128256 (very low confidence, high cost)** — Port R39A TAIL_SCALE_CLAMP + R44A back-edge drain + R44D FINITE_GATE into a new K=128256 HK build. Estimated cost: ~3 R-rounds; very low WIN probability. Only attempt if R59 explicitly elects build-campaign budget.
4. **R59 Opt U — Accept residual + documentation pivot** — Acknowledge L8 at aiter+HK structural floor; freeze manifest at R58 baseline; pivot R59-R65 to systematic per-shape decompositions for the SC/MICRO publication.

### R59+ axes to NOT attempt (closed by R45-R58)
- All R57 closed list PLUS:
- **HK 256×256 lgk2 v12 R40B/R37 fallbacks for L8 K=128256** — both produce WRONG_OUTPUT (R58 Opt O); needs a from-scratch K=128256 HK build with R39A/R44A/R44D fixes ported.
- **128×256 alt-tile for 16384x4096x2048 HK survivor and 32768x14336x2048 HK survivor** — R58 Opt P confirmed -3.68pp and -25.04pp respectively; only the (16384,4096,3072) cell PROMOTEs.
- ANY further attempt to ITERS=1000 the L1 noise-edge cell as a one-off — R57 demonstrated the mixed-protocol cost is not worth the single-cell flip.

### R58 stopping-criterion check
- Floor (Opt N report delivered + 0 PROMOTEs + 42/42 VC HELD): **PARTIAL — Opt N delivered; 1 PROMOTE; 41/42 VC (1 cohort-race tail-draw on UNCHANGED L3 binary)**.
- Stretch (1 PROMOTE → 42/42 WIN): **NOT MET (L1 reverts under ITERS=500; net 40/42 WIN)**.
- Round value: **+1 structural PROMOTE (HK→AITER permanent) + +1 AITER bit-det (largest ever) + -1 HK pool (smallest ever) + -2 cohort-race attention cells permanently + Opt N informational + 4 closed axes documented + 10th consecutive R50D AS-IS reuse**. Trade-off: 3-in-a-row 100% streak ends; net VC -1 strict; net WIN -1.

### Round sequence sanity check (last 16 rounds)
- R43: DEAD (3 axes)
- R44: WIN +8 (27 → 35/42)
- R45-R49: 5 DEAD rounds in a row
- R50: WIN +1 (35 → 36/42, aiter `.co` dlopen first PoC)
...
- R55: WIN +6 NET VC -0 cohort +6 strict (36 → 42/42) + 5 perf claw-backs +1.81-20.99pp; FIRST 100% LEADERBOARD ROUND IN PROJECT HISTORY
- R56: PERF-CLAW-BACK WIN +6 NET WIN CELLS (34→40 LARGEST EVER) -0 cohort +0 strict VC (42/42 HELD) + 7 perf claw-backs +14-81pp; 39/42 cells bit-deterministic AITER; 7/7 PROMOTE / 1 ACCEPT_FALLBACK / 0 DEAD; 8th CONSECUTIVE R50D AS-IS REUSE; 2ND CONSECUTIVE 100% LEADERBOARD ROUND
- R57: WIN +1 NET WIN CELL via Opt J ITERS=1000 (40→41) -0 cohort +0 strict VC (42/42 HELD) + 0 perf claw-backs (4 ACCEPT_FALLBACK closures); 39/42 cells bit-deterministic AITER; 1/5 PROMOTE / 4 ACCEPT_FALLBACK / 0 DEAD; 9th CONSECUTIVE R50D AS-IS REUSE; 3RD CONSECUTIVE 100% LEADERBOARD ROUND
- **R58: STRUCTURAL WIN +1 PROMOTE P-2 HK→AITER 128×256 swap +4.37pp -1 cohort tail-draw on UNCHANGED L3 (predicted by R57 fin_min=0.9847 + R58 Opt N) + ITERS=500 revert net -1 strict VC (42→41) -1 WIN (41→40); 40/42 cells bit-deterministic AITER (LARGEST EVER); HK pool 3→2 (SMALLEST EVER); 1/5 PROMOTE / 4 ACCEPT_FALLBACK / 0 DEAD; 10th CONSECUTIVE R50D AS-IS REUSE; 3-IN-A-ROW STREAK ENDS** ← NET STRUCTURAL PROGRESS via single HK→AITER swap; cohort-race surface PERMANENTLY 3→1 cell
...
**R56 Opt G mechanism validation**: aiter heuristic under-picked 256×256 on the 6 cluster-A/B 64×1024 LOSE cells where `local_round` count was IDENTICAL but `compute2mem_efficiency` differed (60.2 vs 128.0). G-1 256×256 wins on every cell vs G-3 128×512 falsification (eff=102.4) — confirms BOTH mechanism axes operate (wider tile escape AND eff dominance), with 256×256 strictly best. G-4 C1 (L7 4096x128256x32768) HK R41A (97.59%) → AITER 256×256 (178.51%) = +80.92pp = first successful HK→AITER swap on a kept-HK cell. G-4 C2/C3 (L8 4096x32768x128256, current 256×256 98.32%) alt-tiles 128×512 and 192×256 returned 85.01% / 86.48% — D-3A-1 protection correctly held R52D2B baseline.

**R56 attempts summary** (all 7 PROMOTEs reuse R50D shim AS-IS with aiter `.co` 256×256):
- **R56 Opt G-1 — 64×1024→256×256 cluster A (worker, 3/3 PROMOTE +76.11pp aggregate worker)**:
...
- **R56 Opt G-2 — 64×1024→256×256 cluster B (worker, 3/3 PROMOTE +43.93pp aggregate worker)**:
...
- **R56 Opt G-3 — 128×512 falsification probe (worker, 3/3 PROMOTE +64.28pp but DROPPED from manifest; G-1 wins)**:
  - L1 128×512 92.48% (vs G-1 256×256 100.63%) — DROPPED.
  - L2 128×512 104.70% (vs G-1 256×256 105.37%) — DROPPED.
  - L6 128×512 107.19% (vs G-1 256×256 110.20%) — DROPPED.
  - Mechanism: 256×256 wins on every cell; eff dominance confirmed.
- **R56 Opt G-4 — HK→AITER + alt-tile (worker, 1/3 PROMOTE 2 ACCEPT_FALLBACK)**:
  - C1 L7 `(4096,128256,32768)` HK R41A → AITER 256×256: 97.59% → 178.51% **+80.92pp**. **First HK→AITER swap on kept-HK cell**.
...
**R56 reviewer integration (10-run @ 80%, INDEPENDENT seeds [101..1010]; 4 GPUs 4-7, ~16 min wall, 420 runs)**:
- Manifest has **39 aiter `.co` overrides** (R55's 38 + L7 HK→AITER) + **3 HK** baselines (R55 4 minus L7).
- All 7 R56 PROMOTE candidates RE-VERIFIED 10/10 PASS at reviewer with bit-determinism (wcf_max=0, wcf_std=0, fin_min=1.0).
- AITER cells: **39/39 PASS (100% bit-deterministic)**.
- HK cells: **3/3 PASS (zero churn; all 3 surviving HK cells re-verified PASS_10/10)**.
- Cohort-race churn audit on 35 unchanged shapes: **0 lost VC; mean perf drift -0.06pp AITER, -0.02pp HK**.
- Net cohort churn: **0 / 0 = 0** on UNCHANGED .so cells.
- R56 contribution: **+6 NET WIN cells** (34 → 40) + 0 cohort losses + +198.14pp aggregate perf claw-back.
- **Final VC count: 42/42 strict 10-run HELD (2nd consecutive 100% leaderboard round)**.
- WIN cells (>=100% comp): 34 → 40 (+6, largest single-round WIN-cell jump).
- Files: `R56_INTEGRATION_VERDICT.md`, `R56_INTEGRATION_MANIFEST.json`, `bench_all_42_R56_INTEGRATION.py`, `R56_INTEGRATION_{10RUN,SMOKE1}.{json,log,console}`, `R56_DECIDER_PLAN.md`.

**R56 net result**: **7 PROMOTEs + +198.14pp AGGREGATE PERF CLAW-BACK (2.2× STRETCH)**, R56 brings ZERO regressions and ZERO cohort-race losses. Net strict VC: **42/42 HELD (2nd consecutive 100% LEADERBOARD)**. WIN cells 34→40 (+6, largest single-round jump in project history). Cohort-race surface 4→3 HK cells (smallest ever). **Zero kernel modification, zero new shim build (8th consecutive R50D reuse).**

### R56 LOSE cells remaining (only 2; both within 1.7pp of WIN combined)
- `(4096,32768,14336)` 99.98% (L1; reviewer-drift edge; worker reported 100.63% — within 0.6pp drift, effectively at WIN threshold)
- `(4096,32768,128256)` 98.30% (L8; D-3A-1 ACCEPT_FALLBACK; alt-tiles 128×512 and 192×256 both DEAD by 11-13pp)

### R57 candidates (post-R56, ordered by mechanism-confidence)
1. **R57 Opt J — Re-bench L1 with longer ITERS or alt seeds (highest confidence, lowest value)** — L1 4096x32768x14336 currently at 99.98% reviewer (worker 100.63%); 0.02pp below WIN line is noise-level. Re-bench with ITERS=1000 (vs default 500) to tighten p50 distribution and confirm WIN crossing. Expected: +1 WIN cell (40→41) but no perf gain.
2. **R57 Opt K — Probe remaining un-attempted aiter tiles for L8 (medium-low)** — L8 4096x32768x128256 at 98.30%. Remaining aiter `.co` files un-probed for K=128256: 96×640, 64×1024 (both lower-eff; very low confidence). Maybe 192×128, 128×384, 160×256/384 if they exist. Estimated +0 to +1 WIN cell.
3. **R57 Opt L — Kept-HK-cell hardening pivot (low-medium)** — 3 HK cells remain. The remaining 3 HK cells re-passed cleanly at R56; HK 256×256 path produces 100.39%-105.36% on these. R55 D-5B/1 already showed 256×256 AITER underperforms on N=14336. Try non-256×256 alt-tiles (e.g., 128×256, 192×256) for these 3 HK cells. Estimated +0 NET WIN + bit-determinism gain on up to 3 cells.
4. **R57 Opt M — Gate tightening pilot (medium, deferred from R56)** — VC ceiling reached; tighten gate (n_OK→9/10, wcf_max→0.01, fin_min→0.99) to expose remaining sub-optimal cells. Test-design improvement.

### R57+ axes to NOT attempt (closed by R45-R56)
- All R55 closed list PLUS:
- **128×512 and 192×256 on L8 (4096x32768x128256)** — R56 G-4 confirmed 11-13pp WORSE than current 256×256.
- ANY further attempt at the G-3 128×512 cluster A/B cells (L1, L2, L6) — DROPPED by reviewer; G-1 256×256 strictly better.
- Any "improvement" of HK 256×256 path or R50D aiter `.co` dlopen path itself.

### R56 stopping-criterion check
- Floor (≥3 PROMOTE, +25pp aggregate): **EXCEEDED (7 PROMOTE, +198pp)**.
- Mode (≥5 PROMOTE, +60pp): **EXCEEDED (7 PROMOTE, +198pp)**.
- Stretch (≥7 PROMOTE, +90pp): **MET (7 PROMOTE) and EXCEEDED on aggregate (+198pp = 2.2× target)**.
- Round value: **+6 NET WIN cells + +198.14pp aggregate + 5 durable findings** (Opt G mechanism validated; HK→AITER swap successful on L7; D-3A-1 protection load-bearing on L8; 8th consecutive R50D AS-IS reuse; cohort-race surface at smallest ever 3 HK cells).

### Round sequence sanity check (last 14 rounds)
- R43: DEAD (3 axes)
- R44: WIN +8 (27 → 35/42)
- R45-R49: 5 DEAD rounds in a row
- R50: WIN +1 (35 → 36/42, aiter `.co` dlopen first PoC)
...
- R53: PARTIAL WIN +2 NET VC rescues -5 cohort net -3 strict (36 → 33/42) + 6 perf claw-backs
- R54: WIN +6 NET VC rescues -3 cohort net +3 strict (33 → 36/42) + 6 perf claw-backs +17-28pp
- R55: WIN +6 NET VC -0 cohort +6 strict (36 → 42/42) + 5 perf claw-backs +1.81-20.99pp; 38/42 cells bit-deterministic AITER; 11/12 PROMOTE / 1 ACCEPT_FALLBACK; FIRST 100% LEADERBOARD ROUND IN PROJECT HISTORY
- **R56: PERF-CLAW-BACK WIN +6 NET WIN CELLS (34→40 LARGEST EVER) -0 cohort +0 strict VC (42/42 HELD) + 7 perf claw-backs +14-81pp; 39/42 cells bit-deterministic AITER; 7/7 PROMOTE / 1 ACCEPT_FALLBACK / 0 DEAD; 8th CONSECUTIVE R50D AS-IS REUSE; 2ND CONSECUTIVE 100% LEADERBOARD ROUND** ← LARGEST WIN-CELL JUMP IN PROJECT HISTORY

---

## Previous State (2026-04-19, post-R55 — WIN +6 NET VC STRICT = **42/42 FIRST 100% LEADERBOARD ROUND IN PROJECT HISTORY**, COMMIT, 11/12 PROMOTE / 0 DEAD / 1 ACCEPT_FALLBACK ACROSS 4 COHORTS, AITER BIT-DETERMINISTIC SHARE 27→38 OF 42, 7TH CONSECUTIVE R50D AS-IS REUSE, COHORT-RACE SURFACE CUT FROM 15 HK CELLS TO 4)
