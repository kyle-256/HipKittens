...
if K % 2 != 0 or build_K % 2 != 0:
    raise ValueError(
        f"K and build_K must be even for FP4 packing (got K={K}, build_K={build_K})"
    )

num_warmup = int(os.environ.get("MXFP4_WARMUP", "10"))
num_iters = int(os.environ.get("MXFP4_ITERS", "20"))
...
use_preshuffle_quant = os.environ.get("MXFP4_PRESHUFFLE_QUANT", "0") != "0"
requested_layouts = {
    layout.strip().lower()
    for layout in os.environ.get("MXFP4_LAYOUTS", "rcr").split(",")
    if layout.strip()
}
supported_layouts = {"rcr"}
unsupported = requested_layouts - supported_layouts
if unsupported:
    print(
        f"Note: MXFP4 harness currently supports only RCR; ignoring: {sorted(unsupported)}",
        file=sys.stderr,
    )
requested_layouts &= supported_layouts
if not requested_layouts:
    requested_layouts = {"rcr"}

check_results = os.environ.get("MXFP4_CHECK", "1") != "0"
...
def default_output_path():
    ...
    if M == N == K:
        return f"mxfp4_layout_results_{M}{suffix}.json"
    return f"mxfp4_layout_results_{M}x{N}x{K}{suffix}.json"


def generate_fp4_matrix(rows, cols_fp4):
    """Random FP4 indices packed as uint8: shape rows × (cols_fp4 // 2)."""
    assert cols_fp4 % 2 == 0
    cols_bytes = cols_fp4 // 2
    low_nibbles = torch.randint(0, 16, (rows, cols_bytes), dtype=torch.uint8, device="cuda")
    high_nibbles = torch.randint(0, 16, (rows, cols_bytes), dtype=torch.uint8, device="cuda")
    packed = (high_nibbles << 4) | low_nibbles
    return packed


def unpack_fp4_to_float(packed, K):
    """Unpack M×(K//2) uint8 tensor to M×K float32 tensor."""
    rows = packed.shape[0]
    low = (packed & 0x0F).to(torch.int64)
    high = ((packed >> 4) & 0x0F).to(torch.int64)
    lut = FP4_LUT.to(packed.device)
    result = torch.empty(rows, K, dtype=torch.float32, device=packed.device)
    result[:, 0::2] = lut[low]
    result[:, 1::2] = lut[high]
    return result
...
print(
    f"=== MXFP4 GEMM Layout Benchmark ({'preshuffle-quant' if use_preshuffle_quant else 'reference scale layout'}): "
    f"M={M}, N={N}, K={K} ==="
)
...
if "rcr" in requested_layouts:
    ...
    if use_preshuffle_quant:
        ...
        run = lambda: tk_mxfp4_layouts.gemm_rcr_pq(A, B, A_scale, B_scale, C)
    else:
        A_scale = A_scale_exp
        B_scale = B_scale_exp
        run = lambda: tk_mxfp4_layouts.gemm_rcr(A, B, A_scale, B_scale, C)
    C = torch.zeros(build_M, build_N, dtype=torch.bfloat16, device="cuda")
    timings = benchmark_kernel(run, C)
    A_ref = unpack_fp4_to_float(A[:M, : K // 2], K) * expand_row_scales(A_scale_exp[:M, :k_blocks], K)
    B_ref = unpack_fp4_to_float(B[:N, : K // 2], K) * expand_row_scales(B_scale_exp[:N, :k_blocks], K)
    C_ref = A_ref @ B_ref.T
    record_result("rcr", timings, C, C_ref, run)

outfile = os.environ.get("MXFP4_OUTPUT", default_output_path())
