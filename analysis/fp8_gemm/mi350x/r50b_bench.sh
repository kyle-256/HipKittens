#!/bin/bash
# R50 Dev B: MXFP8_RRR_MAIN_UNROLL per-shape sweep at 8B Gate/Up RRR
# Phase 1: U ∈ {0(baseline), 1, 2, 4, 8} at 4096x14336x4096 RRR
#  - 5 runs per cell, 30s cooldown between runs, 60s rebuild cooldown
#  - GPU 2 isolated, MXFP8_PRESHUFFLE_QUANT=1
#  - Capture build remarks (VGPRs, AGPRs, ScratchSize, Spill, Occupancy)
# Phase 2 (only if Phase 1 finds candidate): U on other 6 RRR shapes
set -euo pipefail
cd "$(dirname "$0")"
export THUNDERKITTENS_ROOT=/shared_nfs/kyle/test/Hipkittens2/.claude/worktrees/agent-a2ed4599
...
# Shape list. First entry is the Phase 1 target (8B Gate/Up RRR).
declare -a SHAPES=(
...
# U values to sweep in Phase 1
declare -a US_PHASE1=(0 1 2 4 8)
...
if [ "$PHASE" = "1" ]; then
    echo "######################################################"
    echo "## R50 Dev B PHASE 1: 8B Gate/Up U-sweep            ##"
    echo "######################################################"
...
if [ "$PHASE" = "2" ]; then
    ...
    CAND_U=${CAND_U:-2}
    echo "######################################################"
    echo "## R50 Dev B PHASE 2: candidate U=${CAND_U} on other 6 RRR shapes ##"
    echo "######################################################"
    for i in "${!SHAPES[@]}"; do
        ...
        echo "  corr/det checked"
        # Also rebuild + bench at U=0 baseline for direct comparison
        build_and_capture $M $N $K $L 0
        run_bench $M $N $K $L 0
        echo "  baseline U=0 benched"
