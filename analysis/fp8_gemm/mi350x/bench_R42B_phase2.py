...
ALL_CELLS = ["baseline","no_fused","no_btw","gm1","no_lgk2","no_tailsplit","no_r25c"]
...
def find_so(m, n, k, cell):
    ...
    mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R42Bp2_{cell}"
...
def bench_one_shape(m, n, k, comp, cell, gpu_id):
    ...
    if not so_path:
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                ...
                "best_variant": f"R42Bp2_{cell}", "cell": cell}
    ...
    try:
        ...
        out["best_variant"] = f"R42Bp2_{cell}"
        out["cell"] = cell
        return out
