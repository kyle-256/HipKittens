#!/usr/bin/env python3
"""R52 Opt D-2A — bench harness for the aiter `.co` dlopen shim on (4096, 28672, 32768).

Reuses R50D shim AS-IS (256x256 tile). Only shape constants change.
Cloned from bench_R51D1.py.
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 28672, 32768
COMPETITOR_TFLOPS = 5649.9  # from bench_all_42.py for (4096, 28672, 32768)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2A_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2A_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2A_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2A_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R52 Opt D-2A bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
