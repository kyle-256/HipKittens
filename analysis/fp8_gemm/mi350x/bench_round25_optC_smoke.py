#!/usr/bin/env python3
"""R25C smoke bench (warmup=200 iters=500 trim=10%) — K-loop tail PF-off epilogue.

Per benchmark-rules.md MANDATORY: warmup=200, iters=500, 10% trim.
GPUs 4, 6, 7 (R25-A uses 0-1, R25-B uses 2-3, GPU 5 may be busy).
"""
...
def main():
    ...
    print(f"R25C smoke: {len(args_list)} bench jobs (3 shapes x 5 variants), GPUs {GPU_POOL}")
    ...
    with open(os.path.join(SCRIPT_DIR, "bench_round25_optC_smoke.json"), "w") as f:
        json.dump(out, f, indent=2)

    # Summary: deltas vs each shape's _r25c_baseline
    print("\n" + "=" * 110)
    print("SUMMARY (Δ vs _r25c_baseline, +1.5pp gate)")
    print("=" * 110)
    for (lab, *_rest) in SHAPES:
        b = out["shapes"][lab].get("_r25c_baseline", {})
        bt = b.get("tflops")
        if bt is None:
