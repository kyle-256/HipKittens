#!/usr/bin/env python3
"""R54 Opt D-4B/2 — bench harness for the aiter `.co` dlopen shim on (4096, 14336, 8192).

Reuses R50D shim AS-IS (bdx=256). Tile = 256x256 (aiter heuristic pick).
Marginal HK-VC claw-back candidate (HK 91.92%).
...
Per-shape grid (per task spec):
  gdx = ceil(N/256) = 56, gdy = ceil(M/256) = 16, gdz = 1, bdx = 256
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 14336, 8192
COMPETITOR_TFLOPS = 4345.8  # from bench_all_42.py for (4096, 14336, 8192)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_D4B_2")
    ...
    if args.mode == "smoke":
        ...
        seeds = args.seeds if args.seeds else [101]
        do_perf = True
    else:
        ...
        seeds = args.seeds if args.seeds else [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False

    results = []
    log_lines = []
    log_lines.append(f"R54 Opt D-4B/2 bench — mode={args.mode}, gpu={gpu_list}, "
