#!/bin/bash
# R39 Dev B: 4-GPU STRICT-promotion of R34/R35 Dev A 8B Gate V2-RRR autotune predicate
# Shape: 8B Gate/Up (M=4096, N=14336, K=4096)
# Predicate at kernel_mxfp8_layouts.cpp:5712-5720 (warned_8b_gateup advisory)
# R34 Dev B SHIP @ +5.025% min t=10.13 (4-GPU). R36 Reviewer Phase 2 SHIP-LITE: GPU4 +6.96% t=+6.83; GPU5 +5.05% t=+6.28.
# R39 task: re-bench with N_PAIRS=15 (3x sample count) to clear STRICT min t > 10 gate.
# This is the same pattern Dev C closed for 8B-Down V2-RRR in R38 (N=5 -> N=15 -> STRICT).
#
...
#
# Usage:
#   PHYS_GPU=2 ./r39b_8bgate_orchestrate.sh
#   PHYS_GPU=3 ./r39b_8bgate_orchestrate.sh
#   PHYS_GPU=6 ./r39b_8bgate_orchestrate.sh
#   PHYS_GPU=7 ./r39b_8bgate_orchestrate.sh
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
WT="$(cd "$HERE/../../.." && pwd)"
cd "$HERE"

OUTDIR="$HERE/r39b_8bgate_runs"
mkdir -p "$OUTDIR"
...
echo "===== R39 Dev B 8B-Gate 4-GPU STRICT-promote cell=$CELL M=$M N=$N K=$K phys_gpu=$PHYS_GPU =====" | tee "$OUT"
