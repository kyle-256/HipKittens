#!/usr/bin/env python3
"""R32-OptA-Build: K_LOOP_SYNC_EVERY_2 and NONTEMPORAL load axes on L6.

Parent: ts_lgk2_v12_memc_btw_all (incumbent best at 5354 TFLOPS, 92.6% of comp 5781).

3 variants on L6 (4096x32768x128256):
  V1: +K_LOOP_SYNC_EVERY_2=1
  V2: +B_LOAD_NONTEMPORAL=1 +A_LOAD_NONTEMPORAL=1
  V3: V1+V2 combined
"""
...
# L6 best parent flags (from bench_all_42.py:480, ts_lgk2_v12_memc_btw_all)
PARENT_FLAGS = (
...
JOBS = [
    {
        "tag": "V1_kls2",
        "N": N, "K": K,
        "suffix": "_R32A_ts_lgk2_v12_memc_btw_all_kls2",
        "flags": f"{PARENT_FLAGS} -DK_LOOP_SYNC_EVERY_2=1",
    },
    {
        "tag": "V2_nt",
        "N": N, "K": K,
        "suffix": "_R32A_ts_lgk2_v12_memc_btw_all_nt",
        "flags": f"{PARENT_FLAGS} -DB_LOAD_NONTEMPORAL=1 -DA_LOAD_NONTEMPORAL=1",
    },
    {
        "tag": "V3_kls2_nt",
        "N": N, "K": K,
        "suffix": "_R32A_ts_lgk2_v12_memc_btw_all_kls2_nt",
        "flags": f"{PARENT_FLAGS} -DK_LOOP_SYNC_EVERY_2=1 -DB_LOAD_NONTEMPORAL=1 -DA_LOAD_NONTEMPORAL=1",
    },
]
...
def build_one(job):
    ...
    log_path = os.path.join(BUILD_DIR, f"compile_R32A_{tag}_n{N}_k{K}.log")
    t0 = time.time()
...
def main():
    ...
    print(f"R32-A builds: {len(JOBS)} (parallel)")
    ...
    with ThreadPoolExecutor(max_workers=len(JOBS)) as ex:
        ...
        for fut in as_completed(futs):
            ...
            sc = res.get("scratch", "?")
            print(f"  {tag:14s} {status:10s} compile={dt}s VGPR={v} spills={sp} scratch={sc} occ={oc}", flush=True)

    dt_global = time.time() - t_global
    out_json = os.path.join(SCRIPT_DIR, "R32_OPT_A_BUILD_RESULTS.json")
