#!/usr/bin/env python3
"""R54 Opt D-4B/3 — bench harness for the aiter `.co` dlopen shim on (16384, 4096, 7168).

Reuses R50D shim AS-IS (bdx=256). Tile = 256x256 (aiter heuristic pick).
Marginal HK-VC claw-back candidate (HK 93.76%).
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 16384, 4096, 7168
COMPETITOR_TFLOPS = 4443.2  # from bench_all_42.py for (16384, 4096, 7168)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_D4B_3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R54 Opt D-4B/3 bench — mode={args.mode}, gpu={gpu_list}, "
