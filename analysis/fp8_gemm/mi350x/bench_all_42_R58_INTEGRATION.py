#!/usr/bin/env python3
"""R58 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate, ITERS=500.

Fork of bench_all_42_R57_INTEGRATION.py with these R58 deltas:
  - 1 binary change: P-2 swap (16384, 4096, 3072): HK R40B -> AITER 128x256
    (worker 10-run +3.50pp; bit-deterministic). AITER share 39 -> 40; HK 3 -> 2.
  - ITERS REVERTED 1000 -> 500 (R45+ default; R57 ITERS=1000 was one-time Opt J bump).
  - Adds AITER 128x256 dispatch entry for the P-2 swap.
  - warmup=200, trim=0.10, INDEPENDENT seeds [101..1010] unchanged
  Output defaults: R58_INTEGRATION_10RUN.{json,log}; round label R58_INTEGRATION
...
Bench MANDATORY: warmup=200, iters=500 (R45+ default; R58 reverts R57 ITERS=1000 bump), trim=0.10. Configurable GPU subset.
...
WARMUP = 200
ITERS = 500  # R45+ default; R58 reverts R57's one-time ITERS=1000 Opt J bump
TRIM = 0.10
...
AITER_SHAPES = {
    ...
    # R58 P-2 PROMOTE: HK R40B -> AITER 128x256 (16384, 4096, 3072) +3.50pp; bit-deterministic
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R58 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R58_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R58_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R58_INTEGRATION_10RUN.log")
    ...
    log(f"R58 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
