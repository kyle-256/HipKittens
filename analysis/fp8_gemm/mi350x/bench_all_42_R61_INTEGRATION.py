#!/usr/bin/env python3
"""R61 INTEGRATION reviewer bench — Opt Y2 cohort-race surface re-measurement (4th-consecutive sweep).

R61 Opt Y2 — 4th-consecutive cohort-race surface re-measurement; manifest byte-identical to R60
(which is byte-identical to R59 binary entries); seed set DISJOINT from R58/R59 [101..1010] AND
R60 [202, 404, 606, 808, 1010, 1212, 1414, 1616, 1818, 2020].

R61 deltas vs bench_all_42_R60_INTEGRATION.py (THIS FILE IS A COPY OF THE R60
SCRIPT WITH ONLY THE FOLLOWING CHANGES):
  - SEED_LIST default switched from [202, 404, ..., 2020] to
    [303, 606, 909, 1212, 1515, 1818, 2121, 2424, 2727, 3030] (R61 303-step pattern).
  - Output basenames R60_INTEGRATION_* -> R61_INTEGRATION_* (json, log, console).
  - Round label R60_INTEGRATION -> R61_INTEGRATION.
  - manifest path default switched to R60_INTEGRATION_MANIFEST.json (READ-ONLY;
    R61 binary state IS R60 binary state by definition for Opt Y2).
  - All other parameters (warmup=200, ITERS=500, trim=0.10, GPUs 4,5,6,7,
    AITER dispatch table, gate constants) are IDENTICAL to R60.
...
FINITE_GATE = 0.97
RANDOM_SEEDS = [303, 606, 909, 1212, 1515, 1818, 2121, 2424, 2727, 3030]
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R61 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R60_INTEGRATION_MANIFEST.json")
    ...
    if args.mode == "smoke1":
        ...
        out = args.out if args.out is not None else "R61_INTEGRATION_SMOKE1.json"
        log = args.log if args.log is not None else "R61_INTEGRATION_SMOKE1.log"
    elif args.mode == "10run":
        runs = args.runs if args.runs is not None else 10
        out = args.out if args.out is not None else "R61_INTEGRATION_10RUN.json"
        log = args.log if args.log is not None else "R61_INTEGRATION_10RUN.log"
    else:
        runs = args.runs if args.runs is not None else 10
        out = args.out if args.out is not None else "R61_INTEGRATION_10RUN.json"
        log = args.log if args.log is not None else "R61_INTEGRATION_10RUN.log"
    ...
    logmsg(f"R61 INTEGRATION bench: {len(jobs)} shapes, runs={runs}, GPUs={gpus}")
