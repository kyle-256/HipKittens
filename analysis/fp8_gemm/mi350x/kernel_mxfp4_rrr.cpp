...
static_assert(BLK == 256 && BK == 128 && WARPS_M == 2 && WARPS_N == 4,
    "MXFP4 RRR 8-wave fast path requires BLK=256, BK=128, 2x4 warp grid");
    ...
    ST& dst, const GL& src, const COORD& idx, const uint32_t* swizzled_offsets)
{
    ...
    i32x4 srsrc = std::bit_cast<i32x4>(make_buffer_resource(
        static_cast<uint64_t>(reinterpret_cast<std::uintptr_t>(global_ptr)),
        static_cast<uint32_t>(row_stride * ST::cols * sizeof(T)),
        0x00110000u));
    srsrc[0] = __builtin_amdgcn_readfirstlane(srsrc[0]);
    srsrc[1] = __builtin_amdgcn_readfirstlane(srsrc[1]);
    srsrc[2] = __builtin_amdgcn_readfirstlane(srsrc[2]);
    srsrc[3] = __builtin_amdgcn_readfirstlane(srsrc[3]);
...
void mxfp4_rrr_pq_kernel(const layout_globals g) {
    ...
    prefill_transpose_swizzled_offsets<_NUM_THREADS>(Bs[0], g.b, so_b);
    ...
    for (int bt = 0; bt < k_byte_iters; ++bt) {
        ...
        load_transpose<_NUM_THREADS>(Bs[0], g.b, {0, 0, bc * 2,     bt}, so_b);
        load_transpose<_NUM_THREADS>(Bs[1], g.b, {0, 0, bc * 2 + 1, bt}, so_b);
...
void dispatch_rrr_pq(layout_globals g) {
    g.m = static_cast<int>(g.c.rows());
    g.n = static_cast<int>(g.c.cols());
    g.k = static_cast<int>(g.a.cols()) * 2;
    const dim3 grid((g.m / BLK) * (g.n / BLK));
    mxfp4_rrr_pq_kernel<<<grid, dim3(_NUM_THREADS), 0, g.stream>>>(g);
}

void dispatch_rrr(layout_globals g) { dispatch_rrr_pq(g); }

PYBIND11_MODULE(tk_mxfp4_rrr, m) {
    m.doc() = "MXFP4 GEMM with preshuffle-quant scales (RRR layout; B is (K//2,N) packed, transpose-loaded)";
    py::bind_function<dispatch_rrr>(m, "gemm_rrr",
        &layout_globals::a, &layout_globals::b,
        &layout_globals::a_scale, &layout_globals::b_scale,
        &layout_globals::c);
    py::bind_function<dispatch_rrr_pq>(m, "gemm_rrr_pq",
        &layout_globals::a, &layout_globals::b,
        &layout_globals::a_scale, &layout_globals::b_scale,
        &layout_globals::c);
}
