#!/usr/bin/env python3
"""R38 Opt D: Build candidate non-memc variants for the 19 R37 WRONG_OUTPUT shapes.

For each shape we build the top-N clean candidates (no memc/dc/tv0 flags).
Each builds with the R37 strip_bad_sched logic (in case some flag string still
includes -mllvm -amdgpu-sched-strategy=max-memory-clause via name overlap).
Modules are tagged `_R38D` to avoid colliding with R37 builds.
"""
import json
import os
import re
...
def build_one(n_dim, k_dim, parent_tag, base_cppflags):
    module_name = f"tk_mxfp4_gluon_cpp_n{n_dim}_k{k_dim}_{parent_tag}_R38D"
    out_file = f"{module_name}{EXT_SUFFIX}"
    out_path = os.path.join(BUILD_DIR, out_file)
    key = f"N={n_dim},K={k_dim},{parent_tag}_R38D"
    ...
    wrapper_src = os.path.join(BUILD_DIR, f"wrap_n{n_dim}_k{k_dim}_{parent_tag}_R38D.cpp")
...
def main():
    ...
    plan = []  # (m,n,k,parent_tag)
    tasks = []  # (n,k,parent_tag,flags)
    for shape_str, info in cands["shapes"].items():
        ...
        for c in info["candidates"]:
            ...
            if tag not in suffix_to_flags:
                print(f"  WARN: variant {tag} missing in bench_all_42.py")
                continue
            flags = suffix_to_flags[tag]
            tasks.append((n, k, tag, flags))
            plan.append((m, n, k, tag))

    unique_tasks = list(set(tasks))
    print(f"R38D builder: {len(plan)} shape×variant entries, {len(unique_tasks)} unique builds")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if status == "cached":
                cached += 1
                if completed % 20 == 0:
                    print(f"  [{completed}/{len(unique_tasks)}] (cached batch)")
            elif status == "ok":
                built += 1
                el = res[2]
    ...
    manifest = {
        ...
        "shapes_to_R38D": [
            {"M": m, "N": n, "K": k, "tag": tag,
             "module": f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38D"}
    ...
    with open(os.path.join(SCRIPT_DIR, "R38D_BUILD_MANIFEST.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print("Manifest: R38D_BUILD_MANIFEST.json")
