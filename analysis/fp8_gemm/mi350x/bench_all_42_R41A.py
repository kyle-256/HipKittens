#!/usr/bin/env python3
"""R41 Opt A: bench the 5 catastrophic K=32768 cluster-C shapes against the
R41A_po<N>_ef<0|1> sweep cells, with random-scale + SNR correctness gate.

Cloned from bench_all_42_R40B.py with two changes:
  - BUILD_DIR = build_R41A
  - select cell via --po and --ef args (or --all-cells to iterate all 10)
...
# 5 catastrophic shapes from R41 decider plan (with R40B competitor TFLOPS)
ALL_SHAPES = [
...
def find_so(m, n, k, po, ef):
    tag = _safe_tag(PARENT_VARIANT)
    mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R41A_po{po}_ef{ef}"
...
def bench_one_shape(m, n, k, comp, po, ef, gpu_id):
    module_name, so_path = find_so(m, n, k, po, ef)

    if not so_path:
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                "comp": comp, "status": "MISSING_SO", "kernel_finite": None,
                "best_variant": f"R41A_po{po}_ef{ef}",
                "po": po, "ef": ef}
    ...
    try:
        ...
        out["best_variant"] = f"R41A_po{po}_ef{ef}"
        out["po"] = po
        out["ef"] = ef
        return out
...
def run_one_pass(gpus, run_label, shape_cells):
    """shape_cells: list of (m,n,k,comp,po,ef) tuples."""
    print(f"R41A Bench [{run_label}] - GPUs: {gpus}, n_jobs={len(shape_cells)}")
    ...
    def run_gpu(gpu_id):
        for (i, (m, n, k, comp, po, ef)) in gpu_tasks[gpu_id]:
            r = bench_one_shape(m, n, k, comp, po, ef, gpu_id)
            with lock:
                results[i] = r
                if r["status"] == "OK":
                    ...
                    print(f"  [{i+1:>3}/{len(shape_cells)}] {m:>6}x{n:>6}x{k:>6}  "
                          f"po{po:>2} ef{ef}  "
                          f"{r['status']} snr={snr_s} fin={fin_s} wcf={wcf_s} GPU{gpu_id}",
                          flush=True)
...
def main():
    ...
    ap.add_argument("--out", default="R41_OPT_A_BENCH.json")
    ap.add_argument("--cells", default="all",
                    help="'all' = full 10-cell sweep, or 'po,ef;po,ef;...' explicit")
    ap.add_argument("--shape", default="all",
                    help="'all' = 5 catastrophic shapes, or 'MxNxK[,MxNxK,...]'")
    args = ap.parse_args()
    gpus = [int(g) for g in args.gpus.split(",")]

    # Build (po,ef) list
    if args.cells == "all":
        cells = [(po, ef) for po in [0, 8, 16, 32, 64] for ef in [0, 1]]
    else:
        cells = []
        for cs in args.cells.split(";"):
            po_s, ef_s = cs.strip().split(",")
            cells.append((int(po_s), int(ef_s)))
    ...
    for (m, n, k, comp) in shapes:
        for (po, ef) in cells:
            shape_cells.append((m, n, k, comp, po, ef))

    print(f"R41A bench: {len(shapes)} shapes x {len(cells)} cells = "
    ...
    # Multi-run consensus per (shape,cell) — take median wcf and median tflops across PASS runs
    consensus = []
    ...
    for i in range(n_jobs):
        ...
        m, n_, k_, comp, po, ef = shape_cells[i]
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "n_runs": args.runs,
            "gpus": gpus,
            "shape_cells": [{"m": m, "n": n, "k": k, "comp": c, "po": po, "ef": ef}
                            for (m, n, k, c, po, ef) in shape_cells],
            "consensus": consensus,
            "per_run": all_runs,
        }, f, indent=2)
