"""R33 Dev A — Task 2 paired BABA bench: Path 1 rect vs square V2-CRR.

Same harness as r32c_paired_bench / r32c_c4_paired_bench but for Path 1
single-shape (default 70B KV 4096x1024x8192). Both kernels expose the same
gemm_crr_pq_v2 entry; we load two .so via importlib.

Env: M, N, K, SO_A, SO_B, MOD_A, MOD_B, LABEL_A, LABEL_B, PHYS_GPU,
     N_PAIRS, MXFP8_WARMUP, MXFP8_ITERS, PREHEAT_S, WARMUP_PAIRS
"""
import math, os, statistics, subprocess, importlib.util, time
import torch
...
mod_a_name = os.environ.get('MOD_A', 'tk_mxfp8_a')
mod_b_name = os.environ.get('MOD_B', 'tk_mxfp8_b')
...
def fn_a(): mod_a.gemm_crr_pq_v2(A, B, As, Bs, C)
def fn_b(): mod_b.gemm_crr_pq_v2(A, B, As, Bs, C)
...
for w in range(warmup_pairs):
    ...
    print(f'WARMUP {w} {label_a}={ta:.2f} {label_b}={tb:.2f} sclk={read_sclk()}', flush=True)
for r in range(n_pairs):
    ...
    print(f'PAIR {r} {label_a}_1={ta:.2f} {label_b}_1={tb:.2f} {label_a}_2={ta2:.2f} {label_b}_2={tb2:.2f}', flush=True)
