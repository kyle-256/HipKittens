#!/usr/bin/env python3
"""R54 Opt D-4B/1 — bench harness for the aiter `.co` dlopen shim on (4096, 4096, 16384).

Reuses R50D shim AS-IS (bdx=256). Tile = 256x256 (aiter heuristic pick).
Marginal HK-VC claw-back candidate (HK 93.30%). Sister of R52D2C (4096,4096,32768)=105.94%.
...
COMPETITOR_TFLOPS = 4642.1  # from bench_all_42.py for (4096, 4096, 16384)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_D4B_1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R54 Opt D-4B/1 bench — mode={args.mode}, gpu={gpu_list}, "
