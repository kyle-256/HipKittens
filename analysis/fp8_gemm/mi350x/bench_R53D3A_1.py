#!/usr/bin/env python3
"""R53 Opt D-3A_1 — bench harness for the aiter `.co` dlopen shim on (4096, 14336, 16384).

Reuses R50D shim AS-IS (bdx=256 hardcoded matches aiter's bdx=256 for ALL tiles).
Only shape constants + tile_M=96, tile_N=640 + co_path + kernel_name change.
Cloned from bench_R52D2A.py.
...
Per-shape grid (computed inside shim from M,N,K and tile sizes):
  gdx = ceil(N/640) = ceil(14336/640) = 23
  gdy = ceil(M/96) = ceil(4096/96) = 43
  gdz = 1, bdx = 256
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 14336, 16384
COMPETITOR_TFLOPS = 5013.0  # from bench_all_42.py for (4096, 14336, 16384)

VARIANT_TAG = "R53_OPT_D3A_1"
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, f"{VARIANT_TAG}_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, f"{VARIANT_TAG}_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, f"{VARIANT_TAG}_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, f"{VARIANT_TAG}_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False

    results = []
    log_lines = []
    log_lines.append(f"{VARIANT_TAG} bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K}), tile=({TILE_M},{TILE_N})")
    log_lines.append("=" * 90)
    ...
    for i, seed in enumerate(seeds):
        ...
        r["gpu_id"] = gpu_id
        results.append(r)
        line = (f"  seed={seed:>5} gpu={gpu_id} status={str(r.get('status')):<22} "
