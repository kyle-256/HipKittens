#!/usr/bin/env python3
"""R52 Opt D-2B — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 128256).

Reuses the EXISTING R50D shim built at:
  build_R50D/R50D_aiter_shim.cpython-310-x86_64-linux-gnu.so

Adapted from bench_R51D2.py: only shape (M, N, K) and competitor TFLOPS change.
Grid auto-derives from (N, M) via ceil-div by 256 in the shim itself.
For (4096, 32768, 128256): gdx=ceil(32768/256)=128, gdy=ceil(4096/256)=16, gdz=1.
...
COMPETITOR_TFLOPS = 5781.1  # from bench_all_42_R51_INTEGRATION.py for (4096,32768,128256)
...
def run_one_seed(seed: int, gpu_id, do_perf: bool, timeout: int = 1800,
                 warmup_override=None, iters_override=None):
    script = make_runner_script(seed, do_perf)
    if warmup_override is not None:
        script = script.replace(f"WARMUP, ITERS, TRIM_FRAC = {WARMUP}, {ITERS}, {TRIM_FRAC}",
                                f"WARMUP, ITERS, TRIM_FRAC = {warmup_override}, {iters_override}, {TRIM_FRAC}")
    env = os.environ.copy()
...
def main():
    ...
    ap.add_argument("--gpu", type=str, default="5")
    ap.add_argument("--out", default=None)
    ap.add_argument("--smoke-warmup", type=int, default=200)
    ap.add_argument("--smoke-iters", type=int, default=500)
    args = ap.parse_args()

    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2B_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2B_SMOKE.log")
        seeds = [101]
        do_perf = True
        warmup_o, iters_o = args.smoke_warmup, args.smoke_iters
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2B_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2B_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False
        warmup_o, iters_o = None, None  # use full WARMUP/ITERS

    results = []
    log_lines = []
    log_lines.append(f"R52 Opt D-2B bench — mode={args.mode}, gpu={args.gpu}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
    ...
    for seed in seeds:
        ...
        r = run_one_seed(seed, args.gpu, this_perf,
                         warmup_override=warmup_o, iters_override=iters_o)
        dt = time.time() - t0
        r["wall_s"] = round(dt, 1)
        results.append(r)
        line = (f"  seed={seed:>5} status={r.get('status'):<22} "
