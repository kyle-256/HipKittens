#!/usr/bin/env python3
"""R56 Opt G-2 / L5 — alt-tile 256x256 swap on (14336, 32768, 4096).

Current baseline: R53D3C_1_AITER 64x1024 -> 87.92% pct_comp.
Alt: AITER 256x256 via R50D shim AS-IS (no rebuild). Heuristic eff 60.2 -> 128.0
on identical local_round=28.

Grid: bdx=256, gdx=ceil(N/256)=128, gdy=ceil(M/256)=56, gdz=1.
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 14336, 32768, 4096
COMPETITOR_TFLOPS = 4462.6  # bench_all_42.py
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G2_L5")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-2 / L5 — mode={args.mode}, gpu={gpu_list}, "
