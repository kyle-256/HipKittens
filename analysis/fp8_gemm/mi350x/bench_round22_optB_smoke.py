#!/usr/bin/env python3
"""R22B smoke bench (warmup=200 iters=500 trim=10%) of all 5 R22B variants.

Per benchmark-rules.md MANDATORY: warmup=200, iters=500, 10% trim.
Subprocess-isolated. Runs across GPUs 4-5 only (other agents on 0-3 + 6-7).
"""
...
def main():
    ...
    print(f"R22B smoke: {len(args_list)} bench jobs (3 shapes x 5 variants), GPUs {GPU_POOL}")
    ...
    with open(os.path.join(SCRIPT_DIR, "bench_round22_optB_smoke.json"), "w") as f:
        json.dump(out, f, indent=2)

    # Summary: deltas vs each shape's _r22b_baseline
    print("\n" + "=" * 110)
    print("SUMMARY (Δ vs _r22b_baseline)")
    print("=" * 110)
    for (lab, *_rest) in SHAPES:
        b = out["shapes"][lab].get("_r22b_baseline", {})
        bt = b.get("tflops")
        if bt is None:
