...
#define GROUP_SIZE_M 4
    ...
    // Pre-compute LDS addresses + XOR toggle masks for double-buffer switching.
    // XOR toggle replaces ternary (v_cndmask) with v_xor_b32 — fewer VALU ops.
    ...
    // XOR masks for toggling between db[0] and db[1]
    const uint32_t xor_a0_p0 = a0_0_p0 ^ a0_1_p0, xor_a0_p1 = a0_0_p1 ^ a0_1_p1;
    const uint32_t xor_bl_p0 = bl_0_p0 ^ bl_1_p0, xor_bl_p1 = bl_0_p1 ^ bl_1_p1;
    const uint32_t xor_br_p0 = br_0_p0 ^ br_1_p0, xor_br_p1 = br_0_p1 ^ br_1_p1;
    const uint32_t xor_a1_p0 = a1_0_p0 ^ a1_1_p0, xor_a1_p1 = a1_0_p1 ^ a1_1_p1;

    // Current LDS addresses — initialized for bt=0 (cur=db[0], nxt=db[1])
    // These get XOR-toggled at the end of each iteration
    uint32_t sel_br_p0 = br_0_p0, sel_br_p1 = br_0_p1;   // cur: Br[db0]
    uint32_t sel_a1_p0 = a1_0_p0, sel_a1_p1 = a1_0_p1;   // cur: A1[db0]
    uint32_t sel_a0_p0 = a0_1_p0, sel_a0_p1 = a0_1_p1;   // nxt: A0[db1]
    uint32_t sel_bl_p0 = bl_1_p0, sel_bl_p1 = bl_1_p1;   // nxt: Bl[db1]
    ...
    for (int bt = 0; bt < k_byte_iters; ++bt) {
        ...
        // LDS addresses: sel_* vars are maintained via XOR toggle (see end of loop)
        // No ternary / v_cndmask needed — addresses flip each iteration via XOR
        ...
        // XOR toggle: flip all LDS addresses to the other double-buffer slot
        sel_br_p0 ^= xor_br_p0; sel_br_p1 ^= xor_br_p1;
        sel_a1_p0 ^= xor_a1_p0; sel_a1_p1 ^= xor_a1_p1;
        sel_a0_p0 ^= xor_a0_p0; sel_a0_p1 ^= xor_a0_p1;
        sel_bl_p0 ^= xor_bl_p0; sel_bl_p1 ^= xor_bl_p1;
    }
...
PYBIND11_MODULE(tk_mxfp4_xor, m) {
    m.doc() = "MXFP4 Gluon-arch kernel with XOR toggle";
