#!/usr/bin/env python3
"""R54 Opt E1_3 — bench harness for the aiter `.co` dlopen shim on (28672, 4096, 8192).

Reuses R50D shim AS-IS (256x256 tile, bdx=256). Only shape constants change.
Cohort-race rescue candidate: R52 VC at 91.8%, R53 cohort LOSS PASS_9/10 wcf_max=0.0201.

Per-shape grid (computed inside shim from M,N,K and tile sizes):
  gdx = ceil(N/256) = ceil(4096/256)  = 16
  gdy = ceil(M/256) = ceil(28672/256) = 112
  gdz = 1, bdx = 256
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 28672, 4096, 8192
COMPETITOR_TFLOPS = 4810.0  # from R53_INTEGRATION_10RUN.json consensus for (28672, 4096, 8192)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R54_OPT_E1_3_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R54_OPT_E1_3_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R54_OPT_E1_3_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R54_OPT_E1_3_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R54 Opt E1_3 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
