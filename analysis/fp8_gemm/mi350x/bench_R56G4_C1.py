#!/usr/bin/env python3
"""R56 Opt G-4 / C1 — bench harness for the aiter `.co` dlopen shim on (4096, 128256, 32768).

R56 G-4 / C1: HK->AITER probe on L7 (currently HK R41A baseline, pct_comp=97.59%).
Alt: 256x256 via R50D shim AS-IS.

D-3A-1 RISK MEDIUM: must beat 98.59% pct_comp to PROMOTE (HK 97.59 + 1.0pp).
...
Per-shape grid: gdx=ceil(N/256)=501, gdy=ceil(M/256)=16, gdz=1, bdx=256
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 128256, 32768
COMPETITOR_TFLOPS = 3195.3  # bench_all_42.py competitor for (4096, 128256, 32768)
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G4_C1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-4 C1 bench - mode={args.mode}, gpu={gpu_list}, "
