#!/bin/bash
# R56 Reviewer 9-cell baseline matrix at HEAD 0e068a54.
# Mirror of R54 reviewer's exact shape × layout × tensor list for 1:1 regression compare.
...
#
# Cells = 9.  Default macros only (no R54/R55 dev macros — verifying byte-identical default).
set -uo pipefail
cd "$(dirname "$0")/../"
SRC_DIR="$(pwd)"
...
OUTDIR="r56_reviewer_results/baseline_9cell"
mkdir -p "$OUTDIR"
