#!/usr/bin/env python3
"""R55 Opt E4_2 — bench harness for the aiter `.co` dlopen shim on (6144, 32768, 4096).

Reuses R50D shim AS-IS (256x256 tile, bdx=256). Only shape constants change.
Cohort-race rescue candidate: R54 PASS_9/10 wcf=0.0209 fin=0.97906 (HK R40B).
...
Per-shape grid (computed inside shim from M,N,K and tile sizes):
  gdx = ceil(N/256) = ceil(32768/256) = 128
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 6144, 32768, 4096
COMPETITOR_TFLOPS = 4291.0  # from R54_INTEGRATION_10RUN.json consensus for (6144, 32768, 4096)
...
def main():
    ...
    ap.add_argument("--gpu", default="1",
                    help="Single GPU index (e.g. '1') or comma-list to round-robin (e.g. '0,1').")
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E4_2_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E4_2_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E4_2_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E4_2_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R55 Opt E4_2 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
