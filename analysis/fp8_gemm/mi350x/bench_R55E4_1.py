#!/usr/bin/env python3
"""R55 Opt E4_1 — bench harness for the aiter `.co` dlopen shim on (16384, 6144, 4096).

Reuses R50D shim AS-IS (256x256 tile, bdx=256). Only shape constants change.
Cohort-race rescue candidate: R54 FLAKE_4/10 wcf=0.0631 fin=0.991192 (HK R40B).
...
COMPETITOR_TFLOPS = 4042.5  # from R54_INTEGRATION_10RUN.json consensus for (16384, 6144, 4096)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E4_1_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E4_1_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E4_1_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E4_1_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R55 Opt E4_1 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
