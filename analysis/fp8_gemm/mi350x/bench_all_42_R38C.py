#!/usr/bin/env python3
"""R38 OptC: bench every shape's _R38C module with correctness gate.

Same harness as bench_all_42_R38B.py but loads from build_R38C/ and uses _R38C suffix.
Output JSON: bench_all42_results_R38_optC.json
"""
import json
import os
import subprocess
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_R38C"
        return out
...
def main():
    ...
    print(f"R38 OptC Correctness-Gated Bench - GPUs: {gpus}")
    ...
    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R38_optC.json")
