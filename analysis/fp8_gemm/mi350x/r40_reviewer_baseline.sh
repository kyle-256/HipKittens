#!/bin/bash
# R40 Reviewer Phase 1: 4-GPU baseline 70B-KV V2-CRR (M=4096, N=1024, K=8192)
# Uses single .so + R35_reviewer_bench5x with R36 3-gate + R37 G1' fallback (manual gate-ladder).
# This is the 10th cross-cycle baseline.
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

OUTDIR="$HERE/r40_reviewer_4gpu_runs"
mkdir -p "$OUTDIR"
...
# Use the pre-built baseline .so (built once, reused per-GPU)
SO_SRC=${SO_SRC:-/tmp/r40_70bkv_baseline.so}

# Each GPU gets its own staged .so to avoid concurrent file overwrites
STAGED_SO="$HERE/tk_mxfp8_layouts.cpython-310-x86_64-linux-gnu.so"
# We'll use a symlink/copy per-process via PROD-style; simplest: copy once
if [ ! -f "$STAGED_SO" ] || [ "$SO_SRC" -nt "$STAGED_SO" ]; then
  cp -f "$SO_SRC" "$STAGED_SO"
fi
H=$(md5sum "$STAGED_SO" | awk '{print $1}')
echo "[r40_reviewer_baseline gpu=$PHYS_GPU md5=$H so=$SO_SRC]"

OUT="$OUTDIR/${CELL}_mxfp8_gpu${PHYS_GPU}.txt"
echo "===== R40 Reviewer Phase 1 baseline cell=$CELL phys_gpu=$PHYS_GPU =====" | tee "$OUT"
date | tee -a "$OUT"
