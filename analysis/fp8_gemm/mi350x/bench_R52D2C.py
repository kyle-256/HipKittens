#!/usr/bin/env python3
"""R52 Opt D-2C — bench harness for the aiter `.co` dlopen shim on (4096, 4096, 32768).

Reuses the EXISTING R50D shim (build_R50D/R50D_aiter_shim...so) AS-IS — only the
shape changes (and the shim auto-computes grid from M/N/tile).
...
COMPETITOR_TFLOPS = 5152.8  # from bench_all_42_R51_INTEGRATION.py for (4096,4096,32768)
...
def run_one_seed(seed: int, gpu_id: int, do_perf: bool, timeout: int = 900):
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2C_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2C_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R52_OPT_D2C_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R52_OPT_D2C_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R52 Opt D-2C bench — mode={args.mode}, gpu={args.gpu}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
