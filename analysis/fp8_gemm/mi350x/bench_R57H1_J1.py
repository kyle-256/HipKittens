#!/usr/bin/env python3
"""R57 Opt J-1 / L1 — Cohort H-1 — re-bench at ITERS=1000.

Identical AITER `.co` + R50D shim AS-IS as R56G1_L1; ONLY change is ITERS=1000
(double the bench rule minimum). Goal: tighten p50 distribution to close the
0.02pp gap to WIN line (R56 reviewer p50 99.98% vs worker 100.63%).

L1 = (4096, 32768, 14336), tile 256x256, .co bit-deterministic AITER.

Bench MANDATORY: warmup=200, iters=1000 (Opt J special), trim=0.10.
10-run mode runs perf on EVERY seed [101..1010] so we get a true p50/p25/p75
distribution (vs R56 G-1 which only ran perf on the first seed).

Per-shape grid: gdx=128, gdy=16, gdz=1, bdx=256 (verbatim from R56G1_L1).
"""
import argparse
...
WARMUP = 200
ITERS = 1000  # R57 Opt J: doubled from R56 default 500
TRIM_FRAC = 0.10
SNR_DB_GATE = 10.0
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 32768, 14336
COMPETITOR_TFLOPS = 5296.1
R56_PCT_COMP = 99.98
...
def run_one_seed(seed: int, gpu_id, do_perf: bool, timeout: int = 1800):
...
def main():
    ...
    ap.add_argument("--tag", default="R57_OPT_J1_L1")
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, f"{args.tag}_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, f"{args.tag}_SMOKE.log")
        seeds = args.seeds or [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, f"{args.tag}_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, f"{args.tag}_10RUN.log")
        seeds = args.seeds or [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        # R57 Opt J: perf on EVERY seed to get a real p50 distribution
        do_perf = True

    results = []
    log_lines = []
    log_lines.append(f"R57 Opt J-1 L1 bench - mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K}), tile={TILE_M}x{TILE_N}, "
                     f"warmup={WARMUP}, iters={ITERS}, trim={TRIM_FRAC}")
    ...
    for i, seed in enumerate(seeds):
        ...
        t0 = time.time()
        r = run_one_seed(seed, gpu_id, do_perf)
        dt = time.time() - t0
        r["wall_s"] = round(dt, 1)
        r["gpu_id"] = gpu_id
        results.append(r)
        pct = (round(r.get("tflops") / COMPETITOR_TFLOPS * 100.0, 2)
               if r.get("tflops") is not None else None)
        line = (f"  seed={seed:>5} gpu={gpu_id} status={r.get('status'):<22} "
                ...
                f"tflops={r.get('tflops')!s:<10} pct={pct!s:<7} ({dt:.1f}s)")
    ...
    tflops_vals = sorted([r.get("tflops") for r in results if r.get("tflops") is not None])
    pct_vals = sorted([round(t / COMPETITOR_TFLOPS * 100.0, 4) for t in tflops_vals])
    ...
    summary = {
        ...
        "iters": ITERS,
        "trim_frac": TRIM_FRAC,
        "iters_protocol": "R57_OPT_J_LONG_BENCH",
        "n_runs": len(results),
        "n_OK": n_ok,
        ...
        "tflops_min": tflops_vals[0] if tflops_vals else None,
        "tflops_max": tflops_vals[-1] if tflops_vals else None,
        "tflops_p25": percentile(tflops_vals, 0.25),
        "tflops_p50": percentile(tflops_vals, 0.50),
        "tflops_p75": percentile(tflops_vals, 0.75),
        "pct_comp_p25": percentile(pct_vals, 0.25),
        "pct_comp_p50": percentile(pct_vals, 0.50),
        "pct_comp_p75": percentile(pct_vals, 0.75),
        "pct_comp_min": pct_vals[0] if pct_vals else None,
        "pct_comp_max": pct_vals[-1] if pct_vals else None,
        "pct_comp_first": pct_vals[0] if pct_vals else None,
        "r56_pct_comp": R56_PCT_COMP,
        "perf_delta_pp": (round(percentile(pct_vals, 0.50) - R56_PCT_COMP, 4)
        ...
        "win_line_crossed": (
            (percentile(pct_vals, 0.50) is not None
             and percentile(pct_vals, 0.50) >= 100.0)
        ),
    }
