#!/usr/bin/env python3
"""R54 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R53_INTEGRATION.py with these changes:
  ...
  3. Per-shape backend dispatch — 27 aiter shim cells (all reuse R50D shim AS-IS):
        Carried from R53 (15 cells):
        ...
        NEW IN R54 (12 new aiter overrides, all 256x256 tile):
          E-1 cohort-race rescue (M-heavy):
            (32768, 4096, 2048)   R54E1_1 (+1 NET VC, 110.99% comp)
            (32768, 4096, 3072)   R54E1_2 (+1 NET VC, 118.64% comp)
            (28672, 4096, 8192)   R54E1_3 (+1 NET VC, 109.05% comp)
          E-2 cohort-race rescue (N-heavy):
            (4096, 32768, 4096)   R54E2_1 (+1 NET VC, 106.97% comp)
            (4096, 32768, 6144)   R54E2_2 (+1 NET VC, 109.72% comp)
            (16384, 4096, 6144)   R54E2_3 (+1 NET VC, 117.35% comp)
          D-4A sub-90 HK-VC perf claw-back:
            (4096, 14336, 16384)  R54D4A_1 (108.28% vs HK 84.41%)
            (32768, 4096, 7168)   R54D4A_2 (106.78% vs HK 88.36%)
            (6144, 4096, 8192)    R54D4A_3 (117.75% vs HK 89.53%)
          D-4B 90-95 marginal claw-back:
            (4096, 4096, 16384)   R54D4B_1 (113.11% vs HK 93.30%)
            (4096, 14336, 8192)   R54D4B_2 (111.92% vs HK 91.92%)
            (16384, 4096, 7168)   R54D4B_3 (113.22% vs HK 93.76%)
       all other 15 shapes -> R40B/R41A/R41B baseline manifest entries unchanged
  4. Output defaults: R54_INTEGRATION_10RUN.{json,log}
  5. Round label = R54_INTEGRATION
...
# Aiter shim path (single shim works for all 15 cells; per-shape co/kname/tile vary)
...
AITER_SHAPES = {
    ...
    # R54 new 256x256 cells (E-1 cohort-race rescue, M-heavy)
    ...
    # R54 new 256x256 cells (E-2 cohort-race rescue, N-heavy)
    ...
    # R54 new 256x256 cells (D-4A sub-90 HK-VC perf claw-back)
    ...
    # R54 new 256x256 cells (D-4B 90-95 marginal claw-back)
...
# ---------------------------------------------------------------------------
# Standard HipKittens runner — matches bench_all_42_R52_INTEGRATION.py
# ---------------------------------------------------------------------------
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R54 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R54_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R54_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R54_INTEGRATION_10RUN.log")
    ...
    log(f"R54 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
