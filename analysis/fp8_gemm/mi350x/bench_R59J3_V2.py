#!/usr/bin/env python3
"""R59 Opt V-2 — 64x1024 alt-tile probe on L1 (4096, 32768, 14336).

R59 J-3 / V-2: 64x1024 aiter alt-tile via R50D shim AS-IS
(current AITER 256x256 R57J1 99.94% under ITERS=500).
Hypothesis: low (eff=60.2). PROMOTE only if SMOKE > 100.94% (current + 1.0pp).
Else: ACCEPT_FALLBACK (axis closure for L1 alt-tile space).

Modes:
  --mode smoke    5 seeds, full correctness gate + perf measurement
...
COMPETITOR_TFLOPS = 5296.1  # bench_all_42.py competitor for (4096, 32768, 14336)
...
def main():
    ...
    ap.add_argument("--tag", default="R59_OPT_V2")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R59 Opt V-2 bench - mode={args.mode}, gpu={gpu_list}, "
