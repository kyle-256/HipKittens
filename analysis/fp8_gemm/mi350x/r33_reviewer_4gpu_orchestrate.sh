#!/bin/bash
# R33 Reviewer Phase 1: 4-GPU triangulation for 70B KV V2-CRR (M=4096 N=1024 K=8192)
# Goal: 3rd cross-cycle data point for "high regime rotates per cycle" hypothesis.
#       R31 GPU0 high (786.38), R32 GPU5 high (786.59), R33 ???
#
...
#
# Usage:
#   PHYS_GPU=4 ./r33_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=5 ./r33_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=6 ./r33_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=0 ./r33_reviewer_4gpu_orchestrate.sh   # last to deconflict with R33 Dev A
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
WT="$(cd "$HERE/../../.." && pwd)"
cd "$HERE"

OUTDIR="$HERE/r33_reviewer_4gpu_runs"
mkdir -p "$OUTDIR"
...
ROCR_VISIBLE_DEVICES=$PHYS_GPU HIP_VISIBLE_DEVICES=0 PHYS_GPU=$PHYS_GPU \
  ...
  python3 r33_reviewer_bench5x.py mxfp8 $LAYOUT $M $N $K 2>&1 | tee -a "$OUT"
