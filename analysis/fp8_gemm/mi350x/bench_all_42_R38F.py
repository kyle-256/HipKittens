#!/usr/bin/env python3
"""R38 OptF (variant 3): bench every shape's _R38F3 module with correctness gate.

Same harness as bench_all_42_R38B.py but loads from build_R38F3/ and uses _R38F3 suffix.
Output JSON: bench_all42_results_R38_optF.json
"""
import json
import os
import subprocess
...
BUILD_DIR = os.path.join(SCRIPT_DIR, "build_R38F3")
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_R38F3"
        return out
...
def main():
    ...
    print(f"R38 OptF (variant 3) Correctness-Gated Bench - GPUs: {gpus}")
    ...
    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R38_optF.json")
