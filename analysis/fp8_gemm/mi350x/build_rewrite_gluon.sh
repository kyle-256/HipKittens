...
if [[ "$MODE" != "--rewrite-only" ]]; then
    ...
    THUNDERKITTENS_ROOT="$THUNDERKITTENS_ROOT" ROCM_PATH=$ROCM \
      CPPFLAGS="$DIMS $EXTRA" make -B TARGET=tk_mxfp4_gluon_cpp SRC=$SRC

    echo "=== Step 2: Generate device .s ==="
    $ROCM/bin/hipcc --offload-device-only -S $SRC $COMMON_FLAGS \
      -std=c++20 -w $INCLUDE_FLAGS $DIMS $EXTRA \
...
CUID=$(python3 - "$DEVICE_S" <<'PY'
import re
import sys

text = open(sys.argv[1], "r", encoding="utf-8").read()
m = re.search(r"__hip_cuid_([0-9a-f]+)", text)
if not m:
    sys.exit(1)
print(m.group(1))
PY
...
$CLANG -c -x assembler "$OPT_S" -target amdgcn-amd-amdhsa -mcpu=gfx950 -o "$TMP/device.o"
