"""Test/benchmark: MXFP4 Gluon-arch C++ kernel."""
...
det_runs   = int(os.environ.get("MXFP4_DETERMINISM_RUNS", "3"))

k_blocks = K // 32

print(f"=== MXFP4 Gluon C++: M={M}, N={N}, K={K} ===")
    ...
    if det_runs > 1:
        ...
        for _ in range(det_runs - 1):
            ...
            if not torch.equal(C, ref):
                det_ok = False; break
        print(f"Determinism ({det_runs} runs): {'PASS' if det_ok else 'FAIL'}")
