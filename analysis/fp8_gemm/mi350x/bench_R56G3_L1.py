#!/usr/bin/env python3
"""R56 Opt G-3 / L1 — 128x512 falsification probe on (4096, 32768, 14336).

Reuses R50D shim AS-IS. Tile = 128x512 (alt eff=102.4 vs current 64x1024 eff=60.2,
vs G-1 candidate 256x256 eff=128.0). G-3 cohort tests whether *any* wider tile
beats 64x1024 (mechanism = "tile-too-narrow") versus only 256x256 wins (mechanism
= "compute2mem_efficiency dominates").

Per-shape grid (bdx=256, gdz=1):
  gdx = ceil(N/512) = ceil(32768/512) = 64
  gdy = ceil(M/128) = ceil(4096/128)  = 32
"""
import argparse
...
COMPETITOR_TFLOPS = 5296.1  # L1 baseline from bench_all_42.py
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G3_L1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-3 bench — mode={args.mode}, gpu={gpu_list}, "
