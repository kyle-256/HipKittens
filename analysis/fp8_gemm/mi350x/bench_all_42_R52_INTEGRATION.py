#!/usr/bin/env python3
"""R52 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R51_INTEGRATION.py with these changes:
  ...
  3. Per-shape backend dispatch — 7 aiter shim cells (all reuse R50D shim AS-IS):
       - (4096,  32768,  28672)  -> aiter shim (R50D, carried)
       - (14336, 4096,   32768)  -> aiter shim (R51D1, perf claw-back)
       - (16384, 4096,   28672)  -> aiter shim (R51D2, perf claw-back, replaces R44A drain)
       - (28672, 4096,   16384)  -> aiter shim (R51D3, +1 NET VC, FLAKE->PASS)
       - (4096,  28672,  32768)  -> aiter shim (R52D2A, perf claw-back +39.6pp)
       - (4096,  32768,  128256) -> aiter shim (R52D2B, perf claw-back +27.4pp on largest-K)
       - (4096,  4096,   32768)  -> aiter shim (R52D2C, perf claw-back +28.4pp)
       - all other 35 shapes -> R44/R50 baseline manifest entries unchanged
  4. Output defaults: R52_INTEGRATION_10RUN.{json,log}
  5. Round label = R52_INTEGRATION
...
# Aiter shim paths (4 override cells; all reuse same R50D shim + same .co + same kernel symbol)
...
AITER_CO = "/shared_nfs/kyle/test/aiter/hsa/gfx950/f4gemm/f4gemm_bf16_per1x32Fp4_BpreShuffle_256x256.co"
AITER_KNAME = "_ZN5aiter42f4gemm_bf16_per1x32Fp4_BpreShuffle_256x256E"
AITER_SHAPES = {
...
# (M, N, K, competitor_TFLOPS) — same 42 shapes as R44_INTEG
ALL_SHAPES = [
...
# ---------------------------------------------------------------------------
# Standard HipKittens runner — matches bench_all_42_R44_INTEG.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Aiter shim runner — for (4096, 32768, 28672). Adapted from bench_R50D.py.
# Uses aiter's preshuffle utilities (DIFFERENT from HipKittens prep).
# ---------------------------------------------------------------------------
def make_aiter_runner_script(m, n, k, comp, seed):
    return f"""\
...
def bench_one_shape(m, n, k, comp, so_path, source, gpu_id, seed):
    ...
    if is_aiter:
        ...
        if not os.path.exists(AITER_CO):
            return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                    "comp": comp, "status": "MISSING_CO", "kernel_finite": None,
                    "source": source, "so_path": AITER_CO, "seed": seed}
        script = make_aiter_runner_script(m, n, k, comp, seed)
    else:
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R52 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def aggregate_consensus(per_run_results, n_runs, comp):
    ...
    # R50 INTEGRATION 10-run @ 80% gate:
    # n_OK_5 >= 8 AND wcf_max < 0.02 AND wcf_std < 0.01 AND fin_min >= 0.97
...
def main():
    ...
    ap.add_argument("--manifest", default="R52_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R52_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R52_INTEGRATION_10RUN.log")
    args = ap.parse_args()
    ...
    log(f"R52 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
