#!/usr/bin/env python3
"""R24B smoke bench (warmup=200 iters=500 trim=10%) — L2-only prefetch hint.

Per benchmark-rules.md MANDATORY: warmup=200, iters=500, 10% trim.
GPUs 0,1,2,4,6,7 (skip 5 - mxfp8 sister project, skip 3 - busy).
"""
...
def main():
    ...
    print(f"R24B smoke: {len(args_list)} bench jobs (3 shapes x {len(VARIANT_SUFFIXES)} variants), GPUs {GPU_POOL}")
    ...
    with ProcessPoolExecutor(max_workers=len(GPU_POOL)) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if "error" in res:
                print(f"  {lab:6s} {vs:20s} gpu={gpu}  ERR {res.get('error')}", flush=True)
            else:
                print(f"  {lab:6s} {vs:20s} gpu={gpu}  {res['tflops']:>8.2f} TFLOPS", flush=True)

    print(f"\nElapsed: {time.time()-t0:.1f}s")
    with open(os.path.join(SCRIPT_DIR, "bench_round24_optB_smoke.json"), "w") as f:
        json.dump(out, f, indent=2)

    # Summary: deltas vs each shape's _r24b_baseline
    print("\n" + "=" * 120)
    print("SUMMARY (Δ vs _r24b_baseline, +1.5pp gate)")
    print("=" * 120)
    for (lab, *_rest) in SHAPES:
        b = out["shapes"][lab].get("_r24b_baseline", {})
        bt = b.get("tflops")
        if bt is None:
            print(f"  {lab}: BASELINE FAILED  ({b.get('error', 'no result')})")
            continue
        print(f"  {lab} baseline = {bt:.2f} TFLOPS")
        for vs in VARIANT_SUFFIXES:
            if vs == "_r24b_baseline":
                continue
            v = out["shapes"][lab].get(vs, {})
            vt = v.get("tflops")
            if vt is None:
                print(f"    {vs:20s}  ERR  ({v.get('error', 'no result')})")
                continue
            d = vt - bt
            d_pp = d / bt * 100
            verdict = "WIN" if d_pp >= 1.5 else ("borderline" if d_pp >= 0 else "LOSE")
            print(f"    {vs:20s}  {vt:>8.2f}  Δ={d:+.2f} ({d_pp:+.2f}%)  {verdict}")
