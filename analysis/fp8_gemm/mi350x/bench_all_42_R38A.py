#!/usr/bin/env python3
"""R38 Opt A: bench every shape's _R38A module with correctness gate.

Same harness as bench_all_42_R37.py but loads from build_R38A/ and uses _R38A suffix.
"""
import json
import os
import subprocess
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_R38A"
        return out
...
def main():
    ...
    print(f"R38A optA Correctness-Gated Bench - GPUs: {gpus}")
    ...
    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R38_optA.json")
