#!/usr/bin/env python3
"""R44 INTEGRATION reviewer bench — FINITE_GATE = 0.97 + INDEPENDENT random seeds.

Fork of bench_all_42_R44D.py with these reviewer-required changes:
  1. RANDOM_SEED varies per run (RANDOM_SEEDS = [101, 202, 303, 404, 505])
     to avoid the "fixed seed=42 across all 5 runs" trap that R42 hit.
  2. Output defaults to R44_INTEGRATION_5RUN.json
  3. round label = R44_INTEGRATION

Bench MANDATORY: warmup=200, iters=500, trim=0.10. ALL 8 GPUs in parallel.
Verified-correct gate (per Opt D promotion rule, n_OK >= 4):
  n_OK >= 4 AND wcf_max < 0.02 AND wcf_std < 0.01 AND fin_min >= 0.97
"""
import json
import math
import os
...
FINITE_GATE = 0.97  # R44 Opt D: relaxed from 0.98 (cohort-race tail; +3 VC)
RANDOM_SEEDS = [101, 202, 303, 404, 505]  # INDEPENDENT seeds per run
...
def bench_one_shape(m, n, k, comp, so_path, source, gpu_id, seed):
    ...
    if r.returncode != 0:
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                ...
                "source": source, "stderr_tail": r.stderr[-200:], "seed": seed}
    ...
    try:
        ...
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                ...
                "source": source, "stdout_tail": r.stdout[-200:], "seed": seed}


def run_one_pass(gpus, run_label, jobs, seed):
    """jobs: list of (m,n,k,comp,so_path,source) tuples."""
    print(f"R44 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def aggregate_consensus(per_run_results, n_runs, comp):
    """Compute 5-run consensus per shape with R44 promotion gate (n_OK >= 4)."""
    ...
    # R44 reviewer rule (Opt D promotion gate carries to 0.97 floor):
    # verified_correct = n_OK >= 4 AND wcf_max < 0.02 AND wcf_std < 0.01 AND fin_min >= 0.97
    verified_correct = (
        n_OK >= 4
    ...
    # Also compute the 'liberal' rule (n_OK >= 3) for reporting parity with bench_all_42_R44D
    verified_correct_liberal = (
        n_OK >= 3
        and wcf_max is not None and wcf_max < WRONG_CELL_GATE
        and wcf_std < 0.01
        and fin_min is not None and fin_min >= FINITE_GATE
    ...
    return {
        ...
        "verified_correct_liberal": verified_correct_liberal,
        "comp": comp,
...
def main():
    ...
    ap.add_argument("--manifest", default="R44_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R44_INTEGRATION_5RUN.json")
    args = ap.parse_args()
    ...
    print(f"R44 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
    print(f"  warmup={WARMUP}, iters={ITERS}, trim={TRIM}, seeds={RANDOM_SEEDS[:args.runs]}")
    print(f"  gates: snr_med>={SNR_THRESHOLD_DB}dB, wcf<{WRONG_CELL_GATE}, fin>={FINITE_GATE}")
    print(f"  promote rule: n_OK>=4 AND wcf_max<0.02 AND wcf_std<0.01 AND fin_min>=0.97")
    ...
    n_verified_liberal = sum(1 for v in consensus.values() if v["verified_correct_liberal"])
    n_win = sum(1 for v in consensus.values()
                if v["pct_comp"] is not None and v["pct_comp"] >= 100.0)

    print(f"\n=== R44 INTEGRATION SUMMARY ===")
    print(f"Verified-correct (n_OK>=4): {n_verified}/{len(jobs)}")
    print(f"Verified-correct (n_OK>=3): {n_verified_liberal}/{len(jobs)}")
