...
// ── R34-OptB: VGPR-staged prefetch helper (mimics aiter pattern) ──
// Issues a buffer_load_dwordx4 (NO `lds` modifier, NO M0 update) targeting
// a scratch float4 VGPR. The corresponding ds_write_b128 must be issued
// later (after vmcnt drains) to deposit the data into LDS for the consumer.
// This decouples the load pipe from the M0 LDS-write-pointer commit, which
// is the hypothesized cause of HSA_APERTURE_VIOLATION at vmcnt(15) on our
// kernel (per R34_DECIDER_VERDICT.md).
#ifndef VGPR_PF_MODE
#define VGPR_PF_MODE 0
#endif
#ifndef VGPR_PF_N
// Number of B-tile prefetches (out of STEP4_PF_N=8) to route through VGPR.
// Default 4 keeps VGPR pressure manageable (4 * 4 = 16 dwords).
#define VGPR_PF_N 4
#endif

#if VGPR_PF_MODE
__device__ __forceinline__ void emit_one_pf_vgpr(const tile_pf_params& p, int idx, float4 &dst) {
    asm volatile(
...
// Deposit a previously VGPR-prefetched float4 into the LDS slot it would
// have landed in via emit_one_pf.
//
// CRITICAL CORRECTNESS NOTE (R34-OptB-fix-v2):
// `p.lds_addrs[idx]` is a UNIFORM (readfirstlane'd) LDS pointer — it points
// at the BASE of the wave's LDS write region. The original
// `buffer_load_to_lds size=16` instruction takes this uniform pointer and
// writes each lane's 16-byte payload to LDS at `lds_ptr + lane_id*16`
// (the hardware-applied per-lane stride for size-16 LDS-direct loads).
//
// In contrast, `ds_write_b32 %addr, %val` uses %addr as a PER-LANE address.
// To replicate the same LDS layout we must add `lane_id * 16` to the uniform
// base per lane. Without this, all 64 lanes race-write to the same slot.
//
// Deposit 16 bytes (4 dwords) into LDS at the exact position that
// buffer_load_to_lds (size=16) would have used:  M0 + lane_id * 16.
//
// R34-OptB-fix-v5: CORRECT interleaved LDS layout matching buffer_load_to_lds.
//
// CRITICAL CORRECTNESS FIX: buffer_load_dwordx4 ... lds (size=16) writes data
// to LDS in a THREAD-INTERLEAVED pattern, NOT contiguous-per-lane:
//
//   LDS[M0 + dword_idx * (64*4) + lane_id * 4]
//   = LDS[M0 + dword_idx * 256 + lane_id * 4]
//
// where dword_idx = 0..3, lane_id = 0..63.
//
// So lane 0 writes to M0+{0, 256, 512, 768} and lane 1 writes to M0+{4, 260, 516, 772}.
...
// ── R34-OptB fork: VGPR-staged B-tile prefetch variant ──
// Identical body to kpair_32mfma_with_lds_and_pf above, but the FIRST
// `VGPR_PF_N` (default 4) prefetches go through scratch float4 VGPRs
// (no `lds` modifier, no M0 update) and the corresponding ds_write_b128
// is issued at the END of the function (after vmcnt drains via the
// caller's iter-end barrier site).
//
// Caller MUST pass a `float4 b_scratch[8]` whose lifetime extends until
// the iter-end vmcnt drains. The function does the buffer_load at issue
// time and emits the ds_write_b128 at function end (still under vmcnt
// pressure, but with the vmcnt(15)/etc relaxation enabled).
#if VGPR_PF_MODE

template<int PF_N = 8, int VGPR_N = VGPR_PF_N, bool EMIT_BARRIER = false>
__device__ __forceinline__ void kpair_32mfma_with_lds_and_pf_vgprB(
    fp4_floatx4_t acc[16],
    const fp4_intx8_t A[4], const fp4_intx8_t B[4],
    const fp8e8m0_4 a_raw[2], const fp8e8m0_4 b_raw[2],
    float4 &d0, float4 &d1, float4 &d2, float4 &d3,
    ...
    float4 (&b_scratch)[8])
{
    static_assert(VGPR_N >= 0 && VGPR_N <= 8, "VGPR_PF_N must be 0..8");
    KPAIR_SETUP();
    if constexpr (EMIT_BARRIER) {
        asm volatile(MXFP4_STEP3_BARRIER_INST_S2 ::: "memory");
    }
...
// Helper to drain VGPR-staged B-tile prefetches into LDS. Must be called
// AFTER vmcnt has dropped to 0 (or below the VGPR-PF in-flight count) so
...
#define TAIL_SPLIT 0
    ...
    for (int bt = 0; bt + 1 < k_byte_iters; ++bt) {
...
#if VGPR_PF_MODE && !SPREAD_LDS
        // R34-OptB: route the first VGPR_PF_N B-tile prefetches through scratch VGPRs
        // Tests the R34 decider hypothesis that VGPR-direct loads (no M0 update)
        // reduce LDS-write-pointer pressure, allowing vmcnt(15) to be safe at the
        // next iter's BTW site.
        float4 b_scratch[8];
        kpair_32mfma_with_lds_and_pf_vgprB<STEP4_PF_N, VGPR_PF_N>(acc_A1Br, tA1, tBr, a1_raw, br_raw,
            nxt_bl_d[0], nxt_bl_d[1], nxt_bl_d[2], nxt_bl_d[3],
            nxt_bl_d[4], nxt_bl_d[5], nxt_bl_d[6], nxt_bl_d[7],
            sel_bl_p0, sel_bl_p1, pf_bl_p, pf_br_p, b_scratch);
#if STEP4_EXTERNAL_BR_PREFETCH
        #pragma unroll
        for (int pi = 0; pi < PF_MPT; ++pi) emit_one_pf(pf_br_p, pi);
#else
        emit_pf_tail<STEP4_PF_N>(pf_bl_p, pf_br_p);
#endif
        // Drain only the VGPR-staged loads; LDS-direct loads can stay in flight
        // (they share the same vmcnt counter, but the ds_write src dependency
        // is on the float4 VGPR destinations only). vmcnt(N) where N is the
        // count of loads that may still be in flight after this point — for
        // VGPR_PF_N=4 + 4 LDS-direct in step4 + 8 LDS-direct from prior step3 = 16,
        // so vmcnt(12) lets ~4 LDS-direct loads still be outstanding while
        // the 4 VGPR-PF loads are forced complete.
#ifndef VGPR_PF_DRAIN_VMCNT
// Drain to vmcnt=0 by default for V0 sanity. V1+ may relax to higher
// values to test the M0-relief hypothesis at the next iter's BTW site.
#define VGPR_PF_DRAIN_VMCNT 0
#endif
        // Only drain and deposit when VGPR_PF_N > 0; at N=0, all prefetches
        // go through LDS-direct (tracked by vmcnt but don't need explicit drain
        // before iter-end lgkmcnt(0) — matching the legacyfork path).
#if VGPR_PF_N > 0
        asm volatile("s_waitcnt vmcnt(" MXFP4_STR(VGPR_PF_DRAIN_VMCNT) ")" ::: "memory");
        emit_b_scratch_dswrites(pf_bl_p, pf_br_p, b_scratch, VGPR_PF_N);
        // The emit_one_pf calls inside emit_one_pf_dswrite re-issue
        // buffer_load_to_lds (VMEM), so we need vmcnt(0) again to
        // ensure those loads complete before the iter-end lgkmcnt(0).
