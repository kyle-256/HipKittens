#!/usr/bin/env python3
"""R54 Opt E1_1 — bench harness for the aiter `.co` dlopen shim on (32768, 4096, 2048).

Reuses R50D shim AS-IS (256x256 tile, bdx=256). Only shape constants change.
Cohort-race rescue candidate: R52 VC at 102.0%, R53 cohort LOSS PASS_8/10 fin_min=0.956.
...
COMPETITOR_TFLOPS = 3131.8  # from R53_INTEGRATION_10RUN.json consensus for (32768, 4096, 2048)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R54_OPT_E1_1_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R54_OPT_E1_1_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R54_OPT_E1_1_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R54_OPT_E1_1_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R54 Opt E1_1 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
