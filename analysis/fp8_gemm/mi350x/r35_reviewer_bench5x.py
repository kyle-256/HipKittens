"""R35 Reviewer: Single-process preheat + 5x bench for ONE kind (fp8 or mxfp8)
on ONE shape and ONE layout. Bit-identical bench logic to R33/R34 — R31 sclk-fix
carried forward; R34 auto-retry contention defense lives in the orchestrate
script (post-preheat sclk gate; bench itself unchanged).

R32 fix (R31 Reviewer paradigm correction) carried forward in R33:
  rocm-smi -d <N> reads the *kernel-driver* GPU index, not the
  HIP-runtime index. Under ROCR_VISIBLE_DEVICES=$PHYS_GPU + HIP_VISIBLE_DEVICES=0,
  `rocm-smi -d 0` always reports physical GPU0's clocks regardless of which
  GPU the process is using. We now read PHYS_GPU from env and pass that to
  rocm-smi so sclk lines actually reflect the working GPU.

Usage: ROCR_VISIBLE_DEVICES=N HIP_VISIBLE_DEVICES=0 PHYS_GPU=N \\
  python3 r33_reviewer_bench5x.py <kind> <layout> M N K
  kind   = fp8 | mxfp8
  layout = rcr | rrr | crr

Env:
  PHYS_GPU (default 0)  -- kernel-driver GPU index for rocm-smi -d
  N_RUNS  (default 5)
  WARMUP  (default 50)
  ITERS   (default 100)
"""
...
def read_sclk():
    try:
        out = subprocess.check_output(
            ["rocm-smi", "--showclocks", "-d", str(phys_gpu)], stderr=subprocess.DEVNULL
        ).decode()
        for line in out.splitlines():
            if "sclk clock level" in line:
                return f"phys_gpu={phys_gpu} {line.strip()}"
    except Exception as e:
        return f"sclk-read-err: {e}"
    return f"phys_gpu={phys_gpu} sclk-unknown"


print(f"[sclk-pre-preheat] {read_sclk()}", file=sys.stderr, flush=True)

# Preheat: 8-second sustained 16k FP16 matmul to ramp DPM
...
if kind == "mxfp8":
    ...
    if layout == "rcr":
        ...
        fn = lambda: tk.gemm_rcr_pq_v2(A, B, As, Bs, C)
        ...
        As = preshuffle_v2_a(Ase); Bs = preshuffle_v2_b(Bse)
        C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
        fn = lambda: tk.gemm_rrr_pq_v2(A, B, As, Bs, C)
        A_ref = A.float() * expand_row(Ase, K)
        ...
        As = preshuffle_v2_a(Ase); Bs = preshuffle_v2_b(Bse)
        C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
        fn = lambda: tk.gemm_crr_pq_v2(A, B, As, Bs, C)
    ...
    if layout == "rcr":
        A = gen_fp8(M, K); Bt = gen_fp8(N, K)
        C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
        fn = lambda: tk.gemm_rcr(A, Bt, C, 1.0)
        C_ref = A.float() @ Bt.float().T
    elif layout == "rrr":
        A = gen_fp8(M, K); B = gen_fp8(K, N)
        C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
        fn = lambda: tk.gemm_rrr(A, B, C, 1.0)
        C_ref = A.float() @ B.float()
    elif layout == "crr":
        At = gen_fp8(K, M); B = gen_fp8(K, N)
        C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")
        fn = lambda: tk.gemm_crr(At, B, C, 1.0)
        C_ref = At.float().T @ B.float()
    else:
        raise SystemExit(f"unknown layout {layout}")
else:
