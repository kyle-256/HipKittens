...
// Issue one buffer_load_dwordx4 via intrinsic (returns __uint128_t = 16 bytes)
__device__ __forceinline__ __uint128_t a_bufload(
    i32x4 srd, uint32_t voff, uint32_t soff) {
    return llvm_amdgcn_raw_buffer_load_b128(srd, voff, soff, 0);
}

// Pack 8 __uint128_t loads (lo0,hi0,lo1,hi1,...) into fp4_intx8_t[4]
__device__ __forceinline__ void pack_a_from_bufload(
    fp4_intx8_t dst[4], const __uint128_t d[8]) {
    #pragma unroll
    for (int i = 0; i < 4; i++) {
        auto lo4 = std::bit_cast<fp4_intx4_t>(d[i*2]);
        auto hi4 = std::bit_cast<fp4_intx4_t>(d[i*2+1]);
        dst[i][0]=lo4[0]; dst[i][1]=lo4[1]; dst[i][2]=lo4[2]; dst[i][3]=lo4[3];
        dst[i][4]=hi4[0]; dst[i][5]=hi4[1]; dst[i][6]=hi4[2]; dst[i][7]=hi4[3];
    }
}
...
// Inputs: KPAIR + voff(VGPR), srd(SGPR×4), soff0..soff7(SGPR each)
...
void mxfp4_gluon_cpp_kernel(const gluon_globals g) {
    ...
    // Direct-A: A tiles loaded via buffer_load_dwordx4, not LDS
    // Only B tiles use LDS (halving LDS footprint: 128KB → 64KB)
...
#define GROUP_SIZE_M 4
    ...
    // ── Direct-A: per-thread voffset and per-warp soffset bases ──
    const int lid = kittens::laneid();
    const uint32_t a_voff = static_cast<uint32_t>(lid % 16) * K_STRIDE
                          + static_cast<uint32_t>(lid / 16) * 16;
    // Scalar soffset bases for A0 and A1 tiles (row offset in bytes)
    const uint32_t a0_row_soff = __builtin_amdgcn_readfirstlane(
        static_cast<uint32_t>((br * BLK + wm * RBM) * K_STRIDE));
    const uint32_t a1_row_soff = __builtin_amdgcn_readfirstlane(
        static_cast<uint32_t>((br * BLK + HB + wm * RBM) * K_STRIDE));

    // ── B tiles: LDS infrastructure (unchanged) ──
    ...
    // B LDS addresses (8 named variables: bl/br × db0/db1 × p0/p1)
    ...
    // Tile extraction helper for B (ds_read float4[8] → fp4_intx8_t[4])
    ...
    // ═══════════ Prologue ═══════════
    // Load B tiles to LDS (2-deep pipeline)
    load_b_tiles(0, 0);
    if (k_byte_iters > 1) load_b_tiles(1, 1);

    // Load initial A tiles directly to VGPRs
    fp4_intx8_t tA0[4], tA1[4];
    load_a_tile_direct(tA0, srd_a, a_voff, a0_row_soff + 0 * BK);
    load_a_tile_direct(tA1, srd_a, a_voff, a1_row_soff + 0 * BK);
    ...
    // Wait for B LDS prefetch and A direct loads, then load Bl from LDS
    asm volatile("s_waitcnt vmcnt(0)");  // wait for A direct loads + B buffer_load_lds
    __builtin_amdgcn_s_barrier();        // sync B LDS writes across warps
    B_row_reg bl_rt;
    ...
    // tA0 and tA1 already loaded directly to VGPRs above
    ...
    for (int bt = 0; bt < k_byte_iters; ++bt) {
        ...
        // B LDS address selection (no A LDS addresses needed with direct-A)
        ...
        // B prefetch params only (no A prefetch to LDS)
        ...
        // ── Step 1: A0×Bl (32 MFMAs) + ds_read Br ──
        float4 br_d[8];
        kpair_32mfma_with_lds(acc_A0Bl, tA0, tBl,
            a0_raw, bl_raw,
        ...
        // br_d is dead after extraction — compiler can reuse those 32 VGPRs

        // ── Step 2: A0×Br (32 MFMAs) + 8 buffer_load for nxt_a0 (inline ASM) ──
        const uint32_t nxt_k_soff = static_cast<uint32_t>(
            (bt + 1 < k_byte_iters ? bt + 1 : bt) * BK);
        float4 nxt_a0_d[8];
        {
            uint32_t soffs[8];
            compute_a_soffs(soffs, a0_row_soff, nxt_k_soff);
            kpair_32mfma_with_vmem(acc_A0Br, tA0, tBr, a0_raw, br_raw,
                nxt_a0_d[0], nxt_a0_d[1], nxt_a0_d[2], nxt_a0_d[3],
                nxt_a0_d[4], nxt_a0_d[5], nxt_a0_d[6], nxt_a0_d[7],
                a_voff, srd_a,
                soffs[0], soffs[1], soffs[2], soffs[3],
                soffs[4], soffs[5], soffs[6], soffs[7]);
        }

        // Barrier for B LDS double-buffer swap (wait for B prefetches + A loads)
        asm volatile("s_waitcnt vmcnt(0)\ns_barrier\n" ::: "memory");

        // Pack nxt_a0 into tA0 for next iteration (nxt_a0_d is dead after this)
        extract_tile(nxt_a0_d, tA0);

        // ── Step 3: A1×Bl (32 MFMAs) + ds_read Bl[nxt] + B prefetches ──
        float4 nxt_bl_d[8];
        ...
        // ── Step 4: A1×Br (32 MFMAs) + 8 buffer_load for nxt_a1 (inline ASM) ──
        float4 nxt_a1_d[8];
        {
            uint32_t soffs[8];
            compute_a_soffs(soffs, a1_row_soff, nxt_k_soff);
            kpair_32mfma_with_vmem(acc_A1Br, tA1, tBr, a1_raw, br_raw,
                nxt_a1_d[0], nxt_a1_d[1], nxt_a1_d[2], nxt_a1_d[3],
                nxt_a1_d[4], nxt_a1_d[5], nxt_a1_d[6], nxt_a1_d[7],
                a_voff, srd_a,
                soffs[0], soffs[1], soffs[2], soffs[3],
                soffs[4], soffs[5], soffs[6], soffs[7]);
        }

        // Wait for Bl[nxt] ds_reads + A1 buffer loads
        asm volatile("s_waitcnt lgkmcnt(0) vmcnt(0)");
        extract_tile(nxt_bl_d, tBl);
        extract_tile(nxt_a1_d, tA1);
    }
...
PYBIND11_MODULE(tk_mxfp4_direct_a, m) {
    m.doc() = "MXFP4 Gluon-arch kernel with direct-A loading";
