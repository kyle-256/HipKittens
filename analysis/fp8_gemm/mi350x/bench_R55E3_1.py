#!/usr/bin/env python3
"""R55 Opt E3_1 — bench harness for the aiter `.co` dlopen shim on (16384, 14336, 2048).
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 16384, 14336, 2048
COMPETITOR_TFLOPS = 3301.3  # from R54_INTEGRATION_10RUN.json consensus for (16384, 14336, 2048)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E3_1_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E3_1_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R55_OPT_E3_1_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R55_OPT_E3_1_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R55 Opt E3_1 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
