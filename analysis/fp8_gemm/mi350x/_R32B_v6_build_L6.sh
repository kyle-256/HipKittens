#!/bin/bash
# Build V6 split-K variants at L6 shape (4096x32768x128256).
# Flags MUST match the L6 incumbent: ts_lgk2_v12_memc_btw_all
...
# Default GROUP_SIZE_M=4 (per kernel default).
set -eo pipefail

KERNEL=/shared_nfs/kyle/test/HipKittens/analysis/fp8_gemm/mi350x/kernel_mxfp4_gluon_cpp_v6.cpp
OUT=/shared_nfs/kyle/test/HipKittens/analysis/fp8_gemm/mi350x/build_v6
mkdir -p "$OUT"
...
COMMON_FLAGS=(
  ...
  -DM_DIM=4096 -DN_DIM=32768 -DK_DIM=128256
  -DTAIL_SPLIT=1
  -DSTEP12_BR_LGKMCNT=2
...
build_one() {
  ...
  /opt/rocm/bin/hipcc "$WRAP" \
    "${COMMON_FLAGS[@]}" \
    "$@" \
    -o "$OUT_SO" 2>&1 | tail -10
  echo "[ok]    $NAME"
}

build_one mxfp4_v6_split1    -DK_SPLIT=1
build_one mxfp4_v6_split2_s0 -DK_SPLIT=2 -DS_IDX=0
build_one mxfp4_v6_split2_s1 -DK_SPLIT=2 -DS_IDX=1
build_one mxfp4_v6_split4_s0 -DK_SPLIT=4 -DS_IDX=0
build_one mxfp4_v6_split4_s1 -DK_SPLIT=4 -DS_IDX=1
build_one mxfp4_v6_split4_s2 -DK_SPLIT=4 -DS_IDX=2
build_one mxfp4_v6_split4_s3 -DK_SPLIT=4 -DS_IDX=3
