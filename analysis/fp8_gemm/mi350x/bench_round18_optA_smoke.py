#!/usr/bin/env python3
"""Round 18 OptA smoke bench — SNR-OK variants only.

SNR probe results (snr_probe_r18a.json):
  DLA1 step12 OK-MARGINAL (16.9 vs 14.6 dB noise floor) — bench
  P1   step12 OK          (32.94 vs 11.78 dB noise floor) — bench
  P1   all    OK          (32.79 vs 11.78 dB noise floor) — bench
  Everything else SNR-BROKEN — skipped.

Per-shape bench parent baseline + each SNR-OK variant on GPU 0 (single-shot).
...
# (lab, M, N, K, comp, parent_suffix, variants_to_bench)
SHAPES = [
    ("DLA1", 4096,  32768, 128256, 5781.1, "_ts_pf6_6_v12_memc",
     ["_r18a_p3_step12"]),
    ("P1",   28672,  4096, 16384, 5350.6, "_ts_gm8",
     ["_r18a_p3_step12", "_r18a_p3_all"]),
]
...
def main():
    t0 = time.time()
    print(f"R18 OptA smoke bench. warmup={WARMUP} iters={ITERS} trim={TRIM}.")
    print("=" * 100)
    jobs = []
    i = 0
    for (lab, M, N, K, comp, ps, variants) in SHAPES:
        # parent baseline
        ...
        for v in variants:
            gpu = GPU_POOL[i % len(GPU_POOL)]
            jobs.append((lab, M, N, K, ps + v, gpu, v))
            i += 1
    results = {}
    with ProcessPoolExecutor(max_workers=len(GPU_POOL)) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(f"  {lab:6s} {label:20s} gpu={gpu} -> {r}", flush=True)
    ...
    for (lab, M, N, K, comp, ps, variants) in SHAPES:
        ...
        for v in variants:
            ...
            print(f"    {v:20s}{t:7.2f}  ({ratio:5.2f}%)  Δ{delta_pp:+.2f}pp{mark}")
            out["shapes"][lab]["variants"][v] = {"tflops": t, "delta_pp": round(delta_pp, 2)}
    with open(os.path.join(SCRIPT_DIR, "bench_round18_optA_smoke.json"), "w") as f:
