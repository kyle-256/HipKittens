#!/usr/bin/env python3
"""R57 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate, ITERS=1000.

Fork of bench_all_42_R56_INTEGRATION.py with these R57 deltas:
  - NO binary changes: 42-shape manifest identical to R56 (39 AITER + 3 HK)
  - Single source-label change: R56G1_L1_AITER -> R57J1_L1_AITER (annotation only)
  - ITERS bumped 500 -> 1000 (one-time R57_OPT_J_LONG_BENCH protocol) for ALL 42 shapes
    Justification: Opt J worker found ITERS=500 puts L1 on noise tail; ITERS=1000
    cuts p50 standard error ~sqrt(2)x and confirmed L1 at 102.37% (vs R56 99.98%).
  - warmup=200, trim=0.10, INDEPENDENT seeds [101..1010] unchanged
  Output defaults: R57_INTEGRATION_10RUN.{json,log}; round label R57_INTEGRATION

ORIG R55 docstring follows for posterity:
  1. RANDOM_SEEDS = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010] (10 seeds).
  2. Promotion gate: n_OK >= 8 (80% of 10) AND wcf_max < 0.02 AND wcf_std < 0.01 AND fin_min >= 0.97
  3. Per-shape backend dispatch — 38 aiter shim cells (all reuse R50D shim AS-IS):
        Carried from R54 (27 cells):
          7 R52-carried 256x256 (R50D / R51D1 / R51D2 / R51D3 / R52D2A / R52D2B / R52D2C)
          2 R53 96x640 (R53D3A_2 / R53D3A_3)
          6 R53 64x1024 (R53D3B_1 / R53D3B_2 / R53D3B_3 / R53D3C_1 / R53D3C_2 / R53D3C_3)
          12 R54 256x256 (E1 x3, E2 x3, D4A x3, D4B x3)
        NEW IN R55 (11 new aiter overrides, all 256x256 tile):
          E-3 cohort-race rescue (M=16384 x N in {14336, 28672}):
            (16384, 14336, 2048)  R55E3_1 (+1 NET VC, 106.58% comp)
            (16384, 14336, 4096)  R55E3_2 (+1 NET VC, 110.86% comp)
            (16384, 28672, 2048)  R55E3_3 (+1 NET VC, 105.28% comp)
            (16384, 28672, 4096)  R55E3_4 (+1 NET VC, 105.86% comp)
          E-4 cohort-race rescue (smaller-N + 6144x32768):
            (16384, 6144, 4096)   R55E4_1 (+1 NET VC, 113.67% comp)
            (6144, 32768, 4096)   R55E4_2 (+1 NET VC, 104.96% comp)
          D-5A marginal HK-VC perf claw-back (M in {16384, 4096}):
            (16384, 4096, 4096)   R55D5A_1 (108.06% vs HK 101.16%)
            (16384, 6144, 2048)   R55D5A_2 (108.00% vs HK 106.19%)
            (4096, 4096, 8192)    R55D5A_3 (117.15% vs HK 100.16%)
          D-5B marginal HK-VC perf claw-back (M=32768):
            (32768, 28672, 2048)  R55D5B_2 (101.94% vs HK 99.68%)
            (32768, 6144, 2048)   R55D5B_3 (107.23% vs HK 103.27%)
       all other 4 shapes -> R40B/R41A baseline manifest entries unchanged
       (kept HK: 32768x14336x2048, 16384x4096x2048, 16384x4096x3072, 4096x128256x32768)
  4. Output defaults: R55_INTEGRATION_10RUN.{json,log}
  5. Round label = R55_INTEGRATION

Bench MANDATORY: warmup=200, iters=1000 (R57 one-time bump), trim=0.10. Configurable GPU subset.
...
WARMUP = 200
ITERS = 1000  # R57 one-time bump from R45+ default 500 (Opt J: ITERS=500 puts L1 on noise tail)
TRIM = 0.10
...
# Aiter shim path (single shim works for all 38 cells; per-shape co/kname/tile vary)
...
# Per-shape aiter dispatch table.
# value = (tile_M, tile_N, co_path, kernel_name, source_tag)
AITER_SHAPES = {
    # R52 carried 256x256 cells
    ...
    # R56 G-1/G-2 PROMOTE: 6 ex-R53 64x1024 cells swapped to 256x256 (eff 60.2 -> 128.0; identical local_round)
    ...
    # R56 G-4 PROMOTE: ex-HK R41A swapped to AITER 256x256 (HK -> AITER; AITER share 38 -> 39)
    (4096, 128256, 32768): (256, 256,  AITER_CO_256,    AITER_KNAME_256,    "R56G4_C1"),
    # R54 256x256 cells (E-1 cohort-race rescue, M-heavy)
    ...
    # R54 256x256 cells (E-2 cohort-race rescue, N-heavy)
    ...
    # R54 256x256 cells (D-4A sub-90 HK-VC perf claw-back)
    ...
    # R54 256x256 cells (D-4B 90-95 marginal claw-back)
    ...
    # R55 NEW 256x256 cells (E-3 cohort-race rescue, M=16384 x N in {14336,28672})
    ...
    # R55 NEW 256x256 cells (E-4 cohort-race rescue, smaller-N + 6144x32768)
    (16384, 6144, 4096):   (256, 256,  AITER_CO_256,    AITER_KNAME_256,    "R55E4_1"),
    (6144, 32768, 4096):   (256, 256,  AITER_CO_256,    AITER_KNAME_256,    "R55E4_2"),
    # R55 NEW 256x256 cells (D-5A marginal HK-VC perf claw-back, M in {16384,4096})
    ...
    # R55 NEW 256x256 cells (D-5B marginal HK-VC perf claw-back, M=32768)
...
# ---------------------------------------------------------------------------
# Standard HipKittens runner — matches bench_all_42_R54_INTEGRATION.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Aiter shim runner — generic (per-shape co/kname/tile_M/tile_N injected at script gen time).
# ---------------------------------------------------------------------------
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R57 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R57_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R57_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R57_INTEGRATION_10RUN.log")
    ap.add_argument("--seeds", nargs="+", type=int, default=None,
                    help="optional override of RANDOM_SEEDS; len must >= --runs")
    args = ap.parse_args()
    gpus = [int(g) for g in args.gpus.split(",")]
    seeds = args.seeds if args.seeds else RANDOM_SEEDS
    assert args.runs <= len(seeds), f"runs={args.runs} exceeds {len(seeds)} seeds"
    ...
    log(f"R57 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
    log(f"  warmup={WARMUP}, iters={ITERS}, trim={TRIM}, seeds={seeds[:args.runs]}")
    log(f"  gates: snr_med>={SNR_THRESHOLD_DB}dB, wcf<{WRONG_CELL_GATE}, fin>={FINITE_GATE}")
    OK_TH = max(1, int(math.ceil(args.runs * 0.8)))
    log(f"  promote rule: n_OK>={OK_TH}/{args.runs} AND wcf_max<0.02 AND wcf_std<0.01 AND fin_min>=0.97")

    all_runs = []
    for ri in range(args.runs):
        seed = seeds[ri]
        results, elapsed = run_one_pass(gpus, f"run{ri+1}/{args.runs}", jobs, seed)
        all_runs.append({"run": ri + 1, "seed": seed,
                         "elapsed_minutes": round(elapsed / 60, 1),
                         "results": results})
        log(f"\n[run{ri+1}/{args.runs}] seed={seed} elapsed={elapsed/60:.1f} min")
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "round": "R57_INTEGRATION",
            "warmup": WARMUP, "iters": ITERS, "trim_frac": TRIM,
            "snr_threshold_db": SNR_THRESHOLD_DB,
            "wrong_cell_gate": WRONG_CELL_GATE,
            "finite_gate": FINITE_GATE,
            "random_seeds": seeds[:args.runs],
            "n_shapes": len(jobs),
            "n_runs": args.runs,
            "ok_threshold": OK_TH,
            "gpus": gpus,
    ...
    log(f"\n=== R57 INTEGRATION SUMMARY ===")
    log(f"Verified-correct (n_OK>={OK_TH}/{args.runs}): {n_verified}/{len(jobs)}")
    log(f"WIN (>=100% comp): {n_win}/{len(jobs)}")
