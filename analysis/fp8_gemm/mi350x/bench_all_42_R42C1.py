#!/usr/bin/env python3
"""R42 Opt C1 bench: 42 shapes using R42C_C1_BUILD_MANIFEST.json.

Clone of bench_all_42_R41_INTEGRATION.py but reads the R42C C1 manifest.
Same gates, same warmup/iters/trim/seed.
"""
import json
import math
import os
...
def run_one_pass(gpus, run_label, jobs):
    print(f"R42C1 Bench [{run_label}] - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R42C_C1_BUILD_MANIFEST.json")
    ap.add_argument("--out", default="R42_OPT_C_C1_SMOKE.json")
    args = ap.parse_args()
    ...
    print(f"R42C1 bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
