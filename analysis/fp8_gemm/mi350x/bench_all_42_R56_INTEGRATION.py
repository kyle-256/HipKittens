#!/usr/bin/env python3
"""R56 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R55_INTEGRATION.py with these R56 deltas:
  R56 PROMOTEs (7) vs R55:
    G-1 64x1024 -> 256x256 (L1/L2/L6):  4096x32768x14336, 32768x4096x14336, 16384x4096x14336
    G-2 64x1024 -> 256x256 (L3/L4/L5):  128256x32768x4096, 28672x32768x4096, 14336x32768x4096
    G-4 HK   -> AITER 256x256 (L7):     4096x128256x32768
  L8 (4096x32768x128256) ACCEPT_FALLBACK -> KEEP R52D2B 256x256
  Net: 39 AITER + 3 HK = 42 shapes
  Output defaults: R56_INTEGRATION_10RUN.{json,log}; round label R56_INTEGRATION
...
Bench MANDATORY: warmup=200, iters=500, trim=0.10. Configurable GPU subset.
...
AITER_SHAPES = {
    ...
    (4096, 32768, 14336):  (256, 256,  AITER_CO_256,    AITER_KNAME_256,    "R56G1_L1"),
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R56 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R56_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R56_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R56_INTEGRATION_10RUN.log")
    ...
    log(f"R56 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
