...
// Single partial accumulator. No A/B preshuffle; scale tensors pre-transposed.
// Scale layout: a_scale [Kb, M], b_scale [Kb, Nb].
...
__device__ __forceinline__ void scale_fma_zero(
    acc_tile& c, acc_tile& p, const _gl_f32& asc, float bs,
    int ki, int mb, int r16)
{
    #pragma unroll
    for (int ti = 0; ti < 4; ++ti) {
        #pragma unroll
        for (int d = 0; d < 4; ++d) {
            float s = asc[coord<>(ki, mb + ti*16 + r16 + d)] * bs;
            float *p0 = reinterpret_cast<float*>(&p.tiles[ti][0].data[0]);
            float *p1 = reinterpret_cast<float*>(&p.tiles[ti][1].data[0]);
            float *c0 = reinterpret_cast<float*>(&c.tiles[ti][0].data[0]);
            float *c1 = reinterpret_cast<float*>(&c.tiles[ti][1].data[0]);
            c0[d] += p0[d] * s; p0[d] = 0.f;
            c1[d] += p1[d] * s; p1[d] = 0.f;
        }
    }
}
...
void gemm_rcr_blockwise_mfma(const bw_rcr_globals g) {
    ...
    acc_tile cA, cB, cC, cD, p;
    zero(cA); zero(cB); zero(cC); zero(cD); zero(p);
    ...
    const int mt = br*BLK + wm*RBM, mb = br*BLK + HB + wm*RBM;
    ...
    for (int k = 0; k < g.ki; ++k, tic ^= 1, toc ^= 1) {
        ...
        __builtin_amdgcn_s_setprio(1); mma_ABt(p, a_reg, b0_reg, p); __builtin_amdgcn_s_setprio(0);
        scale_fma_zero(cA, p, g.a_scale, bl, k, mt, r16);

        lb(b1_reg, Bs[tic][1], wn);
        asm volatile("s_waitcnt lgkmcnt(0)");
        __builtin_amdgcn_s_setprio(1); mma_ABt(p, a_reg, b1_reg, p); __builtin_amdgcn_s_setprio(0);
        scale_fma_zero(cB, p, g.a_scale, br2, k, mt, r16);

        la(a_reg, As[tic][1], wm);
        asm volatile("s_waitcnt lgkmcnt(0)");
        __builtin_amdgcn_s_setprio(1); mma_ABt(p, a_reg, b0_reg, p); __builtin_amdgcn_s_setprio(0);
        scale_fma_zero(cC, p, g.a_scale, bl, k, mb, r16);

        __builtin_amdgcn_s_setprio(1); mma_ABt(p, a_reg, b1_reg, p); __builtin_amdgcn_s_setprio(0);
        scale_fma_zero(cD, p, g.a_scale, br2, k, mb, r16);
