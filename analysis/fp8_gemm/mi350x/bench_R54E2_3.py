#!/usr/bin/env python3
"""R54 Opt E-2/3 — bench harness for the aiter `.co` dlopen shim on (16384, 4096, 6144).

Reuses R50D shim AS-IS (bdx=256). Tile = 256x256 (aiter heuristic pick).
Cohort-race rescue candidate: R52 VC at 98.5%, R53 cohort LOSS PASS_8/10 fin_min=0.950.
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 16384, 4096, 6144
COMPETITOR_TFLOPS = 4259.9  # from R53_INTEGRATION_10RUN.json for (16384, 4096, 6144)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_E2_3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R54 Opt E-2/3 bench — mode={args.mode}, gpu={gpu_list}, "
