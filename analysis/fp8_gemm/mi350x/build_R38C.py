#!/usr/bin/env python3
"""R38 Opt C: Build BEST_VARIANTS with R37_FIX_B + R38C_TAIL_L2ONLY both ON.

Fix B3 — route tail-iter prefetches through emit_full_pf_l2only<>:
  - same buffer_load_dwordx4 GMEM addresses as the original LDS prefetch,
  - destination is a scratch VGPR (discarded, no LDS write),
  - keeps the compiler-tracked vmcnt in sync with the iter scheduler footprint,
  - so no s_barrier-vs-load race → no CRASH.
  - And no per-iter LDS double-buffer slot overwrite → no R38B perf cost.

Targets the 9 R37 CRASH shapes + the 14 R37 WIN shapes (the latter to verify
no regression).  Strips `-mllvm -amdgpu-sched-strategy=max-memory-clause`
(same as R37/R38B builders).

Module names use a `_R38C` suffix to keep .so files distinct.
"""
import argparse
import json
import os
...
BUILD_DIR = os.path.join(SCRIPT_DIR, "build_R38C")
...
# Same 42-shape list and BEST_VARIANTS table used by R37/R38B.
ALL_SHAPES = [
...
def build_one(n_dim, k_dim, parent_tag, base_cppflags):
    module_name = f"tk_mxfp4_gluon_cpp_n{n_dim}_k{k_dim}_{parent_tag}_R38C"
    out_file = f"{module_name}{EXT_SUFFIX}"
    out_path = os.path.join(BUILD_DIR, out_file)
    key = f"N={n_dim},K={k_dim},{parent_tag}_R38C"
    ...
    wrapper_src = os.path.join(BUILD_DIR, f"wrap_n{n_dim}_k{k_dim}_{parent_tag}_R38C.cpp")
    with open(wrapper_src, "w") as f:
        f.write(patched)

    extra_cppflags = strip_bad_sched(base_cppflags)
    # R38C_TAIL_L2ONLY=1 in addition to R37_FIX_B=1 (already on by default in source).
    # R38B_TAIL_FIX is left at its default 0 (mutually exclusive with R38C).
    extra_cppflags = "-DR38C_TAIL_L2ONLY=1 " + extra_cppflags
...
# 9 CRASH shapes from R37_LEADERBOARD.md (primary deliverable target).
CRASH_SHAPES = [
    (4096,  32768,  6144),
    (14336, 32768,  4096),
    (16384,  4096,  3072),
...
# 14 R37 WIN shapes (need to verify no regression).
WIN_SHAPES = [
    (4096,    4096,  8192),
    (4096,    4096, 16384),
    (4096,   14336,  8192),
...
def main():
    ...
    print(f"R38C builder: {len(plan)} shape entries, {len(unique_tasks)} unique builds")
    print(f"  (R37_FIX_B=1 + R38C_TAIL_L2ONLY=1; sched-strategy stripped)")
    ...
    manifest = {
        ...
        "shapes_to_R38C": {
            f"{m}x{n}x{k}": f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38C"
    ...
    with open(os.path.join(SCRIPT_DIR, "R38C_BUILD_MANIFEST.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print("Manifest: R38C_BUILD_MANIFEST.json")
