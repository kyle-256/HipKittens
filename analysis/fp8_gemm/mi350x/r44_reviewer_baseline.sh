#!/bin/bash
# R44 Reviewer Phase 1: 14th-cycle 4-GPU baseline 70B-KV V2-CRR (M=4096 N=1024 K=8192)
# Locks to GPU 2/3/6/7 per project standard.
# Reuses R43 baseline .so (r43_70bkv_baseline.so, built with -DM_DIM=4096 -DN_DIM=1024 -DK_DIM=8192).
# Implements R36 3-gate retry per R43 NEW rule 1 (mandatory).
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
cd "$HERE"

OUTDIR="$HERE/r44_reviewer_4gpu_runs"
mkdir -p "$OUTDIR"
...
echo "===== R44 Reviewer Phase 1 baseline cell=$CELL phys_gpu=$PHYS_GPU =====" | tee "$OUT"
date | tee -a "$OUT"
