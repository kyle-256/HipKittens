#!/usr/bin/env python3
"""R56 Opt G-1 / L6 — bench harness for the aiter `.co` dlopen shim on (16384, 4096, 14336).

R56 G-1: very-high-confidence 64x1024 -> 256x256 swap on the 64x1024 LOSE cluster.
L6 current: R53D3C_3_AITER (tile=64x1024, pct_comp=90.28%).
Alt: 256x256 via R50D shim AS-IS.

Per-shape grid: gdx=ceil(N/256)=16, gdy=ceil(M/256)=64, gdz=1, bdx=256
"""
import argparse
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G1_L6")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-1 L6 bench - mode={args.mode}, gpu={gpu_list}, "
