#!/bin/bash
# R39 Reviewer Phase 2: SHIP RECONFIRM of R37 still-pending items
#
# Cells:
...
OUTDIR="$HERE/r39_reviewer_phase2"
mkdir -p "$OUTDIR"
...
echo "===== R39 Reviewer SHIP RECONFIRM cell=$CELL label=$LABEL M=$M N=$N K=$K phys_gpu=$PHYS_GPU mode=$MODE =====" | tee "$OUTLOG"
date | tee -a "$OUTLOG"
...
if [ "$MODE" = "single_so" ]; then
  ...
  MODNAME_A="tk_mxfp8_r39rev_${CELL}_default"
  MODNAME_B="tk_mxfp8_r39rev_${CELL}_b1"
  ...
  if [ ! -f "$SO_A" ]; then
    ...
    rm -f tk_mxfp8_r39rev_${CELL}_default*.so
  ...
  if [ ! -f "$SO_B" ]; then
    ...
    rm -f tk_mxfp8_r39rev_${CELL}_b1*.so
  ...
  # R44 Dev D: NOTE — the R39 findings.md claims this script has R36 3-gate
  # retry, but inspection shows no retry loop (single attempt). DEPRECATED:
