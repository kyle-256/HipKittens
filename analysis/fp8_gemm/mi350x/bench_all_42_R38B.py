#!/usr/bin/env python3
"""R38 OptB: bench every shape's _R38B module with correctness gate.

Same harness as bench_all_42_correct.py but loads from build_R38B/ and uses _R38B suffix.
"""
import json
import os
import subprocess
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_R38B"
        return out
...
def main():
    ...
    print(f"R38 OptB Correctness-Gated Bench - GPUs: {gpus}")
    ...
    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R38B_fixB.json")
