#!/usr/bin/env python3
"""R38 Opt B: Build BEST_VARIANTS with R37_FIX_B + R38B_TAIL_FIX both ON.

Targets the 9 R37 CRASH shapes. Strips `-mllvm -amdgpu-sched-strategy=
max-memory-clause` (same as R37 builder).

Module names use a `_R38B` suffix to keep .so files distinct from R37.
By default builds ONLY the 9 CRASH shapes' variants. Pass --all to build all 42.
"""
import argparse
import json
import os
...
def build_one(n_dim, k_dim, parent_tag, base_cppflags):
    module_name = f"tk_mxfp4_gluon_cpp_n{n_dim}_k{k_dim}_{parent_tag}_R38B"
    out_file = f"{module_name}{EXT_SUFFIX}"
    out_path = os.path.join(BUILD_DIR, out_file)
    key = f"N={n_dim},K={k_dim},{parent_tag}_R38B"
    ...
    wrapper_src = os.path.join(BUILD_DIR, f"wrap_n{n_dim}_k{k_dim}_{parent_tag}_R38B.cpp")
    with open(wrapper_src, "w") as f:
        f.write(patched)

    extra_cppflags = strip_bad_sched(base_cppflags)
    # R38B_TAIL_FIX=1 in addition to R37_FIX_B=1 (already on by default in source).
    extra_cppflags = "-DR38B_TAIL_FIX=1 " + extra_cppflags
...
def main():
    ...
    ap.add_argument("--shape", type=str, default=None,
                    help="Build only this shape, e.g. 16384x6144x2048")
    args = ap.parse_args()
    ...
    print(f"R38B builder: {len(plan)} shape entries, {len(unique_tasks)} unique builds")
    print(f"  (R37_FIX_B=1 + R38B_TAIL_FIX=1; sched-strategy stripped)")
    ...
    manifest = {
        ...
        "shapes_to_R38B": {
            f"{m}x{n}x{k}": f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38B"
    ...
    with open(os.path.join(SCRIPT_DIR, "R38B_BUILD_MANIFEST.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print("Manifest: R38B_BUILD_MANIFEST.json")
