#!/usr/bin/env python3
"""R60 INTEGRATION reviewer bench — Opt Y cohort-race surface re-measurement.

R60 Opt Y — cohort-race surface re-measurement; manifest byte-identical to R59;
seed set disjoint from R59 [101..1010].

R60 deltas vs bench_all_42_R59_INTEGRATION.py (THIS FILE IS A COPY OF THE R59
SCRIPT WITH ONLY THE FOLLOWING CHANGES):
  - SEED_LIST default switched from [101,202,...,1010] to
    [202, 404, 606, 808, 1010, 1212, 1414, 1616, 1818, 2020] (R60 disjoint set).
  - Output basenames R59_INTEGRATION_* -> R60_INTEGRATION_* (json, log, console).
  - Round label R59_INTEGRATION -> R60_INTEGRATION.
  - manifest path default REMAINS R59_INTEGRATION_MANIFEST.json (READ-ONLY;
    R60 binary state IS R59 binary state by definition for Opt Y).
  - All other parameters (warmup=200, ITERS=500, trim=0.10, GPUs 4,5,6,7,
    AITER dispatch table, gate constants) are IDENTICAL to R59.
...
FINITE_GATE = 0.97
RANDOM_SEEDS = [202, 404, 606, 808, 1010, 1212, 1414, 1616, 1818, 2020]
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R60 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    if args.mode == "smoke1":
        ...
        out = args.out if args.out is not None else "R60_INTEGRATION_SMOKE1.json"
        log = args.log if args.log is not None else "R60_INTEGRATION_SMOKE1.log"
    elif args.mode == "10run":
        runs = args.runs if args.runs is not None else 10
        out = args.out if args.out is not None else "R60_INTEGRATION_10RUN.json"
        log = args.log if args.log is not None else "R60_INTEGRATION_10RUN.log"
    else:
        runs = args.runs if args.runs is not None else 10
        out = args.out if args.out is not None else "R60_INTEGRATION_10RUN.json"
        log = args.log if args.log is not None else "R60_INTEGRATION_10RUN.log"
    ...
    logmsg(f"R60 INTEGRATION bench: {len(jobs)} shapes, runs={runs}, GPUs={gpus}")
