#!/usr/bin/env python3
"""R38 Opt D v2: Bench R38Dv2 modules (FUSED_STEP34=1 OR R37_FIX_B=0 axes)
for the shapes that R38D couldn't recover. Same correctness gate.
"""
import json
import os
import subprocess
...
def bench_one(m, n, k, comp, parent_tag, axis, gpu_id):
    module_name = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{parent_tag}_R38Dv2_{axis}"
    so_path = os.path.join(BUILD_DIR, f"{module_name}{EXT_SUFFIX}")

    if not os.path.exists(so_path):
        return {"M": m, "N": n, "K": k, "tflops": None, "avg_ms": None,
                "comp": comp, "status": "MISSING_SO", "kernel_finite": None,
                "variant": parent_tag, "axis": axis}
...
def main():
    ...
    with open(os.path.join(SCRIPT_DIR, "R38Dv2_BUILD_MANIFEST.json")) as f:
        manifest = json.load(f)

    # Build a flat task list
    tasks = []
    seen = set()
    for entry in manifest["shapes_to_R38Dv2"]:
        m, n, k, tag, axis = entry["M"], entry["N"], entry["K"], entry["tag"], entry["axis"]
        if (m, n, k, tag, axis) in seen:
            continue
        seen.add((m, n, k, tag, axis))
        # Look up comp from candidates file
        tasks.append((m, n, k, tag, axis))

    # Get comp from R38D candidates
    with open(os.path.join(SCRIPT_DIR, "R38D_candidates.json")) as f:
        cands = json.load(f)
    comp_lookup = {(info["M"], info["N"], info["K"]): info["comp"]
                   for info in cands["shapes"].values() if info.get("status") == "have_candidates"}

    print(f"R38Dv2 Bench - GPUs: {gpus}")
    ...
    for i, t in enumerate(tasks):
        m, n, k, tag, axis = t
        gpu = gpus[i % len(gpus)]
        gpu_tasks[gpu].append((i, m, n, k, comp_lookup[(m, n, k)], tag, axis))
    ...
    def run_gpu(gpu_id):
        for (i, m, n, k, comp, tag, axis) in gpu_tasks[gpu_id]:
            r = bench_one(m, n, k, comp, tag, axis, gpu_id)
            with lock:
                results[i] = r
                if r["status"] == "OK":
                    ratio = r["tflops"] / comp * 100
                    flag = "WIN" if r["tflops"] >= comp else "LOSE"
                    print(f"  [{i+1:>3}/{len(tasks)}] {m:>6}x{n:>6}x{k:>6} {axis} {tag:<46} "
    ...
    threads = [threading.Thread(target=run_gpu, args=(g,)) for g in gpus]
    ...
    # Aggregate per-shape (across both axes)
    per_shape = {}
    for r in results:
    ...
    for (m, n, k), entries in per_shape.items():
        comp = entries[0]["comp"]
        ok = [e for e in entries if e["status"] == "OK" and e.get("kernel_finite", 0) >= CORRECTNESS_GATE]
        ok.sort(key=lambda e: -e["tflops"])
        ...
        if ok:
            ...
            summary[f"{m}x{n}x{k}"] = {
                ...
                "best_variant": best["variant"], "best_axis": best["axis"],
                "best_tflops": best["tflops"],
                "best_ratio_vs_comp": round(best["tflops"]/comp*100, 1),
                "best_finite": best["kernel_finite"],
                "n_correct": len(ok), "n_wrong": len(wrong), "n_crash": len(crash),
                "all_correct_sorted": [
                    {"v": e["variant"], "a": e["axis"], "t": e["tflops"], "f": e["kernel_finite"]} for e in ok
                ],
            }
        else:
            summary[f"{m}x{n}x{k}"] = {
                "M": m, "N": n, "K": k, "comp": comp,
                "best_variant": None, "best_tflops": None,
                "n_correct": 0, "n_wrong": len(wrong), "n_crash": len(crash),
            }

    out_path = os.path.join(SCRIPT_DIR, "bench_all42_results_R38_optDv2.json")
    with open(out_path, "w") as f:
        json.dump({
            ...
            "n_tasks": len(tasks), "n_shapes": len(per_shape),
    ...
    n_correct = sum(1 for v in summary.values() if v["best_variant"])
    n_winning = sum(1 for v in summary.values() if v["best_variant"] and v["best_tflops"] >= v["comp"])
    print(f"Per-shape: {n_correct}/{len(per_shape)} correct; {n_winning} WIN; elapsed={elapsed/60:.1f}min")
