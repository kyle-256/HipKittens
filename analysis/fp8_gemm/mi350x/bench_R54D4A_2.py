#!/usr/bin/env python3
"""R54 Opt D-4A/2 — bench harness for the aiter `.co` dlopen shim on (32768, 4096, 7168).

Reuses R50D shim AS-IS (bdx=256, kd max_flat_workgroup_size=256).
Tile = 256x256 (aiter heuristic pick). Sub-90% HK-VC rescue candidate (HK 88.36%).
...
Per-shape grid:
  gdx = ceil(N/256) = ceil(4096/256)   = 16
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 32768, 4096, 7168
COMPETITOR_TFLOPS = 4666.8  # from R53_INTEGRATION_10RUN.json for (32768, 4096, 7168)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_D4A_2")
