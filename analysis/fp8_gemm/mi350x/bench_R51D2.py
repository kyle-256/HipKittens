#!/usr/bin/env python3
"""R51 Opt D-2 — bench harness for the aiter `.co` dlopen shim on (16384, 4096, 28672).

Reuses the EXISTING R50D shim built at:
  build_R50D/R50D_aiter_shim.cpython-310-x86_64-linux-gnu.so

Adapted from bench_R50D.py: only shape (M, N, K) and competitor TFLOPS change.
...
COMPETITOR_TFLOPS = 5525.3  # from bench_gluon_a4w4_42.py for (16384,4096,28672)
...
def main():
    ...
    ap.add_argument("--smoke-warmup", type=int, default=5)
    ap.add_argument("--smoke-iters", type=int, default=10)
    args = ap.parse_args()

    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D2_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D2_SMOKE.log")
        seeds = [101]
        do_perf = True
        warmup_o, iters_o = args.smoke_warmup, args.smoke_iters
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D2_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D2_10RUN.log")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R51 Opt D-2 bench — mode={args.mode}, gpu={args.gpu}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
