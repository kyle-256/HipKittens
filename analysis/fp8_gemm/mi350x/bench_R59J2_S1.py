#!/usr/bin/env python3
"""R59 Opt J-2/S-1 — 96x640 alt-tile probe on HK cell (32768, 14336, 2048).

R59 J-2/S-1: 96x640 aiter alt-tile via R50D shim AS-IS (HK baseline 100.49% R40B, VC-flipped).
Grid: gdx=ceil(32768/96)=342, gdy=ceil(14336/640)=23, gdz=1, bdx=256.
Hypothesis: low-medium (eff=83.5 vs HK 256x256). PROMOTE only if SMOKE > 101.49% (HK + 1.0pp).
Else: ACCEPT_FALLBACK (axis closure for L3 96x640).
...
KERNEL_NAME = "_ZN5aiter41f4gemm_bf16_per1x32Fp4_BpreShuffle_96x640E"
TILE_M = 96
TILE_N = 640
...
def main():
    ...
    ap.add_argument("--tag", default="R59_OPT_S1")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R59 Opt J-2/S-1 bench - mode={args.mode}, gpu={gpu_list}, "
