...
// Asynchronous A tile load — issues 8 buffer_load_dwordx4 (caller must waitcnt vmcnt)
__device__ __forceinline__ void load_a_direct_async(
    float4 d[8], i32x4 srd, uint32_t voff, uint32_t soff_base) {
    constexpr uint32_t ss = 16 * K_STRIDE;  // subtile stride
    #pragma unroll
    for (int i = 0; i < 4; i++) {
...
void mxfp4_gluon_cpp_kernel(const gluon_globals g) {
    ...
    // Sequential direct-A: A tiles loaded via buffer_load_dwordx4, not LDS
    // Only B tiles use LDS (halving LDS footprint)
...
#define GROUP_SIZE_M 4
    ...
    // Tile extraction helper (float4[8] → fp4_intx8_t[4])
    ...
    // Load initial A0 directly to VGPRs (A1 loaded later in loop)
    fp4_intx8_t tA[4];  // single A tile array — reused for A0 and A1 sequentially
    load_a_tile_direct(tA, srd_a, a_voff, a0_row_soff + 0 * BK);
    ...
    // Wait for B LDS prefetch, then load Bl from LDS
    asm volatile("s_waitcnt vmcnt(0)");
    ...
    // ═══════════ Main loop (sequential direct-A pipeline) ═══════════
    // Key insight: A0 and A1 are NEVER both live in VGPRs.
    // tA[] is reused for both — A0 in Steps 1-2, then A1 in Steps 3-4.
#ifdef UNROLL_K
  #if UNROLL_K == 0
    #pragma unroll
  #else
    ...
    for (int bt = 0; bt < k_byte_iters; ++bt) {
        ...
        // B LDS address selection (no A LDS addresses needed)
        ...
        // tA currently holds A0
        float4 br_d[8];
        kpair_32mfma_with_lds(acc_A0Bl, tA, tBl,
            a0_raw, bl_raw,
        ...
        // Issue A1 buffer_loads — latency hidden by Step 2's 32 MFMAs
        const uint32_t cur_k_soff = static_cast<uint32_t>(bt * BK);
        float4 a1_d[8];
        load_a_direct_async(a1_d, srd_a, a_voff, a1_row_soff + cur_k_soff);

        // ── Step 2: A0×Br (32 MFMAs) — pure, no interleaved loads ──
        kpair_32mfma_pure(acc_A0Br, tA, tBr, a0_raw, br_raw);
        // tA (A0) is dead after this point

        // Wait for A1 buffer_loads
        asm volatile("s_waitcnt vmcnt(0)");
        extract_tile(a1_d, tA);  // tA now holds A1 (reusing same VGPRs!)

        // Barrier for B LDS double-buffer swap
        __builtin_amdgcn_s_barrier();

        // ── Step 3: A1×Bl (32 MFMAs) + B prefetches ──
        kpair_32mfma_with_pf<8>(acc_A1Bl, tA, tBl, a1_raw, bl_raw, pf_bl_p, pf_br_p);

        // Issue A0[nxt] buffer_loads — latency hidden by Step 4's MFMAs
        const uint32_t nxt_k_soff = static_cast<uint32_t>(
            (bt + 1 < k_byte_iters ? bt + 1 : bt) * BK);
        float4 a0_d[8];
        load_a_direct_async(a0_d, srd_a, a_voff, a0_row_soff + nxt_k_soff);

        // ── Step 4: A1×Br (32 MFMAs) + ds_read Bl[nxt] ──
        float4 nxt_bl_d[8];
        kpair_32mfma_with_lds(acc_A1Br, tA, tBr, a1_raw, br_raw,
            nxt_bl_d[0], nxt_bl_d[1], nxt_bl_d[2], nxt_bl_d[3],
            nxt_bl_d[4], nxt_bl_d[5], nxt_bl_d[6], nxt_bl_d[7],
            sel_bl_p0, sel_bl_p1);

        // Wait for A0[nxt] buffer_loads + Bl[nxt] ds_reads
        asm volatile("s_waitcnt vmcnt(0) lgkmcnt(0)");
        extract_tile(a0_d, tA);     // tA now holds A0[nxt]
...
PYBIND11_MODULE(tk_mxfp4_seq_direct, m) {
    m.doc() = "MXFP4 Gluon-arch kernel with sequential direct-A loading";
