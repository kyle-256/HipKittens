"""R35 Dev A — Phase 2 advisory probe.

Loads a built .so and invokes gemm_crr_pq_v2 once on the matching shape.
Captures stderr (the advisory text) so an outer shell can count occurrences
of '[tk_mxfp8_layouts]' lines.

Usage:  python3 r35a_advisory_probe.py <so_path> <M> <N> <K>
"""
...
so = sys.argv[1]
M = int(sys.argv[2]); N = int(sys.argv[3]); K = int(sys.argv[4])
...
# Invoke twice to confirm the static-int "warned once" pattern (advisory should
# print ONLY on the first call).
mod.gemm_crr_pq_v2(A, B, As, Bs, C); torch.cuda.synchronize()
mod.gemm_crr_pq_v2(A, B, As, Bs, C); torch.cuda.synchronize()
print(f"PROBE_INVOKED so={os.path.basename(so)} M={M} N={N} K={K}")
