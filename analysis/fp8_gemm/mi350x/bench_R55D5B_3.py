#!/usr/bin/env python3
"""R55 Opt D-5B/3 — bench harness for the aiter `.co` dlopen shim on (32768, 6144, 2048).
...
HK R54 PASS_10/10 at 103.3% — D-3A-1 risk HIGH. AITER must clearly win.
...
Per-shape grid:
  gdx = ceil(N/256) = ceil(6144/256) = 24
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 32768, 6144, 2048
COMPETITOR_TFLOPS = 3239.9  # from R54_INTEGRATION_10RUN.json for (32768, 6144, 2048)
HK_PCT_COMP = 103.27  # R54 HK PASS_10/10 baseline
...
def main():
    ...
    ap.add_argument("--tag", default="R55_OPT_D5B_3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R55 Opt D-5B/3 bench — mode={args.mode}, gpu={gpu_list}, "
