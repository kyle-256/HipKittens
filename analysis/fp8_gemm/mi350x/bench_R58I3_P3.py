#!/usr/bin/env python3
"""R58 Opt P-3 — 128x256 alt-tile probe on HK cell (32768, 14336, 2048).

R58 I-3 / P-3: 128x256 aiter alt-tile via R50D shim AS-IS (HK baseline 100.53% R57 near-gate).
Hypothesis: low (eff=85.3 vs HK 256x256). PROMOTE only if SMOKE > 101.53% (HK + 1.0pp).
Else: ACCEPT_FALLBACK (axis closure for HK kept-cell pool).
...
def main():
    ...
    ap.add_argument("--tag", default="R58_OPT_P3")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R58 Opt P-3 bench - mode={args.mode}, gpu={gpu_list}, "
