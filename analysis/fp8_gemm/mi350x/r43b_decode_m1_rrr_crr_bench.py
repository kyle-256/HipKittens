"""R43 Dev B — M=1 RRR/CRR single-token decode MXFP8 fastpath bench.

Measures (per layout, per shape):
    (1) baseline-mxfp8: V1-LEGACY-FALLBACK (current production path).
    (2) decode-m1-mxfp8: NEW R43B fastpath for RRR/CRR.
    (3) baseline-fp8:    per-tensor FP8 reference.

Usage:
    python3 r43b_decode_m1_rrr_crr_bench.py <module> <kind> <impl> <layout> M N K
        kind:   mxfp8 | fp8
        impl:   legacy_v1pq | decode_m1_rrr_crr | fp8_pertensor
        layout: rrr | crr | rcr (rcr only valid for fp8 reference)
"""
import math
import os
import random
...
def parse_args():
    if len(sys.argv) < 8:
        print("usage: r43b_decode_m1_rrr_crr_bench.py <module> <kind> <impl> <layout> M N K",
              file=sys.stderr)
        sys.exit(2)
    mod_name = sys.argv[1]
...
def reference_mxfp8(A, B, A_se, B_se, layout):
    """Slow CPU reference for SNR. Always returns C of shape (M, N)."""
    ...
    if layout == "rrr":
        ...
        # B_sf row-axis is N (same per-N-row scale repeats per K), use B as (K,N): scale (N, KB).
        B_sf_full = B_sf.repeat_interleave(32, dim=1)[:, :K]
        A_scaled = Af * A_sf_full
        # B_scaled is (K, N); broadcast scale per-col.
...
def main():
    ...
    if kind == "mxfp8":
        if layout == "rrr":
            ...
            print(f"ERROR: layout {layout} not supported here for mxfp8", file=sys.stderr)
            sys.exit(2)
    ...
    print(f"R43B_RESULT module={mod_name} kind={kind} impl={impl} layout={layout} "
