#!/bin/bash
# R32 Reviewer Phase 1: 4-GPU triangulation for 70B KV V2-CRR (M=4096 N=1024 K=8192)
# Goal: re-verify 766.72 TFLOPS canonical baseline with R31 sclk-fix applied.
#
# R31 paradigm correction APPLIED in this orchestrate:
#   - r32_reviewer_bench5x.py reads PHYS_GPU env var for `rocm-smi -d $PHYS_GPU`
...
#
# Methodology rules (R31 closures, MUST follow):
#   1. rm -f tk_mxfp8_layouts*.so + log per-build md5 (R29 Dev C rule)
#   2. rocm-smi -d $PHYS_GPU not -d 0 (R31 Reviewer rule)
#   3. SHIP claim normalization: discount GPU0 by 2.6% (R31 Reviewer rule)
...
#
# Usage:
#   PHYS_GPU=4 ./r32_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=5 ./r32_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=6 ./r32_reviewer_4gpu_orchestrate.sh
#   PHYS_GPU=0 ./r32_reviewer_4gpu_orchestrate.sh   # last to deconflict with R32 Dev A
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
WT="$(cd "$HERE/../../.." && pwd)"
cd "$HERE"

OUTDIR="$HERE/r32_reviewer_4gpu_runs"
mkdir -p "$OUTDIR"
...
ROCR_VISIBLE_DEVICES=$PHYS_GPU HIP_VISIBLE_DEVICES=0 PHYS_GPU=$PHYS_GPU \
  ...
  python3 r32_reviewer_bench5x.py mxfp8 $LAYOUT $M $N $K 2>&1 | tee -a "$OUT"
