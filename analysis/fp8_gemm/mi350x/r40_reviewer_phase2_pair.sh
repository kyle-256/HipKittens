...
#
# R44 Dev D PATCH (R43 NEW methodology rule 1, MANDATORY, retroactive):
...
run_bench_attempt() {
  local attempt=$1
  BENCH_OUT="$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_attempt${attempt}.txt"
  BENCH_ERR="$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_attempt${attempt}.err"
...
while [ $attempt -le $MAX_RETRIES ]; do
  ...
  if [ $PASS -eq 1 ]; then
    ...
    BENCH_OUT="$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_attempt${attempt}.txt"
    BENCH_ERR="$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_attempt${attempt}.err"
    cp "$BENCH_OUT" "$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_clean.txt"
    cp "$BENCH_ERR" "$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_clean.err"
    cp "$BENCH_OUT" "$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_bench.txt"
    cp "$BENCH_ERR" "$OUTDIR/${CELL}_${LABEL}_gpu${PHYS_GPU}_bench.err"
    break
  fi
