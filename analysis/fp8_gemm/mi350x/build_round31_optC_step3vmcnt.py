#!/usr/bin/env python3
"""R31-OptC-Build: STEP3_BARRIER_VMCNT sweep on L6 (4096x32768x128256).

Parent: ts_lgk2_v12_memc_btw_all (incumbent best at 5353.9 TFLOPS, 92.6% of comp).
Sweep: STEP3_BARRIER_VMCNT in {4, 8, 10, 16, 20, 24}.
...
# L6 best parent flags (from bench_all_42.py:480) — STEP3_BARRIER_VMCNT swept below.
PARENT_FLAGS_NO_VMCNT = (
...
JOBS = []
for v in [4, 8, 10, 16, 20, 24]:
    JOBS.append({
        "tag": f"v{v}",
        "N": N,
        "K": K,
        "suffix": f"_R31C_ts_lgk2_memc_btw_all_v{v}",
        "flags": f"{PARENT_FLAGS_NO_VMCNT} -DSTEP3_BARRIER_VMCNT={v}",
    })
...
def build_one(job):
    ...
    log_path = os.path.join(BUILD_DIR, f"compile_R31C_{tag}_n{N}_k{K}.log")
    t0 = time.time()
...
def main():
    ...
    print(f"R31-C builds: {len(JOBS)} (parallel)")
    print(f"Parent: ts_lgk2_v12_memc_btw_all on L6 (N=32768 K=128256)")
    print(f"Sweeping STEP3_BARRIER_VMCNT in [4, 8, 10, 16, 20, 24]")
    t_global = time.time()
    results = []
    ...
    out_json = os.path.join(SCRIPT_DIR, "R31_OPT_C_BUILD_RESULTS.json")
