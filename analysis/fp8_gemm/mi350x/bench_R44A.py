#!/usr/bin/env python3
"""R44 Opt A — bench new R44A K=28672 variants under random-scale 5-run @ GATE=0.98."""
import json
import math
import os
import re
...
ALL_CELLS = [
    "no_r25c", "R38B_no_r25c", "R38B_R39A_R38F4", "R38B_R39A_R38F2",
    "R38B_pfoff32", "R38B_pfoff8", "R38B_R39A_pfoff32",
]


def find_so(m, n, k, cell):
    mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_ts_lgk2_gm7_kx28672_btw_all_R44A_{cell}"
...
def main():
    ...
    ap.add_argument("--out", default="R44_OPT_A_PHASE1B_5RUN.json")
    args = ap.parse_args()
    ...
    print(f"R44 Opt A Phase 1b: {len(jobs)} jobs across {len(cells)} cells x {len(ALL_SHAPES)} shapes x {len(SEEDS)} seeds")
    print(f"  cells: {cells}")
    print(f"  GPUs: {gpus}")
    print("=" * 110)
    ...
    def run_gpu(gpu_id):
        for (i, (m, n, k, comp, cell, seed)) in gpu_tasks[gpu_id]:
            ...
            with lock:
                ...
                if r["status"] == "OK":
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<22s} seed={seed} {r['tflops']:>7.1f} TF fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
                else:
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<22s} seed={seed} {r['status']} fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
    threads = []
    for g in gpus:
    ...
    for c in consensus:
        ...
        print(f"  {c['M']:>5}x{c['N']:>5}x{c['K']:>5} {c['cell']:<22s} n_OK={c['n_OK_5']}/5 "
