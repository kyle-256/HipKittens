#!/usr/bin/env python3
"""R39 Opt B: bench every shape's _R38E module with **random-scale + SNR** correctness gate.
...
def find_so(m, n, k):
    ...
    # Try a few variant suffixes in priority order: full-tagged > short-tagged
    for tag in [parent_tag]:
        if tag.startswith("v32") or tag.startswith("lgk2_v") or tag.startswith("ts_lgk2_v"):
            mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38E"
        else:
            mod = f"tk_mxfp4_gluon_cpp_n{n}_k{k}_{tag}_R38E"
...
def bench_one_shape(m, n, k, comp, gpu_id):
    ...
    try:
        ...
        out["best_variant"] = parent_tag + "_R38E"
        return out
...
def main():
    ...
    ap.add_argument("--out", default="bench_all42_results_R39_optB.json")
    args = ap.parse_args()
