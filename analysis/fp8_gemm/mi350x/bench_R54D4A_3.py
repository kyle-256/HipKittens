#!/usr/bin/env python3
"""R54 Opt D-4A/3 — bench harness for the aiter `.co` dlopen shim on (6144, 4096, 8192).

Reuses R50D shim AS-IS (bdx=256, kd max_flat_workgroup_size=256).
Tile = 256x256 (aiter heuristic pick). Sub-90% HK-VC rescue candidate (HK 89.53%).
...
Per-shape grid:
  ...
  gdy = ceil(M/256) = ceil(6144/256) = 24
  gdz = 1, bdx = 256
"""
import argparse
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 6144, 4096, 8192
COMPETITOR_TFLOPS = 3822.0  # from R53_INTEGRATION_10RUN.json for (6144, 4096, 8192)
...
def main():
    ...
    ap.add_argument("--tag", default="R54_OPT_D4A_3")
