#!/usr/bin/env python3
"""R44 Opt A Phase 1 (preflight) — sweep ALL existing R42B Phase-2B cells
under random-scale 5-run consensus at GATE=0.98 on both K=28672 shapes.

This re-uses the already-built .so files in build_R42B_phase2b/, sweeping cells:
  nf_R38B           (Phase 0, already known: FAIL)
  nf_R38F2          (drain only, no R38B)
  nf_R38B_R38F2     (R38B + drain F2)
  nf_R38B_R38F4     (R38B + drain F4)
  nf_R38B_R39A      (R38B + scale clamp)

Bench rules: warmup=200, iters=500, trim=0.10. Fresh seeds per run.
"""
import json
import math
import os
...
# Skip nf_R38B (already done in Phase 0); test the rest
ALL_CELLS = ["nf_R38F2", "nf_R38B_R38F2", "nf_R38B_R38F4", "nf_R38B_R39A"]
...
def find_so(m, n, k, cell):
    ...
    return (mod, path) if os.path.exists(path) else (None, None)


# Same script template as bench_R44A_phase0.py (random scales, fresh seed)
...
def dequant_fp4_rows(packed_uint8, K, row_lo, row_hi):
    ...
    out[:, 0::2] = FP4_TBL[lo]; out[:, 1::2] = FP4_TBL[hi]
    return out[:, :K]
...
try:
    ...
    C0 = C.clone()
    finite_frac = round(float(torch.isfinite(C0.float()).sum().item()) / float(C0.numel()), 6)
    m_rows = min(M, 1024); n_rows = min(N, 1024)
    A_blk = dequant_fp4_rows(A, K, 0, m_rows); B_blk = dequant_fp4_rows(B, K, 0, n_rows)
    A_scaled = apply_scales_rows(A_blk, sc_a, K, 0, m_rows)
    B_scaled = apply_scales_rows(B_blk, sc_b, K, 0, n_rows)
    del A_blk, B_blk
    C_ref = torch.matmul(A_scaled, B_scaled.T); del A_scaled, B_scaled
    C_test = C0[:m_rows, :n_rows].float()
    base_mask = torch.isfinite(C_ref) & torch.isfinite(C_test)
    REL_TOL = 0.05; ABS_TOL = 1e-2
    ...
    if n_valid < 1024:
        ...
        abs_diff = (test_d - ref_d).abs(); abs_ref = ref_d.abs(); abs_test = test_d.abs()
        ...
        if n_good < 1024:
            ...
            sig = float((rv * rv).mean().item()); err = float(((tv - rv) ** 2).mean().item())
    ...
    correct = ((snr_med_db >= SNR_DB_GATE) and (wrong_cell_frac < WRONG_CELL_GATE) and (finite_frac >= FINITE_GATE))
    if not correct:
        out = {{"M": M, "N": N, "K": K, "tflops": None, "avg_ms": None, "comp": COMP,
                ...
                "snr_n_valid": n_valid, "wrong_cell_frac": round(wrong_cell_frac, 6), "seed": {seed}}}
        print("BENCH_JSON_START"); print(json.dumps(out)); print("BENCH_JSON_END"); sys.exit(0)
    ...
    avg = sum(times) / len(times); tflops = 2.0 * M * N * K / (avg * 1e-3) / 1e12
    out = {{"M": M, "N": N, "K": K, "tflops": round(tflops, 1), "avg_ms": round(avg, 4),
            "comp": COMP, "status": "OK", "kernel_finite": finite_frac,
...
def bench_one(m, n, k, comp, cell, seed, gpu_id):
...
def aggregate(per_run, comp):
    ...
    wcf_max = safe_max(wcfs); wcf_std = safe_std(wcfs); fin_min = safe_min(fins)
    verified_correct = (n_OK >= 3 and wcf_max is not None and wcf_max < WRONG_CELL_GATE
                        and wcf_std < 0.01 and fin_min is not None and fin_min >= FINITE_GATE)
    promote = (n_OK >= 4 and wcf_max is not None and wcf_max < WRONG_CELL_GATE
               and wcf_std < 0.005 and fin_min is not None and fin_min >= 0.985)
    return {"n_OK_5": n_OK, "tflops_p50": tflops_p50, "wcf_max": wcf_max,
            "wcf_std": wcf_std, "fin_min": fin_min, "fins": fins, "wcfs": wcfs,
            "verified_correct": verified_correct, "promote": promote,
            "tflops_per_run": [r.get("tflops") for r in per_run],
            "status_per_run": [r.get("status") for r in per_run],
            "pct_comp": (tflops_p50 / comp * 100) if (tflops_p50 is not None) else None}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--gpus", default="0,1")
    ap.add_argument("--out", default="R44_OPT_A_PHASE1_EXISTING_5RUN.json")
    args = ap.parse_args()
    ...
    for (m, n, k, comp) in ALL_SHAPES:
        for cell in ALL_CELLS:
            for seed in SEEDS:
                jobs.append((m, n, k, comp, cell, seed))

    print(f"R44 Opt A Phase 1 (existing-cell sweep): {len(jobs)} jobs across {len(ALL_CELLS)} cells x {len(ALL_SHAPES)} shapes x {len(SEEDS)} seeds")
    ...
    def run_gpu(gpu_id):
        for (i, (m, n, k, comp, cell, seed)) in gpu_tasks[gpu_id]:
            r = bench_one(m, n, k, comp, cell, seed, gpu_id)
            with lock:
                results[i] = r
                done[0] += 1
                fin = r.get("kernel_finite"); fin_s = f"{fin:.4f}" if fin is not None else "N/A"
                wcf = r.get("wrong_cell_frac"); wcf_s = f"{wcf:.4f}" if wcf is not None else "N/A"
                if r["status"] == "OK":
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<16s} seed={seed} {r['tflops']:>7.1f} TF fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
                else:
                    print(f"  [{done[0]:>3}/{len(jobs)}] {m:>5}x{n:>5}x{k:>5} {cell:<16s} seed={seed} {r['status']} fin={fin_s} wcf={wcf_s} GPU{gpu_id}", flush=True)
    threads = []
    for g in gpus:
    ...
    for (m, n, k, comp) in ALL_SHAPES:
        for cell in ALL_CELLS:
            per_run = [r for r in results if r["M"] == m and r["N"] == n and r["K"] == k and r.get("cell") == cell]
            agg = aggregate(per_run, comp)
            agg["M"], agg["N"], agg["K"] = m, n, k
            agg["comp"] = comp
            agg["cell"] = cell
            agg["per_run"] = per_run
            consensus.append(agg)
    out_path = os.path.join(SCRIPT_DIR, args.out)
    with open(out_path, "w") as f:
        json.dump({
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "round": "R44_OPT_A_PHASE1_EXISTING",
            "warmup": WARMUP, "iters": ITERS, "trim_frac": TRIM,
            "snr_threshold_db": SNR_THRESHOLD_DB, "wrong_cell_gate": WRONG_CELL_GATE,
            "finite_gate": FINITE_GATE, "seeds": SEEDS,
            "n_shapes": len(ALL_SHAPES), "n_cells": len(ALL_CELLS), "n_runs": len(SEEDS),
            "gpus": gpus, "elapsed_minutes": round(elapsed / 60, 1),
            "consensus": consensus,
        }, f, indent=2)

    print(f"\n=== R44 Opt A Phase 1 (existing-cell sweep) SUMMARY ===")
    print(f"Elapsed: {elapsed/60:.1f} min")
    for c in consensus:
        flag = "PROMOTE" if c["promote"] else ("VC" if c["verified_correct"] else "FAIL")
        tf = c["tflops_p50"]; tf_s = f"{tf:.1f}" if tf else "N/A"
        wm = c["wcf_max"]; wm_s = f"{wm:.4f}" if wm is not None else "N/A"
        fm = c["fin_min"]; fm_s = f"{fm:.4f}" if fm is not None else "N/A"
        print(f"  {c['M']:>5}x{c['N']:>5}x{c['K']:>5} {c['cell']:<16s} n_OK={c['n_OK_5']}/5 "
              f"tflops_p50={tf_s} wcf_max={wm_s} fin_min={fm_s}  -> {flag}")
