#!/usr/bin/env python3
"""R34-OptA bench: 5-rep bench at warmup=200 iters=500 trim=10% on L6.

Variants:
  V0 NONVOLATILE_SCALE_X2_POC=0 — sanity (volatile dwordx2)
  V1 SCALE_LOAD_X1=1            — Finding C: 4× single dword
  V3 SCALE_LOAD_X1=1+s_nop=1    — compose with R33-A V4
  V4 incumbent rebuild
  incumbent (canonical .so)
"""
...
def main():
    ...
    p.add_argument("--out", default="R34_OPT_A_BENCH_RESULTS.json")
    args = p.parse_args()

    selected = VARIANTS if args.variants is None else [v for v in VARIANTS if v[0] in args.variants]
    print(f"Bench R34-A: {len(selected)} variants × {args.reps} reps on GPU{args.gpu}")
