#!/usr/bin/env python3
"""R56 Opt G-4 / C2 — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 128256).

R56 G-4 / C2: 128x512 alt-tile probe on L8 (currently AITER 256x256, pct_comp=98.32%).
Alt: 128x512 via R50D shim AS-IS.
...
Per-shape grid: gdx=ceil(N/512)=64, gdy=ceil(M/128)=32, gdz=1, bdx=256
"""
import argparse
...
def main():
    ...
    ap.add_argument("--tag", default="R56_OPT_G4_C2")
    ...
    results = []
    log_lines = []
    log_lines.append(f"R56 Opt G-4 C2 bench - mode={args.mode}, gpu={gpu_list}, "
