#!/usr/bin/env python3
"""R51 Opt D-1 — bench harness for the aiter `.co` dlopen shim on (14336, 4096, 32768).
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 14336, 4096, 32768
COMPETITOR_TFLOPS = 5245.4  # from bench_all_42.py for (14336, 4096, 32768)
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D1_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D1_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R51_OPT_D1_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R51_OPT_D1_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        do_perf = False  # correctness only (first seed gets perf below)

    results = []
    log_lines = []
    log_lines.append(f"R51 Opt D-1 bench — mode={args.mode}, gpu={gpu_list}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
