#!/usr/bin/env python3
"""R50 Opt D — bench harness for the aiter `.co` dlopen shim on (4096, 32768, 28672).
...
This harness uses **aiter's own quant + shuffle utilities** to prepare A, B, A_scale,
B_scale in the layout aiter's f4gemm asm kernel expects. The R50D shim then calls
`hipModuleLaunchKernel` directly on the `.co`.

Reference torch math: FP4 dequant + per-1x32 e8m0 scale → bf16 matmul.
...
WCF_GATE = 0.02
FINITE_GATE = 0.97
M, N, K = 4096, 32768, 28672
COMPETITOR_TFLOPS = 5568.2  # from R44 INTEG manifest
...
def run_one_seed(seed: int, gpu_id: int, do_perf: bool, timeout: int = 700):
...
def main():
    ...
    if args.mode == "smoke":
        out_path = args.out or os.path.join(SCRIPT_DIR, "R50_OPT_D_SMOKE.json")
        log_path = os.path.join(SCRIPT_DIR, "R50_OPT_D_SMOKE.log")
        seeds = [101]
        do_perf = True
    else:
        out_path = args.out or os.path.join(SCRIPT_DIR, "R50_OPT_D_10RUN.json")
        log_path = os.path.join(SCRIPT_DIR, "R50_OPT_D_10RUN.log")
        seeds = [101, 202, 303, 404, 505, 606, 707, 808, 909, 1010]
        # For 10-run we still need correctness on every seed; only the first
        # gets the perf timing to keep wall time reasonable.
        do_perf = False  # correctness only

    results = []
    log_lines = []
    log_lines.append(f"R50 Opt D bench — mode={args.mode}, gpu={args.gpu}, "
                     f"shape=({M},{N},{K})")
    log_lines.append("=" * 90)
