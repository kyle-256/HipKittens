#!/usr/bin/env python3
"""R50 INTEGRATION reviewer bench — 10-run @ 80% INDEPENDENT-seed gate.

Fork of bench_all_42_R44_INTEG.py with these changes:
  ...
  3. Per-shape backend dispatch:
       - (4096, 32768, 28672) -> aiter shim (R50D)
       - (4096, 28672, 32768) -> R50C gm8_lgk2_po28 (HipKittens, standard prep)
       - all other 40 shapes -> R44 baseline manifest entries unchanged
  4. Output defaults: R50_INTEGRATION_10RUN.{json,log}
  5. Round label = R50_INTEGRATION
...
# Aiter shim paths (single override cell)
...
def run_one_pass(gpus, run_label, jobs, seed):
    print(f"R50 INTEG Bench [{run_label}] seed={seed} - GPUs: {gpus}, n_jobs={len(jobs)}")
    print("=" * 110)
...
def main():
    ...
    ap.add_argument("--manifest", default="R50_INTEGRATION_MANIFEST.json")
    ap.add_argument("--out", default="R50_INTEGRATION_10RUN.json")
    ap.add_argument("--log", default="R50_INTEGRATION_10RUN.log")
    args = ap.parse_args()
    ...
    log(f"R50 INTEGRATION bench: {len(jobs)} shapes, runs={args.runs}, GPUs={gpus}")
