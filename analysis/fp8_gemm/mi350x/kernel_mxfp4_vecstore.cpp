...
#define GROUP_SIZE_M 4
    ...
    // ═══════════ Store C — LDS transpose + vectorized global store ═══════════
    // Phase 1: scatter f32→bf16 into LDS in row-major layout (ds_write_b16)
    // Phase 2: gather contiguous rows from LDS (ds_read_b128) → wide global store (global_store_dwordx4)
    // Replaces 256× global_store_short_d16_hi with 32× global_store_dwordx4.
    {
        const int lid = kittens::laneid();
        const int wid = warpid() % _NUM_WARPS;

        // Reinterpret shared tile memory as raw bf16 buffer for LDS transpose.
        // Each warp gets 8192 bytes (64×64×2) at a fixed offset.
        // Total needed: 4 warps × 8192 = 32768 bytes. Available: 128KB+.
        constexpr int LDS_BLK = 64 * 64 * sizeof(bf16); // 8192
        uint32_t lds_warp_base = static_cast<uint32_t>(
            reinterpret_cast<uintptr_t>(&A0_db[0].data[0])) + wid * LDS_BLK;

        // Accumulator layout per thread:
        //   row_off = 4 * (lid / 16)  →  {0, 4, 8, 12}
        //   col_off = lid % 16        →  {0..15}
        const int row_off = 4 * (lid / 16);
        const int col_off = lid % 16;

        // Phase 2 thread assignment:
        //   8 threads per row × 8 bf16 = 64 bf16 = 1 full row
        //   64 threads / 8 = 8 rows per pass, 8 passes = 64 rows
        const int thread_in_row = lid % 8;
        const int row_in_pass   = lid / 8;

        // Lambda for the two-phase store of one 64×64 accumulator block.
        // Uses LDS as a transpose buffer: scatter bf16 values in row-major,
        // then read back contiguous rows for wide global stores.
        auto store_via_lds = [&](const fp4_floatx4_t acc_blk[16], int mh, int nh) __attribute__((always_inline)) {
            // ── Phase 1: Write accumulators → LDS in row-major order ──
            #pragma unroll
            for (int i = 0; i < 4; i++) {
                #pragma unroll
                for (int j = 0; j < 4; j++) {
                    fp4_floatx4_t s = acc_blk[i * 4 + j] * g.scale;
                    const int row_base = i * 16 + row_off;
                    const int col = j * 16 + col_off;
                    #pragma unroll
                    for (int k = 0; k < 4; k++) {
                        uint16_t val = std::bit_cast<uint16_t>(
                            base_types::convertor<bf16, float>::convert(s[k]));
                        uint32_t addr = lds_warp_base +
                            static_cast<uint32_t>((row_base + k) * 64 + col) * 2u;
            ...
            // Wait for all LDS writes within this warp (no barrier needed — warp-private LDS)
            asm volatile("s_waitcnt lgkmcnt(0)");

            // ── Phase 2: Read contiguous rows from LDS → wide global store ──
            ...
            // Pipeline: issue all 8 ds_read_b128, then wait, then store all 8
            float4 lds_data[8];
            #pragma unroll
            for (int pass = 0; pass < 8; pass++) {
                const int row = pass * 8 + row_in_pass;
                const int col_start = thread_in_row * 8;
                uint32_t read_addr = lds_warp_base +
                    static_cast<uint32_t>(row * 64 + col_start) * 2u;
                asm volatile("ds_read_b128 %0, %1"
                    : "=v"(lds_data[pass]) : "v"(read_addr) : "memory");
            }
            asm volatile("s_waitcnt lgkmcnt(0)");
            ...
            for (int pass = 0; pass < 8; pass++) {
                const int row = pass * 8 + row_in_pass;
                const int col_start = thread_in_row * 8;
                *reinterpret_cast<float4*>(&dst_ptr[row * row_stride + col_start]) = lds_data[pass];
            }
        };

        // Each warp uses its own LDS region — no barriers needed between blocks
        store_via_lds(acc_A0Bl, 0, 0);
        store_via_lds(acc_A1Bl, 1, 0);
        store_via_lds(acc_A0Br, 0, 1);
        store_via_lds(acc_A1Br, 1, 1);
    }
}
...
PYBIND11_MODULE(tk_mxfp4_vecstore, m) {
